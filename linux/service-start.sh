...
#   - Example: ALERT_EMAIL="ops@example.com" ./service-start.sh #
#################################################################
...
Start any SERVICES entry that is not running, with optional alerting.
...
# Start every configured service that is not currently running (idempotent).
started=()
for svc in "${SERVICES[@]}"; do
    if pgrep -x "$svc" >/dev/null 2>&1; then
        printf '%b%-20s already running%b\n' "$GREEN" "$svc" "$RST"
    else
        if (( DRY_RUN )); then
