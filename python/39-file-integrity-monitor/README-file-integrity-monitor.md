...
Detect changes to critical files via SHA-256 baselining.

---

## Purpose

Computes SHA-256 for each watched file, compares against the stored baseline, and alerts on any change or removal. The first run only establishes the baseline.

---

## Features

- **SHA-256 baseline of critical files.**
- **Detects modification, addition, and removal.**
- **Self-managing baseline stored beside the script.**
...
- read access to the watched files (root for /etc/shadow)
...
```bash
python3 file-integrity-monitor.py              # run; prints a JSON result on stdout
python3 file-integrity-monitor.py --dry-run    # print configuration and state, do nothing
python3 file-integrity-monitor.py --maintenance # toggle maintenance mode (suppresses alerts)
python3 file-integrity-monitor.py --version    # print version and exit
```
...
|---|---|---|
| `WATCH` | `see script` | List of files to baseline and monitor. |
...
- *Status-aware:* alerts fire once on transition into the alert state and a recovery email is sent once
  when the condition clears (state in `file-integrity-monitor.status`).
