...
# Hashes a watch-list of critical files with SHA-256.                  #
# Stores a baseline and compares against it on each run.               #
# Emits JSON; alerts on changed, new, or removed files.                #
...
VERSION     = "0.1"
SCRIPT_NAME = "file-integrity-monitor"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "file-integrity-monitor-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "file-integrity-monitor-execution.log")
...
def evaluate() -> Tuple[Dict, List[str]]:
    baseline_path = os.path.join(SCRIPT_DIR, SCRIPT_NAME + ".baseline.json")
    cur = {}
    for w in WATCH:
        if os.path.isfile(w):
            s = sha256(w)
    ...
    if prev:
        ...
        for k in prev:
            if k not in cur:
                removed.append(k)
        alerts += [f"File changed: {k}" for k in changed]
        alerts += [f"File removed: {k}" for k in removed]
    try:
        with open(baseline_path, "w") as f:
            json.dump(cur, f)
    except OSError:
        pass
    return {"watched": len(cur), "changed": changed, "new": new,
            "removed": removed, "baseline_existed": bool(prev)}, alerts
