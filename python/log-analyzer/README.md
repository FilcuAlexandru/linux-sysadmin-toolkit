...
Parses log files or journalctl, extracts patterns, detects spikes. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./log-analyzer.py              # run and output JSON to stdout
./log-analyzer.py --dry-run    # show prerequisites and configuration without running
./log-analyzer.py --maintenance # toggle maintenance mode on/off
./log-analyzer.py --version    # print version and exit
./log-analyzer.py --help       # print help
```
...
| File | Content |
|---|---|
| `log-analyzer-execution.log` | Every run: start, result, end. |
| `log-analyzer-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/log-analyzer.py >> /var/log/log-analyzer.json 2>/dev/null
```

### Pipe to jq

```bash
python3 log-analyzer.py | jq '.data'
python3 log-analyzer.py | jq '.alerts[]'
python3 log-analyzer.py | jq '.status'
```

### Checkmk local check

```bash
cp log-analyzer.py /usr/lib/check_mk_agent/local/log-analyzer.py
```
...
|---|---|
| `LOG_FILE` | See inline comment in script. |
| `LINES` | See inline comment in script. |
| `WINDOW_HOURS` | See inline comment in script. |
| `TOP_PATTERNS` | See inline comment in script. |
| `ERROR_SPIKE_MULTIPLIER` | See inline comment in script. |
