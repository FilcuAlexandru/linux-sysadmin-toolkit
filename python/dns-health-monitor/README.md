...
dns-health-monitor.py                                                # Resolves a configurable list of hostnames using the stdlib resolver. # Measures per-target latency.                                         # Emits JSON; alerts on failure or latency above MAX_LATENCY_MS.       #.
...
cd linux-sysadmin-toolkit/python/dns-health-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/dns-health-monitor/dns-health-monitor.py \
     -o /opt/scripts/dns-health-monitor.py
```
...
|---|---|---|
| `TARGETS` | `["example.com", "www.google.com"]` | — |
| `MAX_LATENCY_MS` | `500.0` | alert when a lookup takes longer than this |
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "dns-health-monitor.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "dns-health-monitor.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "dns-health-monitor.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "dns-health-monitor.email.state")
```
...
```
Usage: dns-health-monitor.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 dns-health-monitor.py --dry-run
```
...
python3 dns-health-monitor.py --maintenance
...
dns-health-monitor/
├── dns-health-monitor.py
├── dns-health-monitor.status          # OK / ALERT state
├── dns-health-monitor.email.state     # last-email timestamp
├── dns-health-monitor.lock            # flock instance lock
├── dns-health-monitor.maintenance     # present while maintenance mode is on
├── dns-health-monitor-execution.log   # every run (START / END)
└── dns-health-monitor-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/dns-health-monitor.maintenance"
LOCK_FILE        = "/tmp/dns-health-monitor.lock"
STATUS_FILE      = "/tmp/dns-health-monitor.status"
STATE_FILE       = "/tmp/dns-health-monitor.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: dns-health-monitor
            image: python:3-slim
            command: ["python3", "/scripts/dns-health-monitor.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now dns-health-monitor.timer
```
...
```bash
python3 /opt/scripts/dns-health-monitor.py \
  | jq -r '"dns-health-monitor_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/dns-health-monitor.prom
```
