...
ntp-sync-audit.py                                                    # Verifies the clock is synchronized and within MAX_OFFSET_MS.         # Tries chronyc, ntpq, then timedatectl for portability.               # Emits JSON; alerts on unsynced clock, large offset, or no service.   #.
...
- **`chronyc`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`ntpq`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`timedatectl`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/ntp-sync-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/ntp-sync-audit/ntp-sync-audit.py \
     -o /opt/scripts/ntp-sync-audit.py
```
...
|---|---|---|
| `MAX_OFFSET_MS` | `1000.0` | alert when absolute clock offset exceeds this many milliseconds |
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "ntp-sync-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "ntp-sync-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "ntp-sync-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "ntp-sync-audit.email.state")
```
...
```
Usage: ntp-sync-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 ntp-sync-audit.py --dry-run
```
...
python3 ntp-sync-audit.py --maintenance
...
ntp-sync-audit/
├── ntp-sync-audit.py
├── ntp-sync-audit.status          # OK / ALERT state
├── ntp-sync-audit.email.state     # last-email timestamp
├── ntp-sync-audit.lock            # flock instance lock
├── ntp-sync-audit.maintenance     # present while maintenance mode is on
├── ntp-sync-audit-execution.log   # every run (START / END)
└── ntp-sync-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/ntp-sync-audit.maintenance"
LOCK_FILE        = "/tmp/ntp-sync-audit.lock"
STATUS_FILE      = "/tmp/ntp-sync-audit.status"
STATE_FILE       = "/tmp/ntp-sync-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: ntp-sync-audit
            image: python:3-slim
            command: ["python3", "/scripts/ntp-sync-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now ntp-sync-audit.timer
```
...
```bash
python3 /opt/scripts/ntp-sync-audit.py \
  | jq -r '"ntp-sync-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/ntp-sync-audit.prom
```
