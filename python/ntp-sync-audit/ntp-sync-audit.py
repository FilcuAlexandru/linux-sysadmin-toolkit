...
########################################################################
# ntp-sync-audit.py                                                    #
# Verifies the clock is synchronized and within MAX_OFFSET_MS.         #
# Tries chronyc, ntpq, then timedatectl for portability.               #
# Emits JSON; alerts on unsynced clock, large offset, or no service.   #
...
VERSION     = "0.1"
SCRIPT_NAME = "ntp-sync-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "ntp-sync-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "ntp-sync-audit-execution.log")
...
def collect_ntp() -> Dict:
    ...
    if _have("chronyc"):
        ...
        if out:
            d = {"service": "chrony", "synchronized": None, "offset_ms": None}
            for line in out.splitlines():
                if "Leap status" in line:
                    d["synchronized"] = "normal" in line.lower()
                elif "Last offset" in line:
                    try:
                        d["offset_ms"] = round(abs(float(line.split(":")[1].split()[0])) * 1000, 3)
                    except Exception:
                        pass
            return d
    if _have("ntpq"):
        ...
        for line in out.splitlines():
            if line.startswith("*"):
                p = line.split()
                try:
                    return {"service": "ntpd", "synchronized": True, "offset_ms": round(abs(float(p[8])), 3)}
                except Exception:
                    pass
        if out:
            return {"service": "ntpd", "synchronized": False, "offset_ms": None}
    if _have("timedatectl"):
        s = _run(["timedatectl", "show", "-p", "NTPSynchronized", "--value"]).strip()
        if s:
            return {"service": "timesyncd", "synchronized": s == "yes", "offset_ms": None}
    return {"service": None, "synchronized": False, "offset_ms": None}
...
def evaluate() -> Tuple[Dict, List[str]]:
    ...
    if d["service"] is None:
        alerts.append("No time-sync service detected")
    else:
        if d["synchronized"] is False:
            alerts.append(f"Clock not synchronized ({d['service']})")
        if d["offset_ms"] is not None and d["offset_ms"] > MAX_OFFSET_MS:
            alerts.append(f"Clock offset {d['offset_ms']}ms exceeds {MAX_OFFSET_MS}ms")
    return {"ntp": d, "max_offset_ms": MAX_OFFSET_MS}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"max_offset_ms": MAX_OFFSET_MS,
