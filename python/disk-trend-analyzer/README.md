...
Linear regression on disk usage history to predict filesystem full date. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./disk-trend-analyzer.py              # run and output JSON to stdout
./disk-trend-analyzer.py --dry-run    # show prerequisites and configuration without running
./disk-trend-analyzer.py --maintenance # toggle maintenance mode on/off
./disk-trend-analyzer.py --version    # print version and exit
./disk-trend-analyzer.py --help       # print help
```
...
| File | Content |
|---|---|
| `disk-trend-analyzer-execution.log` | Every run: start, result, end. |
| `disk-trend-analyzer-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/disk-trend-analyzer.py >> /var/log/disk-trend-analyzer.json 2>/dev/null
```

### Pipe to jq

```bash
python3 disk-trend-analyzer.py | jq '.data'
python3 disk-trend-analyzer.py | jq '.alerts[]'
python3 disk-trend-analyzer.py | jq '.status'
```

### Checkmk local check

```bash
cp disk-trend-analyzer.py /usr/lib/check_mk_agent/local/disk-trend-analyzer.py
```
...
| `MAX_HISTORY` | See inline comment in script. |
| `ALERT_DAYS` | See inline comment in script. |
