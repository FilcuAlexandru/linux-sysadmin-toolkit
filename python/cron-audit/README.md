...
cron-audit.py                                                         # Inventories all crontab entries across the system: /etc/crontab,      # /etc/cron.d/*, /etc/cron.{hourly,daily,weekly,monthly}/, and all user # crontabs in /var/spool/cron/crontabs/. Validates that each referenced # script exists and is executable. Outputs a structured JSON report.    #.
...
cd linux-sysadmin-toolkit/python/cron-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/cron-audit/cron-audit.py \
     -o /opt/scripts/cron-audit.py
```
...
```
Usage: cron-audit.py [--dry-run] [--maintenance] [--version]
...
python3 cron-audit.py --maintenance
...
cron-audit/
├── cron-audit.py
├── cron-audit.status          # OK / ALERT state
├── cron-audit.email.state     # last-email timestamp
├── cron-audit.lock            # flock instance lock
├── cron-audit.maintenance     # present while maintenance mode is on
├── cron-audit-execution.log   # every run (START / END)
└── cron-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/cron-audit.maintenance"
LOCK_FILE        = "/tmp/cron-audit.lock"
STATUS_FILE      = "/tmp/cron-audit.status"
STATE_FILE       = "/tmp/cron-audit.email.state"
```
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/cron-audit.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/cron-audit.service
[Unit]
Description=cron-audit check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/cron-audit.py
```

```ini
# /etc/systemd/system/cron-audit.timer
[Unit]
Description=Run cron-audit every 5 minutes
...
```bash
sudo systemctl enable --now cron-audit.timer
```
...
```bash
python3 /opt/scripts/cron-audit.py \
  | jq -r '"cron-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/cron-audit.prom
```
