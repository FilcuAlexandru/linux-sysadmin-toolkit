...
Aggregates data from other toolkit scripts into a daily summary report. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./system-report.py              # run and output JSON to stdout
./system-report.py --dry-run    # show prerequisites and configuration without running
./system-report.py --maintenance # toggle maintenance mode on/off
./system-report.py --version    # print version and exit
./system-report.py --help       # print help
```
...
| File | Content |
|---|---|
| `system-report-execution.log` | Every run: start, result, end. |
| `system-report-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/system-report.py >> /var/log/system-report.json 2>/dev/null
```

### Pipe to jq

```bash
python3 system-report.py | jq '.data'
python3 system-report.py | jq '.alerts[]'
python3 system-report.py | jq '.status'
```

### Checkmk local check

```bash
cp system-report.py /usr/lib/check_mk_agent/local/system-report.py
```
...
|---|---|
| `SYSTEM_MONITOR_OUTPUT` | See inline comment in script. |
| `USER_AUDIT_OUTPUT` | See inline comment in script. |
| `PORTS_AUDIT_OUTPUT` | See inline comment in script. |
| `FAILED_LOGIN_OUTPUT` | See inline comment in script. |
