...
########################################################################
# sudoers-audit.py                                                     #
# Parses /etc/sudoers and /etc/sudoers.d for risky directives.         #
# Flags NOPASSWD grants and overly broad rules.                        #
# Emits JSON; alerts on each risky finding (requires root to read).    #
...
VERSION     = "0.1"
SCRIPT_NAME = "sudoers-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "sudoers-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "sudoers-audit-execution.log")
...
def read_sudoers() -> List[Tuple[str, str]]:
    """Read sudoers."""
    files = ["/etc/sudoers"]
    d = "/etc/sudoers.d"
    if os.path.isdir(d):
    ...
    lines = []
    for f in files:
        for ln in _read_file(f).splitlines():
            s = ln.strip()
...
def evaluate() -> Tuple[Dict, List[str]]:
    ...
    for f, ln in rules:
        up = ln.upper()
        if FLAG_NOPASSWD and "NOPASSWD" in up:
            findings.append({"file": f, "rule": ln, "issue": "NOPASSWD"})
        elif "ALL=(ALL" in up.replace(" ", "") and not up.startswith("DEFAULTS") and "%" not in ln:
            findings.append({"file": f, "rule": ln, "issue": "broad-grant"})
    alerts = [f"{x['issue']} in {os.path.basename(x['file'])}: {x['rule']}" for x in findings]
    return {"rule_count": len(rules), "findings": findings}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"checks": "NOPASSWD and broad sudo grants",
