...
sudoers-audit.py                                                     # Parses /etc/sudoers and /etc/sudoers.d for risky directives.         # Flags NOPASSWD grants and overly broad rules.                        # Emits JSON; alerts on each risky finding (requires root to read).    #.
...
cd linux-sysadmin-toolkit/python/sudoers-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/sudoers-audit/sudoers-audit.py \
     -o /opt/scripts/sudoers-audit.py
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "sudoers-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "sudoers-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "sudoers-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "sudoers-audit.email.state")
```
...
```
Usage: sudoers-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 sudoers-audit.py --dry-run
```
...
python3 sudoers-audit.py --maintenance
...
sudoers-audit/
├── sudoers-audit.py
├── sudoers-audit.status          # OK / ALERT state
├── sudoers-audit.email.state     # last-email timestamp
├── sudoers-audit.lock            # flock instance lock
├── sudoers-audit.maintenance     # present while maintenance mode is on
├── sudoers-audit-execution.log   # every run (START / END)
└── sudoers-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/sudoers-audit.maintenance"
LOCK_FILE        = "/tmp/sudoers-audit.lock"
STATUS_FILE      = "/tmp/sudoers-audit.status"
STATE_FILE       = "/tmp/sudoers-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: sudoers-audit
            image: python:3-slim
            command: ["python3", "/scripts/sudoers-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now sudoers-audit.timer
```
...
```bash
python3 /opt/scripts/sudoers-audit.py \
  | jq -r '"sudoers-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/sudoers-audit.prom
```
