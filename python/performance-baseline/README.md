...
performance-baseline.py                                                     # Captures a performance baseline: CPU usage, iowait%, memory percent, load   # averages, and network throughput. Compares with a saved baseline and alerts # on significant deviations (> DEVIATION_PCT%). Run once to establish the     # baseline; subsequent runs compare against it.                               #.
...
cd linux-sysadmin-toolkit/python/performance-baseline

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/performance-baseline/performance-baseline.py \
     -o /opt/scripts/performance-baseline.py
```

### Manual copy

```bash
cp performance-baseline.py /opt/scripts/performance-baseline.py
chmod +x /opt/scripts/performance-baseline.py
```

### Verify

```bash
python3 /opt/scripts/performance-baseline.py --version
python3 /opt/scripts/performance-baseline.py --dry-run
```
...
```
Usage: performance-baseline.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 performance-baseline.py --dry-run
```
...
python3 performance-baseline.py --maintenance
...
├── performance-baseline.py
├── performance-baseline.status          # OK / ALERT state
├── performance-baseline.email.state     # last-email timestamp
├── performance-baseline.lock            # flock instance lock
├── performance-baseline.maintenance     # present while maintenance mode is on
├── performance-baseline-execution.log   # every run (START / END)
└── performance-baseline-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/performance-baseline.maintenance"
LOCK_FILE        = "/tmp/performance-baseline.lock"
STATUS_FILE      = "/tmp/performance-baseline.status"
STATE_FILE       = "/tmp/performance-baseline.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: performance-baseline
            image: python:3-slim
            command: ["python3", "/scripts/performance-baseline.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/performance-baseline.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/performance-baseline.service
[Unit]
Description=performance-baseline check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/performance-baseline.py
```

```ini
# /etc/systemd/system/performance-baseline.timer
[Unit]
Description=Run performance-baseline every 5 minutes
...
```bash
sudo systemctl enable --now performance-baseline.timer
```
...
```bash
python3 /opt/scripts/performance-baseline.py \
  | jq -r '"performance-baseline_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/performance-baseline.prom
```
