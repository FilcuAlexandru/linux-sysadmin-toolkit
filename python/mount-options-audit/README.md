...
mount-options-audit.py                                                      # Checks that sensitive mounts carry hardening options (nosuid/nodev/noexec). # Parses /proc/mounts directly.                                               # Emits JSON; alerts on any missing recommended option.                       #.
...
cd linux-sysadmin-toolkit/python/mount-options-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/mount-options-audit/mount-options-audit.py \
     -o /opt/scripts/mount-options-audit.py
```
...
```
Usage: mount-options-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 mount-options-audit.py
```
...
```bash
python3 mount-options-audit.py --dry-run
```
...
python3 mount-options-audit.py --maintenance
...
mount-options-audit/
├── mount-options-audit.py
├── mount-options-audit.status          # OK / ALERT state
├── mount-options-audit.email.state     # last-email timestamp
├── mount-options-audit.lock            # flock instance lock
├── mount-options-audit.maintenance     # present while maintenance mode is on
├── mount-options-audit-execution.log   # every run (START / END)
└── mount-options-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/mount-options-audit.maintenance"
LOCK_FILE        = "/tmp/mount-options-audit.lock"
STATUS_FILE      = "/tmp/mount-options-audit.status"
STATE_FILE       = "/tmp/mount-options-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: mount-options-audit
            image: python:3-slim
            command: ["python3", "/scripts/mount-options-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now mount-options-audit.timer
```
...
```bash
python3 /opt/scripts/mount-options-audit.py \
  | jq -r '"mount-options-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/mount-options-audit.prom
```
