...
Lists all systemd services with state and alerts on newly failed units. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./service-inventory.py              # run and output JSON to stdout
./service-inventory.py --dry-run    # show prerequisites and configuration without running
./service-inventory.py --maintenance # toggle maintenance mode on/off
./service-inventory.py --version    # print version and exit
./service-inventory.py --help       # print help
```
...
| File | Content |
|---|---|
| `service-inventory-execution.log` | Every run: start, result, end. |
| `service-inventory-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/service-inventory.py >> /var/log/service-inventory.json 2>/dev/null
```

### Pipe to jq

```bash
python3 service-inventory.py | jq '.data'
python3 service-inventory.py | jq '.alerts[]'
python3 service-inventory.py | jq '.status'
```

### Checkmk local check

```bash
cp service-inventory.py /usr/lib/check_mk_agent/local/service-inventory.py
```
