...
service-inventory.py                                                         # Lists all systemd services with their active state (active/inactive/failed), # enabled/disabled status, and unit file path. Groups by state for quick       # overview. Saves a snapshot of service states and alerts when new failed      # units appear since the last run.                                             #.
...
cd linux-sysadmin-toolkit/python/service-inventory

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/service-inventory/service-inventory.py \
     -o /opt/scripts/service-inventory.py
```

### Manual copy

```bash
cp service-inventory.py /opt/scripts/service-inventory.py
chmod +x /opt/scripts/service-inventory.py
```

### Verify

```bash
python3 /opt/scripts/service-inventory.py --version
python3 /opt/scripts/service-inventory.py --dry-run
```
...
```
Usage: service-inventory.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 service-inventory.py --dry-run
```
...
python3 service-inventory.py --maintenance
...
service-inventory/
├── service-inventory.py
├── service-inventory.status          # OK / ALERT state
├── service-inventory.email.state     # last-email timestamp
├── service-inventory.lock            # flock instance lock
├── service-inventory.maintenance     # present while maintenance mode is on
├── service-inventory-execution.log   # every run (START / END)
└── service-inventory-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/service-inventory.maintenance"
LOCK_FILE        = "/tmp/service-inventory.lock"
STATUS_FILE      = "/tmp/service-inventory.status"
STATE_FILE       = "/tmp/service-inventory.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: service-inventory
            image: python:3-slim
            command: ["python3", "/scripts/service-inventory.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/service-inventory.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/service-inventory.service
[Unit]
Description=service-inventory check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/service-inventory.py
```

```ini
# /etc/systemd/system/service-inventory.timer
[Unit]
Description=Run service-inventory every 5 minutes
...
```bash
sudo systemctl enable --now service-inventory.timer
```
...
```bash
python3 /opt/scripts/service-inventory.py \
  | jq -r '"service-inventory_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/service-inventory.prom
```
