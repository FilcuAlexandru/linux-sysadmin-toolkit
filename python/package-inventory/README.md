...
package-inventory.py                                                     # Lists all installed packages across distributions (dpkg/rpm/apk/zypper). # Auto-detects the package manager. Saves a JSON snapshot on each run and  # diffs against the previous run to report installed and removed packages. # Outputs JSON with full package list and change summary.                  #.
...
cd linux-sysadmin-toolkit/python/package-inventory

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/package-inventory/package-inventory.py \
     -o /opt/scripts/package-inventory.py
```

### Manual copy

```bash
cp package-inventory.py /opt/scripts/package-inventory.py
chmod +x /opt/scripts/package-inventory.py
```

### Verify

```bash
python3 /opt/scripts/package-inventory.py --version
python3 /opt/scripts/package-inventory.py --dry-run
```
...
```python
SNAPSHOT_FILE = os.path.join(SCRIPT_DIR, "package-inventory.snapshot.json")
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "package-inventory.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "package-inventory.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "package-inventory.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "package-inventory.email.state")
```
...
```
Usage: package-inventory.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 package-inventory.py --dry-run
```
...
python3 package-inventory.py --maintenance
...
package-inventory/
├── package-inventory.py
├── package-inventory.status          # OK / ALERT state
├── package-inventory.email.state     # last-email timestamp
├── package-inventory.lock            # flock instance lock
├── package-inventory.maintenance     # present while maintenance mode is on
├── package-inventory-execution.log   # every run (START / END)
└── package-inventory-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/package-inventory.maintenance"
LOCK_FILE        = "/tmp/package-inventory.lock"
STATUS_FILE      = "/tmp/package-inventory.status"
STATE_FILE       = "/tmp/package-inventory.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: package-inventory
            image: python:3-slim
            command: ["python3", "/scripts/package-inventory.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/package-inventory.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/package-inventory.service
[Unit]
Description=package-inventory check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/package-inventory.py
```

```ini
# /etc/systemd/system/package-inventory.timer
[Unit]
Description=Run package-inventory every 5 minutes
...
```bash
sudo systemctl enable --now package-inventory.timer
```
...
```bash
python3 /opt/scripts/package-inventory.py \
  | jq -r '"package-inventory_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/package-inventory.prom
```
