...
Lists installed packages cross-distro and diffs against previous snapshot. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./package-inventory.py              # run and output JSON to stdout
./package-inventory.py --dry-run    # show prerequisites and configuration without running
./package-inventory.py --maintenance # toggle maintenance mode on/off
./package-inventory.py --version    # print version and exit
./package-inventory.py --help       # print help
```
...
| File | Content |
|---|---|
| `package-inventory-execution.log` | Every run: start, result, end. |
| `package-inventory-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/package-inventory.py >> /var/log/package-inventory.json 2>/dev/null
```

### Pipe to jq

```bash
python3 package-inventory.py | jq '.data'
python3 package-inventory.py | jq '.alerts[]'
python3 package-inventory.py | jq '.status'
```

### Checkmk local check

```bash
cp package-inventory.py /usr/lib/check_mk_agent/local/package-inventory.py
```
