...
# Resolves a configurable list of hostnames using the stdlib resolver. #
# Measures per-target latency.                                         #
# Emits JSON; alerts on failure or latency above MAX_LATENCY_MS.       #
...
VERSION     = "0.1"
SCRIPT_NAME = "dns-health-monitor"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "dns-health-monitor-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "dns-health-monitor-execution.log")
...
def evaluate() -> Tuple[Dict, List[str]]:
    ...
    for t in TARGETS:
        ...
        ok, err = True, ""
        try:
            socket.getaddrinfo(t, None)
        except Exception as exc:
            ok, err = False, str(exc)
        ms = round((time.time() - start) * 1000, 1)
        results.append({"target": t, "resolved": ok, "latency_ms": ms, "error": err})
        if not ok:
            alerts.append(f"DNS resolution failed for {t}: {err}")
        elif ms > MAX_LATENCY_MS:
            alerts.append(f"DNS slow for {t}: {ms}ms exceeds {MAX_LATENCY_MS}ms")
    return {"targets": results, "max_latency_ms": MAX_LATENCY_MS}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"targets": TARGETS, "max_latency_ms": MAX_LATENCY_MS,
