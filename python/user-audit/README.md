...
Audits system users with risk scoring across passwd, shadow, sudoers, and login history. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./user-audit.py              # run and output JSON to stdout
./user-audit.py --dry-run    # show prerequisites and configuration without running
./user-audit.py --maintenance # toggle maintenance mode on/off
./user-audit.py --version    # print version and exit
./user-audit.py --help       # print help
```
...
| File | Content |
|---|---|
| `user-audit-execution.log` | Every run: start, result, end. |
| `user-audit-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/user-audit.py >> /var/log/user-audit.json 2>/dev/null
```

### Pipe to jq

```bash
python3 user-audit.py | jq '.data'
python3 user-audit.py | jq '.alerts[]'
python3 user-audit.py | jq '.status'
```

### Checkmk local check

```bash
cp user-audit.py /usr/lib/check_mk_agent/local/user-audit.py
```
...
| `INCLUDE_SYSTEM_USERS` | See inline comment in script. |
| `PASSWORD_WARN_DAYS` | See inline comment in script. |
| `LAST_LINES` | See inline comment in script. |
