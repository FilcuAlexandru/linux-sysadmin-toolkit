...
user-audit.py                                                         # Audits system users by parsing /etc/passwd, /etc/shadow, /etc/group,  # and /etc/sudoers. Cross-references with last/lastb login history.     # Assigns a risk score per user (HIGH/MEDIUM/LOW) based on sudo access, # missing passwords, locked accounts, and failed login counts.          #.
...
- **`last`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`lastb`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/user-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/user-audit/user-audit.py \
     -o /opt/scripts/user-audit.py
```

### Manual copy

```bash
cp user-audit.py /opt/scripts/user-audit.py
chmod +x /opt/scripts/user-audit.py
```

### Verify

```bash
python3 /opt/scripts/user-audit.py --version
python3 /opt/scripts/user-audit.py --dry-run
```
...
```
Usage: user-audit.py [--dry-run] [--maintenance] [--version]
...
python3 user-audit.py --maintenance
...
user-audit/
├── user-audit.py
├── user-audit.status          # OK / ALERT state
├── user-audit.email.state     # last-email timestamp
├── user-audit.lock            # flock instance lock
├── user-audit.maintenance     # present while maintenance mode is on
├── user-audit-execution.log   # every run (START / END)
└── user-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/user-audit.maintenance"
LOCK_FILE        = "/tmp/user-audit.lock"
STATUS_FILE      = "/tmp/user-audit.status"
STATE_FILE       = "/tmp/user-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: user-audit
            image: python:3-slim
            command: ["python3", "/scripts/user-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/user-audit.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/user-audit.service
[Unit]
Description=user-audit check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/user-audit.py
```

```ini
# /etc/systemd/system/user-audit.timer
[Unit]
Description=Run user-audit every 5 minutes
...
```bash
sudo systemctl enable --now user-audit.timer
```
...
```bash
python3 /opt/scripts/user-audit.py \
  | jq -r '"user-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/user-audit.prom
```
