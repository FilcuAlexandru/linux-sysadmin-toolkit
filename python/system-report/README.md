...
system-report.py                                                            # Generates a comprehensive daily system report by collecting CPU/memory/disk # inline and optionally loading cached JSON from other toolkit scripts.       # Aggregates user risk, failed logins, and disk alerts into a single          # daily JSON report with a summary section.                                   #.
...
cd linux-sysadmin-toolkit/python/system-report

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/system-report/system-report.py \
     -o /opt/scripts/system-report.py
```

### Manual copy

```bash
cp system-report.py /opt/scripts/system-report.py
chmod +x /opt/scripts/system-report.py
```

### Verify

```bash
python3 /opt/scripts/system-report.py --version
python3 /opt/scripts/system-report.py --dry-run
```
...
```
Usage: system-report.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 system-report.py --dry-run
```
...
python3 system-report.py --maintenance
...
system-report/
├── system-report.py
├── system-report.status          # OK / ALERT state
├── system-report.email.state     # last-email timestamp
├── system-report.lock            # flock instance lock
├── system-report.maintenance     # present while maintenance mode is on
├── system-report-execution.log   # every run (START / END)
└── system-report-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/system-report.maintenance"
LOCK_FILE        = "/tmp/system-report.lock"
STATUS_FILE      = "/tmp/system-report.status"
STATE_FILE       = "/tmp/system-report.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: system-report
            image: python:3-slim
            command: ["python3", "/scripts/system-report.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/system-report.py >/dev/null 2>&1
```
...
[Unit]
Description=system-report check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/system-report.py
```
...
```bash
python3 /opt/scripts/system-report.py \
  | jq -r '"system-report_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/system-report.prom
```
