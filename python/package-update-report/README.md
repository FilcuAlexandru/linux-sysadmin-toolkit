...
package-update-report.py                                             # Counts and lists pending updates on apt, dnf, yum, or zypper.        # Read-only: never installs anything.                                  # Emits JSON; alerts when updates exceed UPDATE_THRESHOLD.             #.
...
- **`apt-get`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`dnf`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`yum`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`zypper`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/package-update-report

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/package-update-report/package-update-report.py \
     -o /opt/scripts/package-update-report.py
```
...
|---|---|---|
| `UPDATE_THRESHOLD` | `20` | alert when available updates exceed this (0 = any update) |
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "package-update-report.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "package-update-report.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "package-update-report.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "package-update-report.email.state")
```
...
```
Usage: package-update-report.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 package-update-report.py
```
...
```bash
python3 package-update-report.py --dry-run
```
...
python3 package-update-report.py --maintenance
...
├── package-update-report.py
├── package-update-report.status          # OK / ALERT state
├── package-update-report.email.state     # last-email timestamp
├── package-update-report.lock            # flock instance lock
├── package-update-report.maintenance     # present while maintenance mode is on
├── package-update-report-execution.log   # every run (START / END)
└── package-update-report-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/package-update-report.maintenance"
LOCK_FILE        = "/tmp/package-update-report.lock"
STATUS_FILE      = "/tmp/package-update-report.status"
STATE_FILE       = "/tmp/package-update-report.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: package-update-report
            image: python:3-slim
            command: ["python3", "/scripts/package-update-report.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now package-update-report.timer
```
...
```bash
python3 /opt/scripts/package-update-report.py \
  | jq -r '"package-update-report_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/package-update-report.prom
```
