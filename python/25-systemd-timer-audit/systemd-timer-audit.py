...
# Inventories systemd timers via systemctl list-timers.                #
# Detects failed timer units.                                          #
# Emits JSON; alerts when any timer unit has failed.                   #
...
VERSION     = "0.1"
SCRIPT_NAME = "systemd-timer-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "systemd-timer-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "systemd-timer-audit-execution.log")
...
def collect_timers() -> Dict:
    if not _have("systemctl"):
        return {"available": False, "timer_count": 0, "failed": []}
    flags = ["--all"] if INCLUDE_INACTIVE else []
    out = _run(["systemctl", "list-timers", *flags, "--no-legend", "--no-pager"])
    count = len([l for l in out.splitlines() if l.strip()])
    fout = _run(["systemctl", "--failed", "--type=timer", "--no-legend", "--no-pager"])
    failed = [l.split()[0] for l in fout.splitlines() if l.strip()]
    return {"available": True, "timer_count": count, "failed": failed}


def evaluate() -> Tuple[Dict, List[str]]:
    d = collect_timers()
    alerts = []
    if d.get("available") and d["failed"]:
        alerts.append("Failed timers: " + ", ".join(d["failed"]))
    return {"timers": d}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"checks": "systemd timer states",
