...
failed-login-analyzer.py                                                  # Counts failed SSH login attempts in a configurable time window and alerts # when the count exceeds THRESHOLD. Uses journalctl as primary source with  # /var/log/auth.log as fallback. Extracts top attacker IPs, targeted        # usernames, and generates an hourly heatmap. Status-aware alerting.        #.
...
cd linux-sysadmin-toolkit/python/failed-login-analyzer

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/failed-login-analyzer/failed-login-analyzer.py \
     -o /opt/scripts/failed-login-analyzer.py
```
...
```
Usage: failed-login-analyzer.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 failed-login-analyzer.py
```
...
```bash
python3 failed-login-analyzer.py --dry-run
```
...
python3 failed-login-analyzer.py --maintenance
...
├── failed-login-analyzer.py
├── failed-login-analyzer.status          # OK / ALERT state
├── failed-login-analyzer.email.state     # last-email timestamp
├── failed-login-analyzer.lock            # flock instance lock
├── failed-login-analyzer.maintenance     # present while maintenance mode is on
├── failed-login-analyzer-execution.log   # every run (START / END)
└── failed-login-analyzer-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/failed-login-analyzer.maintenance"
LOCK_FILE        = "/tmp/failed-login-analyzer.lock"
STATUS_FILE      = "/tmp/failed-login-analyzer.status"
STATE_FILE       = "/tmp/failed-login-analyzer.email.state"
```
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/failed-login-analyzer.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/failed-login-analyzer.service
[Unit]
Description=failed-login-analyzer check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/failed-login-analyzer.py
```

```ini
# /etc/systemd/system/failed-login-analyzer.timer
[Unit]
Description=Run failed-login-analyzer every 5 minutes
...
```bash
sudo systemctl enable --now failed-login-analyzer.timer
```
...
```bash
python3 /opt/scripts/failed-login-analyzer.py \
  | jq -r '"failed-login-analyzer_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/failed-login-analyzer.prom
```
