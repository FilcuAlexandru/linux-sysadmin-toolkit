...
########################################################################
# sysctl-audit.py                                                      #
# Compares selected sysctl keys against recommended values.            #
# Reads from /proc/sys with a sysctl fallback.                         #
# Emits JSON; alerts on any deviation from the baseline.               #
...
VERSION     = "0.1"
SCRIPT_NAME = "sysctl-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "sysctl-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "sysctl-audit-execution.log")
...
def evaluate() -> Tuple[Dict, List[str]]:
    report = {}
    alerts = []
    for k, exp in RECOMMENDED.items():
        cur = sysctl_get(k)
        ok = (cur == str(exp))
        report[k] = {"current": cur, "expected": str(exp), "ok": ok}
        if cur and not ok:
            alerts.append(f"sysctl {k}={cur} (recommended {exp})")
    return {"sysctl": report}, alerts
