...
Monitors processes for crashes, D-state hangs, zombies, and memory leaks. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./process-monitor.py              # run and output JSON to stdout
./process-monitor.py --dry-run    # show prerequisites and configuration without running
./process-monitor.py --maintenance # toggle maintenance mode on/off
./process-monitor.py --version    # print version and exit
./process-monitor.py --help       # print help
```
...
| File | Content |
|---|---|
| `process-monitor-execution.log` | Every run: start, result, end. |
| `process-monitor-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/process-monitor.py >> /var/log/process-monitor.json 2>/dev/null
```

### Pipe to jq

```bash
python3 process-monitor.py | jq '.data'
python3 process-monitor.py | jq '.alerts[]'
python3 process-monitor.py | jq '.status'
```

### Checkmk local check

```bash
cp process-monitor.py /usr/lib/check_mk_agent/local/process-monitor.py
```
...
|---|---|
| `PROCESSES` | See inline comment in script. |
| `MEMORY_GROWTH_MB` | See inline comment in script. |
| `MEMORY_HISTORY` | See inline comment in script. |
| `MEMORY_HISTORY_FILE` | See inline comment in script. |
