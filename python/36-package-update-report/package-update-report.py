...
# Counts and lists pending updates on apt, dnf, yum, or zypper.        #
# Read-only: never installs anything.                                  #
# Emits JSON; alerts when updates exceed UPDATE_THRESHOLD.             #
...
VERSION     = "0.1"
SCRIPT_NAME = "package-update-report"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "package-update-report-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "package-update-report-execution.log")
...
def collect_updates() -> Tuple[Optional[str], List[str]]:
    if _have("apt-get"):
        out = _run(["apt-get", "--just-print", "upgrade"], timeout=60)
        return "apt", [l.split()[1] for l in out.splitlines() if l.startswith("Inst")]
    if _have("dnf"):
        out = _run(["dnf", "-q", "check-update"], timeout=120)
        return "dnf", [l.split()[0] for l in out.splitlines()
                       if l[:1].isalnum() and len(l.split()) >= 3]
    if _have("yum"):
        out = _run(["yum", "-q", "check-update"], timeout=120)
        return "yum", [l.split()[0] for l in out.splitlines()
                       if l[:1].isalnum() and len(l.split()) >= 3]
    if _have("zypper"):
        out = _run(["zypper", "-q", "list-updates"], timeout=120)
        return "zypper", [l.split("|")[2].strip() for l in out.splitlines() if l.startswith("v |")]
    return None, []


def evaluate() -> Tuple[Dict, List[str]]:
    mgr, pkgs = collect_updates()
    alerts = []
    if mgr and len(pkgs) > UPDATE_THRESHOLD:
        alerts.append(f"{len(pkgs)} updates available ({mgr}) exceed {UPDATE_THRESHOLD}")
    return {"manager": mgr, "update_count": len(pkgs), "updates": pkgs[:200]}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"update_threshold": UPDATE_THRESHOLD,
