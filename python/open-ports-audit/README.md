...
open-ports-audit.py                                                        # Lists all listening TCP/UDP ports via ss(8), compares against a configured # expected port list, and alerts on unexpected open ports. Saves a JSON      # snapshot on each run and diffs against the previous run to report ports    # that have opened or closed since the last check.                           #.
...
- **`netstat`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`ss`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/open-ports-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/open-ports-audit/open-ports-audit.py \
     -o /opt/scripts/open-ports-audit.py
```

### Manual copy

```bash
cp open-ports-audit.py /opt/scripts/open-ports-audit.py
chmod +x /opt/scripts/open-ports-audit.py
```

### Verify

```bash
python3 /opt/scripts/open-ports-audit.py --version
python3 /opt/scripts/open-ports-audit.py --dry-run
```
...
### Set to [] to disable expected-ports check and only track changes.
...
```python
SNAPSHOT_FILE = os.path.join(SCRIPT_DIR, "open-ports-audit.snapshot.json")
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "open-ports-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "open-ports-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "open-ports-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "open-ports-audit.email.state")
```
...
```
Usage: open-ports-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 open-ports-audit.py --dry-run
```
...
python3 open-ports-audit.py --maintenance
...
open-ports-audit/
├── open-ports-audit.py
├── open-ports-audit.status          # OK / ALERT state
├── open-ports-audit.email.state     # last-email timestamp
├── open-ports-audit.lock            # flock instance lock
├── open-ports-audit.maintenance     # present while maintenance mode is on
├── open-ports-audit-execution.log   # every run (START / END)
└── open-ports-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/open-ports-audit.maintenance"
LOCK_FILE        = "/tmp/open-ports-audit.lock"
STATUS_FILE      = "/tmp/open-ports-audit.status"
STATE_FILE       = "/tmp/open-ports-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: open-ports-audit
            image: python:3-slim
            command: ["python3", "/scripts/open-ports-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/open-ports-audit.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/open-ports-audit.service
[Unit]
Description=open-ports-audit check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/open-ports-audit.py
```

```ini
# /etc/systemd/system/open-ports-audit.timer
[Unit]
Description=Run open-ports-audit every 5 minutes
...
```bash
sudo systemctl enable --now open-ports-audit.timer
```
...
```bash
python3 /opt/scripts/open-ports-audit.py \
  | jq -r '"open-ports-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/open-ports-audit.prom
```
