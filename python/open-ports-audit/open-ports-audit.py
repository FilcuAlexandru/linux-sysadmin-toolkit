...
##############################################################################
# open-ports-audit.py                                                        #
# Lists all listening TCP/UDP ports via ss(8), compares against a configured #
# expected port list, and alerts on unexpected open ports. Saves a JSON      #
# snapshot on each run and diffs against the previous run to report ports    #
# that have opened or closed since the last check.                           #
...
# Expected ports — anything outside this list triggers an alert.
...
VERSION     = "0.1"
SCRIPT_NAME = "open-ports-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "open-ports-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "open-ports-audit-execution.log")
...
def collect_listening_ports() -> List[Dict]:
    ...
    for proto, flag in (("tcp", "-tlnp"), ("udp", "-ulnp")):
        try:
            out = subprocess.run(["ss"] + flag.split(),
                                 capture_output=True, text=True, timeout=10).stdout
        except Exception:
            try:
                out = subprocess.run(["netstat", "-" + flag.replace("-", "")],
                                     capture_output=True, text=True, timeout=10).stdout
            except Exception:
                continue
        for line in out.splitlines()[1:]:
            ...
            ports.append({"proto": proto, "port": int(port_str),
                "address": local.rsplit(":", 1)[0],
                "key": f"{proto}:{port_str}", "process": proc})
    seen = set(); unique = []
    for p in ports:
        if p["key"] not in seen:
            seen.add(p["key"]); unique.append(p)
    return sorted(unique, key=lambda x: (x["proto"], x["port"]))
...
def save_snapshot(port_keys: List[str]) -> None:
    """Write the current snapshot to disk."""
    try:
        with open(SNAPSHOT_FILE, "w") as f:
            json.dump(port_keys, f)
    except OSError:
        pass
...
def _collect_all() -> Tuple[Dict, List[str]]:
    ...
    prev_keys  = load_snapshot()
    save_snapshot(port_keys)
    alerts     = []; unexpected = []
    if EXPECTED_PORTS:
        for p in ports:
            if p["key"] not in EXPECTED_PORTS:
                unexpected.append(p)
                alerts.append(f"Unexpected port: {p['key']} "
                               f"({p['address']}) {p['process']}")
    new_ports    = [k for k in port_keys  if k not in prev_keys]
    closed_ports = [k for k in prev_keys  if k not in port_keys]
    for k in new_ports:
        if k not in EXPECTED_PORTS:
            alerts.append(f"New port since last run: {k}")
    return {
        "ports": ports, "unexpected": unexpected,
        "new_since_last_run":    new_ports,
        "closed_since_last_run": closed_ports,
        "expected_ports":        EXPECTED_PORTS,
        "summary": {"total_listening": len(ports), "unexpected": len(unexpected),
            "new_ports": len(new_ports), "closed_ports": len(closed_ports)},
    }, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"expected_ports": EXPECTED_PORTS, "snapshot_file": SNAPSHOT_FILE, "maintenance": is_maintenance(), "current_status": get_status()},
