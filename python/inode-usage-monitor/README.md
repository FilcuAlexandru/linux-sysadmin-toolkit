...
Monitor filesystem inode usage and alert per-filesystem.

---

## Purpose

Runs `df -iP`, parses the `IUse%` column for every real filesystem, and raises one alert per filesystem that exceeds `INODE_THRESHOLD`.

---

## Features

- **Inode-aware: detects exhaustion separate from byte usage.**
- **Per-filesystem reporting with worst-case visibility.**
- **Skips pseudo filesystems (tmpfs, proc, cgroup, overlay).**
...
```bash
python3 inode-usage-monitor.py              # run; prints a JSON result on stdout
python3 inode-usage-monitor.py --dry-run    # print configuration and state, do nothing
python3 inode-usage-monitor.py --maintenance # toggle maintenance mode (suppresses alerts)
python3 inode-usage-monitor.py --version    # print version and exit
```
...
|---|---|---|
| `INODE_THRESHOLD` | `90.0` | Alert when any filesystem's inode usage exceeds this percent. |
...
- *Status-aware:* alerts fire once on transition into the alert state and a recovery email is sent once
  when the condition clears (state in `inode-usage-monitor.status`).
