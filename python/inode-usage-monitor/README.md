# inode-usage-monitor.py

inode-usage-monitor.py                                               # Reports inode usage for every real filesystem via 'df -iP'.          # Catches inode exhaustion that byte-based disk monitors miss.         # Emits JSON; alerts when any filesystem exceeds INODE_THRESHOLD.      #.
...
cd linux-sysadmin-toolkit/python/inode-usage-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/inode-usage-monitor/inode-usage-monitor.py \
     -o /opt/scripts/inode-usage-monitor.py
```

### Manual copy

```bash
cp inode-usage-monitor.py /opt/scripts/inode-usage-monitor.py
chmod +x /opt/scripts/inode-usage-monitor.py
```

### Verify

```bash
python3 /opt/scripts/inode-usage-monitor.py --version
python3 /opt/scripts/inode-usage-monitor.py --dry-run
```
...
|---|---|---|
| `INODE_THRESHOLD` | `90.0` | alert when any filesystem inode usage exceeds this percent |
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "inode-usage-monitor.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "inode-usage-monitor.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "inode-usage-monitor.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "inode-usage-monitor.email.state")
```
...
```
Usage: inode-usage-monitor.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 inode-usage-monitor.py
```
...
```bash
python3 inode-usage-monitor.py --dry-run
```
...
python3 inode-usage-monitor.py --maintenance
...
inode-usage-monitor/
├── inode-usage-monitor.py
├── inode-usage-monitor.status          # OK / ALERT state
├── inode-usage-monitor.email.state     # last-email timestamp
├── inode-usage-monitor.lock            # flock instance lock
├── inode-usage-monitor.maintenance     # present while maintenance mode is on
├── inode-usage-monitor-execution.log   # every run (START / END)
└── inode-usage-monitor-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/inode-usage-monitor.maintenance"
LOCK_FILE        = "/tmp/inode-usage-monitor.lock"
STATUS_FILE      = "/tmp/inode-usage-monitor.status"
STATE_FILE       = "/tmp/inode-usage-monitor.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: inode-usage-monitor
            image: python:3-slim
            command: ["python3", "/scripts/inode-usage-monitor.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/inode-usage-monitor.py >/dev/null 2>&1
```
...
[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/inode-usage-monitor.py
```
...
```bash
sudo systemctl enable --now inode-usage-monitor.timer
```
...
```bash
python3 /opt/scripts/inode-usage-monitor.py \
  | jq -r '"inode-usage-monitor_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/inode-usage-monitor.prom
```
