...
suid-audit.py                                                        # Walks configured directories for SUID/SGID and world-writable files. # Emits JSON with counts and paths.                                    # Alerts when world-writable files are present in scanned paths.       #.
...
cd linux-sysadmin-toolkit/python/suid-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/suid-audit/suid-audit.py \
     -o /opt/scripts/suid-audit.py
```

### Manual copy

```bash
cp suid-audit.py /opt/scripts/suid-audit.py
chmod +x /opt/scripts/suid-audit.py
```

### Verify

```bash
python3 /opt/scripts/suid-audit.py --version
python3 /opt/scripts/suid-audit.py --dry-run
```
...
```python
SCAN_DIRS = ["/usr/bin", "/usr/sbin", "/bin", "/sbin", "/usr/local/bin", "/usr/local/sbin"]
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "suid-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "suid-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "suid-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "suid-audit.email.state")
```
...
```
Usage: suid-audit.py [--dry-run] [--maintenance] [--version]
...
python3 suid-audit.py --maintenance
...
suid-audit/
├── suid-audit.py
├── suid-audit.status          # OK / ALERT state
├── suid-audit.email.state     # last-email timestamp
├── suid-audit.lock            # flock instance lock
├── suid-audit.maintenance     # present while maintenance mode is on
├── suid-audit-execution.log   # every run (START / END)
└── suid-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/suid-audit.maintenance"
LOCK_FILE        = "/tmp/suid-audit.lock"
STATUS_FILE      = "/tmp/suid-audit.status"
STATE_FILE       = "/tmp/suid-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: suid-audit
            image: python:3-slim
            command: ["python3", "/scripts/suid-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/suid-audit.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/suid-audit.service
[Unit]
Description=suid-audit check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/suid-audit.py
```

```ini
# /etc/systemd/system/suid-audit.timer
[Unit]
Description=Run suid-audit every 5 minutes
...
```bash
python3 /opt/scripts/suid-audit.py \
  | jq -r '"suid-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/suid-audit.prom
```
