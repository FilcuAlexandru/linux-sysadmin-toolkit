...
########################################################################
# suid-audit.py                                                        #
# Walks configured directories for SUID/SGID and world-writable files. #
# Emits JSON with counts and paths.                                    #
# Alerts when world-writable files are present in scanned paths.       #
...
VERSION     = "0.1"
SCRIPT_NAME = "suid-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "suid-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "suid-audit-execution.log")
...
def scan_suid() -> Tuple[List[str], List[str]]:
    """Scan suid."""
    suid, ww = [], []
...
def evaluate() -> Tuple[Dict, List[str]]:
    ...
    alerts = []
    if ww:
        alerts.append(f"{len(ww)} world-writable file(s) found in scanned paths")
    return {"suid_sgid_count": len(suid), "world_writable": ww, "suid_sgid": sorted(suid)[:300]}, alerts
