# systemd-timer-audit.py

systemd-timer-audit.py                                               # Inventories systemd timers via systemctl list-timers.                # Detects failed timer units.                                          # Emits JSON; alerts when any timer unit has failed.                   #.
...
- **`systemctl`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/systemd-timer-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/systemd-timer-audit/systemd-timer-audit.py \
     -o /opt/scripts/systemd-timer-audit.py
```

### Manual copy

```bash
cp systemd-timer-audit.py /opt/scripts/systemd-timer-audit.py
chmod +x /opt/scripts/systemd-timer-audit.py
```

### Verify

```bash
python3 /opt/scripts/systemd-timer-audit.py --version
python3 /opt/scripts/systemd-timer-audit.py --dry-run
```
...
### (no thresholds; alerts on any failed timer unit)
...
|---|---|---|
| `INCLUDE_INACTIVE` | `True` | include inactive timers in the inventory count |
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "systemd-timer-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "systemd-timer-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "systemd-timer-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "systemd-timer-audit.email.state")
```
...
```
Usage: systemd-timer-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 systemd-timer-audit.py
```
...
```bash
python3 systemd-timer-audit.py --dry-run
```
...
python3 systemd-timer-audit.py --maintenance
...
systemd-timer-audit/
├── systemd-timer-audit.py
├── systemd-timer-audit.status          # OK / ALERT state
├── systemd-timer-audit.email.state     # last-email timestamp
├── systemd-timer-audit.lock            # flock instance lock
├── systemd-timer-audit.maintenance     # present while maintenance mode is on
├── systemd-timer-audit-execution.log   # every run (START / END)
└── systemd-timer-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/systemd-timer-audit.maintenance"
LOCK_FILE        = "/tmp/systemd-timer-audit.lock"
STATUS_FILE      = "/tmp/systemd-timer-audit.status"
STATE_FILE       = "/tmp/systemd-timer-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: systemd-timer-audit
            image: python:3-slim
            command: ["python3", "/scripts/systemd-timer-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/systemd-timer-audit.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/systemd-timer-audit.service
[Unit]
Description=systemd-timer-audit check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/systemd-timer-audit.py
```

```ini
# /etc/systemd/system/systemd-timer-audit.timer
[Unit]
Description=Run systemd-timer-audit every 5 minutes
...
```bash
sudo systemctl enable --now systemd-timer-audit.timer
```
...
```bash
python3 /opt/scripts/systemd-timer-audit.py \
  | jq -r '"systemd-timer-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/systemd-timer-audit.prom
```
