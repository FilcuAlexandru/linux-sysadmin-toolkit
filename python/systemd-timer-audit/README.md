...
Audit systemd timers and detect failed timer units.

---

## Purpose

Counts timers from `systemctl list-timers` and lists failed timer units from `systemctl --failed --type=timer`, alerting if any have failed.

---

## Features

- **Inventories all systemd timers.**
- **Detects failed timer units.**
...
```bash
python3 systemd-timer-audit.py              # run; prints a JSON result on stdout
python3 systemd-timer-audit.py --dry-run    # print configuration and state, do nothing
python3 systemd-timer-audit.py --maintenance # toggle maintenance mode (suppresses alerts)
python3 systemd-timer-audit.py --version    # print version and exit
```
...
- *Status-aware:* alerts fire once on transition into the alert state and a recovery email is sent once
  when the condition clears (state in `systemd-timer-audit.status`).
