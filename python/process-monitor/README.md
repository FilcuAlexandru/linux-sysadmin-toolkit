...
process-monitor.py                                                 # Monitors configured processes for: not running, zombie state (Z),  # D-state hang (uninterruptible sleep), and RSS memory growth across # MEMORY_HISTORY samples. Reads /proc directly. Per-process status   # tracking with aggregated alert and individual recovery emails.     #.
...
cd linux-sysadmin-toolkit/python/process-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/process-monitor/process-monitor.py \
     -o /opt/scripts/process-monitor.py
```

### Manual copy

```bash
cp process-monitor.py /opt/scripts/process-monitor.py
chmod +x /opt/scripts/process-monitor.py
```

### Verify

```bash
python3 /opt/scripts/process-monitor.py --version
python3 /opt/scripts/process-monitor.py --dry-run
```
...
```
Usage: process-monitor.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 process-monitor.py --dry-run
```
...
python3 process-monitor.py --maintenance
...
process-monitor/
├── process-monitor.py
├── process-monitor.status          # OK / ALERT state
├── process-monitor.email.state     # last-email timestamp
├── process-monitor.lock            # flock instance lock
├── process-monitor.maintenance     # present while maintenance mode is on
├── process-monitor-execution.log   # every run (START / END)
└── process-monitor-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/process-monitor.maintenance"
LOCK_FILE        = "/tmp/process-monitor.lock"
    ...
    spec:
      template:
        spec:
          ...
          - name: process-monitor
            image: python:3-slim
            command: ["python3", "/scripts/process-monitor.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/process-monitor.py >/dev/null 2>&1
```
...
[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/process-monitor.py
```
...
```bash
python3 /opt/scripts/process-monitor.py \
  | jq -r '"process-monitor_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/process-monitor.prom
```
