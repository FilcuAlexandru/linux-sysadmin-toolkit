...
def get_alerted() -> List[str]:
    """Read the set of already-alerted items."""
    try:
        with open(STATUS_FILE) as f:
            d = json.load(f)
...
def is_in_alert(name: str) -> bool:
    """Return True if the item is already in the alert set."""
    return name in get_alerted()


def add_alert(name: str) -> None:
    """Add an item to the alert set."""
    alerted = get_alerted()
...
def remove_alert(name: str) -> None:
    """Remove an item from the alert set."""
...
def send_proc_recovery(name: str) -> None:
    """Send a recovery email for a process."""
    if not ALERT_EMAIL:
        return
...
def find_pids(name: str) -> List[int]:
    """Find the PIDs matching a process name."""
    pids = []
...
def read_proc_stat(pid: int) -> Optional[Dict]:
    """Read a process's /proc/<pid>/stat fields."""
    try:
