...
network-monitor.py                                                         # Monitors per-interface network throughput (Mbit/s), error/drop deltas,     # and TCP/UDP connection state counts. Reads /proc/net/dev for byte counters # and /proc/net/tcp + /proc/net/tcp6 for connection states. Two samples      # SAMPLE_INTERVAL seconds apart compute throughput. Alerts per-metric.       #.
...
cd linux-sysadmin-toolkit/python/network-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/network-monitor/network-monitor.py \
     -o /opt/scripts/network-monitor.py
```
...
ERROR_THRESHOLD = 100
SAMPLE_INTERVAL = 2
TCP_ESTABLISHED_THRESHOLD = 1000
TCP_TIME_WAIT_THRESHOLD = 500
```
...
```
Usage: network-monitor.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 network-monitor.py --dry-run
```
...
python3 network-monitor.py --maintenance
...
network-monitor/
├── network-monitor.py
├── network-monitor.status          # OK / ALERT state
├── network-monitor.email.state     # last-email timestamp
├── network-monitor.lock            # flock instance lock
├── network-monitor.maintenance     # present while maintenance mode is on
├── network-monitor-execution.log   # every run (START / END)
└── network-monitor-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/network-monitor.maintenance"
LOCK_FILE        = "/tmp/network-monitor.lock"
STATUS_FILE      = "/tmp/network-monitor.status"
STATE_FILE       = "/tmp/network-monitor.email.state"
```
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/network-monitor.py >/dev/null 2>&1
```
...
[Unit]
Description=network-monitor check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/network-monitor.py
```
...
```bash
python3 /opt/scripts/network-monitor.py \
  | jq -r '"network-monitor_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/network-monitor.prom
```
