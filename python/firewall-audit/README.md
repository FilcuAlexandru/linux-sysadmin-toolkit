...
firewall-audit.py                                                    # Detects an active firewalld, ufw, nftables, or iptables ruleset.     # Emits JSON identifying the active backend.                           # Alerts when no firewall is enforcing rules.                          #.
...
- **`firewall-cmd`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`iptables`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`nft`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`ufw`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/firewall-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/firewall-audit/firewall-audit.py \
     -o /opt/scripts/firewall-audit.py
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "firewall-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "firewall-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "firewall-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "firewall-audit.email.state")
```
...
```
Usage: firewall-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 firewall-audit.py --dry-run
```
...
python3 firewall-audit.py --maintenance
...
firewall-audit/
├── firewall-audit.py
├── firewall-audit.status          # OK / ALERT state
├── firewall-audit.email.state     # last-email timestamp
├── firewall-audit.lock            # flock instance lock
├── firewall-audit.maintenance     # present while maintenance mode is on
├── firewall-audit-execution.log   # every run (START / END)
└── firewall-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/firewall-audit.maintenance"
LOCK_FILE        = "/tmp/firewall-audit.lock"
STATUS_FILE      = "/tmp/firewall-audit.status"
STATE_FILE       = "/tmp/firewall-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: firewall-audit
            image: python:3-slim
            command: ["python3", "/scripts/firewall-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now firewall-audit.timer
```
...
```bash
python3 /opt/scripts/firewall-audit.py \
  | jq -r '"firewall-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/firewall-audit.prom
```
