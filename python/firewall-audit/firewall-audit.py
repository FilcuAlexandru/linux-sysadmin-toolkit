...
########################################################################
# firewall-audit.py                                                    #
# Detects an active firewalld, ufw, nftables, or iptables ruleset.     #
# Emits JSON identifying the active backend.                           #
# Alerts when no firewall is enforcing rules.                          #
...
VERSION     = "0.1"
SCRIPT_NAME = "firewall-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "firewall-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "firewall-audit-execution.log")
...
def collect_firewall() -> Dict:
    """Collect firewall."""
    if _have("firewall-cmd") and "running" in _run(["firewall-cmd", "--state"]).lower():
        return {"backend": "firewalld", "active": True}
    if _have("ufw") and "status: active" in _run(["ufw", "status"]).lower():
        return {"backend": "ufw", "active": True}
    if _have("nft"):
        rs = _run(["nft", "list", "ruleset"])
        if any(tok in rs for tok in ("chain", "rule")):
            return {"backend": "nftables", "active": True}
    if _have("iptables"):
        rs = _run(["iptables", "-S"])
        if any(l for l in rs.splitlines() if l and not l.startswith("-P")):
            return {"backend": "iptables", "active": True}
    return {"backend": None, "active": False}


def evaluate() -> Tuple[Dict, List[str]]:
    """Collect the data and return the (data, alerts) tuple."""
    d = collect_firewall()
    alerts = [] if d["active"] or not REQUIRE_FIREWALL else ["No active firewall detected"]
    return {"firewall": d}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"require_firewall": REQUIRE_FIREWALL,
