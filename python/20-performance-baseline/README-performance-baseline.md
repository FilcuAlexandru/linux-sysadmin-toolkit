...
Captures performance baseline and alerts on significant deviations. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./performance-baseline.py              # run and output JSON to stdout
./performance-baseline.py --dry-run    # show prerequisites and configuration without running
./performance-baseline.py --maintenance # toggle maintenance mode on/off
./performance-baseline.py --version    # print version and exit
./performance-baseline.py --help       # print help
```
...
| File | Content |
|---|---|
| `performance-baseline-execution.log` | Every run: start, result, end. |
| `performance-baseline-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/performance-baseline.py >> /var/log/performance-baseline.json 2>/dev/null
```

### Pipe to jq

```bash
python3 performance-baseline.py | jq '.data'
python3 performance-baseline.py | jq '.alerts[]'
python3 performance-baseline.py | jq '.status'
```

### Checkmk local check

```bash
cp performance-baseline.py /usr/lib/check_mk_agent/local/performance-baseline.py
```
...
|---|---|
| `BASELINE_FILE` | See inline comment in script. |
| `DEVIATION_PCT` | See inline comment in script. |
| `SAMPLE_INTERVAL` | See inline comment in script. |
| `TOP_PROCESSES` | See inline comment in script. |
