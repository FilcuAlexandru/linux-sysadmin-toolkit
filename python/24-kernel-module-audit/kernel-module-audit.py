...
# Inventories loaded modules from /proc/modules.                       #
# Reads the kernel taint flag from /proc/sys/kernel/tainted.           #
# Emits JSON; alerts when the kernel is tainted.                       #
...
VERSION     = "0.1"
SCRIPT_NAME = "kernel-module-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "kernel-module-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "kernel-module-audit-execution.log")
...
def collect_modules() -> Tuple[List[str], int]:
    mods = []
    for line in _read_file("/proc/modules").splitlines():
        p = line.split()
        if p:
            mods.append(p[0])
    tainted = _read_file("/proc/sys/kernel/tainted").strip()
    try:
        tval = int(tainted)
    except ValueError:
        tval = 0
...
def evaluate() -> Tuple[Dict, List[str]]:
    mods, tval = collect_modules()
    alerts = []
    if ALERT_ON_TAINT and tval != 0:
        alerts.append(f"Kernel taint flag is {tval} (non-zero)")
    return {"module_count": len(mods), "tainted": tval, "modules": sorted(mods)}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"alert_on_taint": ALERT_ON_TAINT,
