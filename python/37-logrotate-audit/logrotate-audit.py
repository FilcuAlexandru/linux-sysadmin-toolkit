...
########################################################################
# logrotate-audit.py                                                   #
# Parses /etc/logrotate.conf and /etc/logrotate.d entries.             #
# Checks that each configured log target matches existing files.       #
# Emits JSON; alerts on targets that match nothing.                    #
...
VERSION     = "0.1"
SCRIPT_NAME = "logrotate-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "logrotate-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "logrotate-audit-execution.log")
...
def collect_logrotate() -> Tuple[List[str], List[str]]:
    files = []
    if os.path.isfile("/etc/logrotate.conf"):
        files.append("/etc/logrotate.conf")
    d = "/etc/logrotate.d"
    if os.path.isdir(d):
        try:
...
def evaluate() -> Tuple[Dict, List[str]]:
    import glob
    files, paths = collect_logrotate()
    missing = [p for p in paths if not glob.glob(p)]
    alerts = []
    if missing:
        alerts.append(f"{len(missing)} logrotate target(s) match no files")
    data = {"config_files": len(files), "target_count": len(paths), "missing_targets": missing}
    if REPORT_MATCHED:
        data["targets"] = paths
    return data, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"checks": "logrotate target existence",
