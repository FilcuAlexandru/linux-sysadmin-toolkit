...
tls-cert-scanner.py                                                         # Walks configured directories for certificate files.                         # Reads expiry dates via openssl.                                             # Emits JSON; alerts on certs expiring within EXPIRY_DAYS or already expired. #.
...
- **`openssl`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/tls-cert-scanner

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/tls-cert-scanner/tls-cert-scanner.py \
     -o /opt/scripts/tls-cert-scanner.py
```
...
```
Usage: tls-cert-scanner.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 tls-cert-scanner.py --dry-run
```
...
python3 tls-cert-scanner.py --maintenance
...
tls-cert-scanner/
├── tls-cert-scanner.py
├── tls-cert-scanner.status          # OK / ALERT state
├── tls-cert-scanner.email.state     # last-email timestamp
├── tls-cert-scanner.lock            # flock instance lock
├── tls-cert-scanner.maintenance     # present while maintenance mode is on
├── tls-cert-scanner-execution.log   # every run (START / END)
└── tls-cert-scanner-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/tls-cert-scanner.maintenance"
LOCK_FILE        = "/tmp/tls-cert-scanner.lock"
STATUS_FILE      = "/tmp/tls-cert-scanner.status"
STATE_FILE       = "/tmp/tls-cert-scanner.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: tls-cert-scanner
            image: python:3-slim
            command: ["python3", "/scripts/tls-cert-scanner.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now tls-cert-scanner.timer
```
...
```bash
python3 /opt/scripts/tls-cert-scanner.py \
  | jq -r '"tls-cert-scanner_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/tls-cert-scanner.prom
```
