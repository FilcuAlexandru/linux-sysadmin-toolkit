...
# Samples per-process RSS from /proc and stores a history snapshot.    #
# Compares against the previous run to detect sustained growth.        #
# Emits JSON; alerts when a process grows beyond GROWTH_PCT.           #
...
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "memory-leak-detector-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "memory-leak-detector-execution.log")
...
def rss_by_proc() -> Dict[str, int]:
    res = {}
    for pid in os.listdir("/proc"):
        if not pid.isdigit():
            continue
        st = _read_file(f"/proc/{pid}/status")
        if not st:
            continue
        name, rss = "", 0
...
def evaluate() -> Tuple[Dict, List[str]]:
    ...
    for k, v in cur.items():
        if k in prev and prev[k] > 0 and v > RSS_FLOOR_KB:
            g = (v - prev[k]) / prev[k] * 100
            if g >= GROWTH_PCT:
                growth.append({"proc": k, "prev_kb": prev[k], "now_kb": v, "growth_pct": round(g, 1)})
                alerts.append(f"{k} RSS grew {round(g, 1)}% ({prev[k]}->{v} kB)")
    try:
        with open(hist_path, "w") as f:
            json.dump(cur, f)
    except OSError:
        pass
    return {"tracked": len(cur), "growth": growth, "growth_pct_threshold": GROWTH_PCT,
            "baseline_existed": bool(prev)}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"growth_pct": GROWTH_PCT, "rss_floor_kb": RSS_FLOOR_KB,
