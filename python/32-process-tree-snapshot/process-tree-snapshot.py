...
# Builds a process inventory from /proc (pid, ppid, comm).             #
# Emits JSON including parent/child relationships.                     #
# Alerts when the total process count exceeds MAX_PROCESSES.           #
...
VERSION     = "0.1"
SCRIPT_NAME = "process-tree-snapshot"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "process-tree-snapshot-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "process-tree-snapshot-execution.log")
...
def collect_procs() -> Dict[int, Dict]:
    procs = {}
    for pid in os.listdir("/proc"):
        ...
        try:
            ...
            ppid = int(rest[1])
            procs[int(pid)] = {"pid": int(pid), "ppid": ppid, "comm": comm}
        except Exception:
            continue
    return procs
...
def evaluate() -> Tuple[Dict, List[str]]:
    ...
    if len(procs) > MAX_PROCESSES:
        alerts.append(f"Process count {len(procs)} exceeds {MAX_PROCESSES}")
    children: Dict[int, int] = {}
    for p in procs.values():
        children[p["ppid"]] = children.get(p["ppid"], 0) + 1
    top_parents = sorted(children.items(), key=lambda kv: kv[1], reverse=True)[:5]
    return {"process_count": len(procs), "max": MAX_PROCESSES,
            "top_parents": [{"ppid": k, "children": v} for k, v in top_parents]}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"max_processes": MAX_PROCESSES,
