...
Collects CPU, memory, disks, network interfaces, and PCI devices. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./hardware-inventory.py              # run and output JSON to stdout
./hardware-inventory.py --dry-run    # show prerequisites and configuration without running
./hardware-inventory.py --maintenance # toggle maintenance mode on/off
./hardware-inventory.py --version    # print version and exit
./hardware-inventory.py --help       # print help
```
...
| File | Content |
|---|---|
| `hardware-inventory-execution.log` | Every run: start, result, end. |
| `hardware-inventory-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/hardware-inventory.py >> /var/log/hardware-inventory.json 2>/dev/null
```

### Pipe to jq

```bash
python3 hardware-inventory.py | jq '.data'
python3 hardware-inventory.py | jq '.alerts[]'
python3 hardware-inventory.py | jq '.status'
```

### Checkmk local check

```bash
cp hardware-inventory.py /usr/lib/check_mk_agent/local/hardware-inventory.py
```
