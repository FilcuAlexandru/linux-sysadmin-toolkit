...
hardware-inventory.py                                                       # Collects detailed hardware information: CPU model and core count, total     # and available memory, block devices, network interfaces with MAC addresses, # and PCI devices. Reads /proc directly where possible; falls back to         # lsblk and lspci for supplementary data. No thresholds — inventory only.     #.
...
- **`lsblk`** — used by this script when available; missing tools degrade the affected check gracefully.
- **`lspci`** — used by this script when available; missing tools degrade the affected check gracefully.
...
cd linux-sysadmin-toolkit/python/hardware-inventory

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/hardware-inventory/hardware-inventory.py \
     -o /opt/scripts/hardware-inventory.py
```

### Manual copy

```bash
cp hardware-inventory.py /opt/scripts/hardware-inventory.py
chmod +x /opt/scripts/hardware-inventory.py
```

### Verify

```bash
python3 /opt/scripts/hardware-inventory.py --version
python3 /opt/scripts/hardware-inventory.py --dry-run
```
...
```
Usage: hardware-inventory.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 hardware-inventory.py --dry-run
```
...
python3 hardware-inventory.py --maintenance
...
hardware-inventory/
├── hardware-inventory.py
├── hardware-inventory.status          # OK / ALERT state
├── hardware-inventory.email.state     # last-email timestamp
├── hardware-inventory.lock            # flock instance lock
├── hardware-inventory.maintenance     # present while maintenance mode is on
├── hardware-inventory-execution.log   # every run (START / END)
└── hardware-inventory-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/hardware-inventory.maintenance"
LOCK_FILE        = "/tmp/hardware-inventory.lock"
STATUS_FILE      = "/tmp/hardware-inventory.status"
STATE_FILE       = "/tmp/hardware-inventory.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: hardware-inventory
            image: python:3-slim
            command: ["python3", "/scripts/hardware-inventory.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/hardware-inventory.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/hardware-inventory.service
[Unit]
Description=hardware-inventory check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/hardware-inventory.py
```

```ini
# /etc/systemd/system/hardware-inventory.timer
[Unit]
Description=Run hardware-inventory every 5 minutes
...
```bash
sudo systemctl enable --now hardware-inventory.timer
```
...
```bash
python3 /opt/scripts/hardware-inventory.py \
  | jq -r '"hardware-inventory_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/hardware-inventory.prom
```
