...
file-integrity-monitor.py                                            # Hashes a watch-list of critical files with SHA-256.                  # Stores a baseline and compares against it on each run.               # Emits JSON; alerts on changed, new, or removed files.                #.
...
cd linux-sysadmin-toolkit/python/file-integrity-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/file-integrity-monitor/file-integrity-monitor.py \
     -o /opt/scripts/file-integrity-monitor.py
```

### Manual copy

```bash
cp file-integrity-monitor.py /opt/scripts/file-integrity-monitor.py
chmod +x /opt/scripts/file-integrity-monitor.py
```

### Verify

```bash
python3 /opt/scripts/file-integrity-monitor.py --version
python3 /opt/scripts/file-integrity-monitor.py --dry-run
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "file-integrity-monitor.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "file-integrity-monitor.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "file-integrity-monitor.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "file-integrity-monitor.email.state")
```
...
```
Usage: file-integrity-monitor.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 file-integrity-monitor.py
```
...
```bash
python3 file-integrity-monitor.py --dry-run
```
...
python3 file-integrity-monitor.py --maintenance
...
├── file-integrity-monitor.py
├── file-integrity-monitor.status          # OK / ALERT state
├── file-integrity-monitor.email.state     # last-email timestamp
├── file-integrity-monitor.lock            # flock instance lock
├── file-integrity-monitor.maintenance     # present while maintenance mode is on
├── file-integrity-monitor-execution.log   # every run (START / END)
└── file-integrity-monitor-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/file-integrity-monitor.maintenance"
LOCK_FILE        = "/tmp/file-integrity-monitor.lock"
STATUS_FILE      = "/tmp/file-integrity-monitor.status"
STATE_FILE       = "/tmp/file-integrity-monitor.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: file-integrity-monitor
            image: python:3-slim
            command: ["python3", "/scripts/file-integrity-monitor.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/file-integrity-monitor.py >/dev/null 2>&1
```

### systemd timer

```ini
# /etc/systemd/system/file-integrity-monitor.service
[Unit]
Description=file-integrity-monitor check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/file-integrity-monitor.py
```

```ini
# /etc/systemd/system/file-integrity-monitor.timer
[Unit]
Description=Run file-integrity-monitor every 5 minutes
...
```bash
sudo systemctl enable --now file-integrity-monitor.timer
```
...
```bash
python3 /opt/scripts/file-integrity-monitor.py \
  | jq -r '"file-integrity-monitor_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/file-integrity-monitor.prom
```
