...
system-monitor.py                                                      # Collects CPU, memory, swap, disk, and network metrics in one run.      # Reads /proc directly for CPU/memory/swap/network; uses df -P for disk. # Two /proc/net/dev samples NETWORK_SAMPLE_INTERVAL seconds apart give   # throughput in Mbit/s. Alerts per-metric with individual thresholds.    #.
...
cd linux-sysadmin-toolkit/python/system-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/system-monitor/system-monitor.py \
     -o /opt/scripts/system-monitor.py
```

### Manual copy

```bash
cp system-monitor.py /opt/scripts/system-monitor.py
chmod +x /opt/scripts/system-monitor.py
```

### Verify

```bash
python3 /opt/scripts/system-monitor.py --version
python3 /opt/scripts/system-monitor.py --dry-run
```
...
```
Usage: system-monitor.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 system-monitor.py --dry-run
```
...
python3 system-monitor.py --maintenance
...
system-monitor/
├── system-monitor.py
├── system-monitor.status          # OK / ALERT state
├── system-monitor.email.state     # last-email timestamp
├── system-monitor.lock            # flock instance lock
├── system-monitor.maintenance     # present while maintenance mode is on
├── system-monitor-execution.log   # every run (START / END)
└── system-monitor-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/system-monitor.maintenance"
LOCK_FILE        = "/tmp/system-monitor.lock"
STATUS_FILE      = "/tmp/system-monitor.status"
STATE_FILE       = "/tmp/system-monitor.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: system-monitor
            image: python:3-slim
            command: ["python3", "/scripts/system-monitor.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/system-monitor.py >/dev/null 2>&1
```
...
[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/system-monitor.py
```
...
```bash
python3 /opt/scripts/system-monitor.py \
  | jq -r '"system-monitor_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/system-monitor.prom
```
