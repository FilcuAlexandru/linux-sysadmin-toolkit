...
Collects CPU, memory, swap, disk, and network metrics. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./system-monitor.py              # run and output JSON to stdout
./system-monitor.py --dry-run    # show prerequisites and configuration without running
./system-monitor.py --maintenance # toggle maintenance mode on/off
./system-monitor.py --version    # print version and exit
./system-monitor.py --help       # print help
```
...
| File | Content |
|---|---|
| `system-monitor-execution.log` | Every run: start, result, end. |
| `system-monitor-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/system-monitor.py >> /var/log/system-monitor.json 2>/dev/null
```

### Pipe to jq

```bash
python3 system-monitor.py | jq '.data'
python3 system-monitor.py | jq '.alerts[]'
python3 system-monitor.py | jq '.status'
```

### Checkmk local check

```bash
cp system-monitor.py /usr/lib/check_mk_agent/local/system-monitor.py
```
...
|---|---|
| `CPU_THRESHOLD` | See inline comment in script. |
| `MEMORY_THRESHOLD` | See inline comment in script. |
| `SWAP_THRESHOLD` | See inline comment in script. |
| `DISK_THRESHOLD` | See inline comment in script. |
| `LOAD_MULTIPLIER` | See inline comment in script. |
| `NETWORK_INTERFACE` | See inline comment in script. |
| `NETWORK_THRESHOLD_MBPS` | See inline comment in script. |
| `NETWORK_SAMPLE_INTERVAL` | See inline comment in script. |
