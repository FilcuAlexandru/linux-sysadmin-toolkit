...
Inventory SUID/SGID binaries and detect world-writable files.

---

## Purpose

Walks each directory in `SCAN_DIRS`, classifying files by mode bits. SUID/SGID files are inventoried; world-writable files raise an alert.

---

## Features

- **Inventories SUID and SGID binaries.**
- **Flags world-writable files (a privilege-escalation risk).**
- **Configurable scan roots; pure stdlib `os.walk`.**
...
- read access to the scanned directories
...
```bash
python3 suid-audit.py              # run; prints a JSON result on stdout
python3 suid-audit.py --dry-run    # print configuration and state, do nothing
python3 suid-audit.py --maintenance # toggle maintenance mode (suppresses alerts)
python3 suid-audit.py --version    # print version and exit
```
...
|---|---|---|
| `SCAN_DIRS` | `see script` | Directories to scan for SUID/SGID and world-writable files. |
...
- *Status-aware:* alerts fire once on transition into the alert state and a recovery email is sent once
  when the condition clears (state in `suid-audit.status`).
