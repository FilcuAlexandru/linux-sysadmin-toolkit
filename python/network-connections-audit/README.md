...
network-connections-audit.py                                         # Parses /proc/net/tcp and tcp6 and counts sockets by state.           # Emits JSON with a state histogram.                                   # Alerts when ESTABLISHED or TIME_WAIT exceed configured thresholds.   #.
...
cd linux-sysadmin-toolkit/python/network-connections-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/network-connections-audit/network-connections-audit.py \
     -o /opt/scripts/network-connections-audit.py
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "network-connections-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "network-connections-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "network-connections-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "network-connections-audit.email.state")
```
...
```
Usage: network-connections-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 network-connections-audit.py
```
...
```bash
python3 network-connections-audit.py --dry-run
```
...
python3 network-connections-audit.py --maintenance
...
├── network-connections-audit.py
├── network-connections-audit.status          # OK / ALERT state
├── network-connections-audit.email.state     # last-email timestamp
├── network-connections-audit.lock            # flock instance lock
├── network-connections-audit.maintenance     # present while maintenance mode is on
├── network-connections-audit-execution.log   # every run (START / END)
└── network-connections-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/network-connections-audit.maintenance"
LOCK_FILE        = "/tmp/network-connections-audit.lock"
STATUS_FILE      = "/tmp/network-connections-audit.status"
STATE_FILE       = "/tmp/network-connections-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: network-connections-audit
            image: python:3-slim
            command: ["python3", "/scripts/network-connections-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now network-connections-audit.timer
```
...
```bash
python3 /opt/scripts/network-connections-audit.py \
  | jq -r '"network-connections-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/network-connections-audit.prom
```
