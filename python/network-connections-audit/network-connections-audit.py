...
# Parses /proc/net/tcp and tcp6 and counts sockets by state.           #
# Emits JSON with a state histogram.                                   #
# Alerts when ESTABLISHED or TIME_WAIT exceed configured thresholds.   #
...
VERSION     = "0.1"
SCRIPT_NAME = "network-connections-audit"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "network-connections-audit-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "network-connections-audit-execution.log")
...
def collect_conn() -> Dict[str, int]:
    """Collect conn."""
    counts: Dict[str, int] = {}
    for f in ("/proc/net/tcp", "/proc/net/tcp6"):
        for line in _read_file(f).splitlines()[1:]:
            p = line.split()
            if len(p) < 4:
                continue
            st = TCP_STATES.get(p[3].upper(), p[3])
...
def evaluate() -> Tuple[Dict, List[str]]:
    ...
    c = collect_conn()
    alerts = []
    if c.get("ESTABLISHED", 0) > EST_THRESHOLD:
        alerts.append(f"ESTABLISHED connections {c['ESTABLISHED']} exceed {EST_THRESHOLD}")
    if c.get("TIME_WAIT", 0) > TIMEWAIT_THRESHOLD:
        alerts.append(f"TIME_WAIT sockets {c['TIME_WAIT']} exceed {TIMEWAIT_THRESHOLD}")
    return {"states": c}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"established_threshold": EST_THRESHOLD, "timewait_threshold": TIMEWAIT_THRESHOLD,
