...
Detect processes whose memory grows between runs.

---

## Purpose

Samples RSS for every process, compares each against the previous run stored in a history file, and alerts when growth exceeds `GROWTH_PCT` for processes above the noise floor. The first run only establishes a baseline.

---

## Features

- **Tracks per-process RSS across runs via a JSON history file.**
- **Flags sustained growth that indicates a leak.**
- **Noise floor filter via `RSS_FLOOR_KB`.**
...
```bash
python3 memory-leak-detector.py              # run; prints a JSON result on stdout
python3 memory-leak-detector.py --dry-run    # print configuration and state, do nothing
python3 memory-leak-detector.py --maintenance # toggle maintenance mode (suppresses alerts)
python3 memory-leak-detector.py --version    # print version and exit
```
...
|---|---|---|
| `GROWTH_PCT` | `50.0` | Alert when a process RSS grows by at least this percent between runs. |
| `RSS_FLOOR_KB` | `50000` | Ignore processes smaller than this size. |
...
- *Status-aware:* alerts fire once on transition into the alert state and a recovery email is sent once
  when the condition clears (state in `memory-leak-detector.status`).
