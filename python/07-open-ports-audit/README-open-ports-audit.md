...
Lists listening ports, compares against expected list, diffs against previous run. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./open-ports-audit.py              # run and output JSON to stdout
./open-ports-audit.py --dry-run    # show prerequisites and configuration without running
./open-ports-audit.py --maintenance # toggle maintenance mode on/off
./open-ports-audit.py --version    # print version and exit
./open-ports-audit.py --help       # print help
```
...
| File | Content |
|---|---|
| `open-ports-audit-execution.log` | Every run: start, result, end. |
| `open-ports-audit-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/open-ports-audit.py >> /var/log/open-ports-audit.json 2>/dev/null
```

### Pipe to jq

```bash
python3 open-ports-audit.py | jq '.data'
python3 open-ports-audit.py | jq '.alerts[]'
python3 open-ports-audit.py | jq '.status'
```

### Checkmk local check

```bash
cp open-ports-audit.py /usr/lib/check_mk_agent/local/open-ports-audit.py
```
