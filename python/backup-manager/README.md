...
Incremental rsync backup with SHA-256 integrity and rotative retention. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./backup-manager.py              # run and output JSON to stdout
./backup-manager.py --dry-run    # show prerequisites and configuration without running
./backup-manager.py --maintenance # toggle maintenance mode on/off
./backup-manager.py --version    # print version and exit
./backup-manager.py --help       # print help
```
...
| File | Content |
|---|---|
| `backup-manager-execution.log` | Every run: start, result, end. |
| `backup-manager-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/backup-manager.py >> /var/log/backup-manager.json 2>/dev/null
```

### Pipe to jq

```bash
python3 backup-manager.py | jq '.data'
python3 backup-manager.py | jq '.alerts[]'
python3 backup-manager.py | jq '.status'
```

### Checkmk local check

```bash
cp backup-manager.py /usr/lib/check_mk_agent/local/backup-manager.py
```
...
|---|---|
| `SOURCE_DIR` | See inline comment in script. |
| `BACKUP_BASE` | See inline comment in script. |
| `DAILY_KEEP` | See inline comment in script. |
| `WEEKLY_KEEP` | See inline comment in script. |
| `MONTHLY_KEEP` | See inline comment in script. |
| `VERIFY_CHECKSUMS` | See inline comment in script. |
| `CHECKSUM_SAMPLE` | See inline comment in script. |
