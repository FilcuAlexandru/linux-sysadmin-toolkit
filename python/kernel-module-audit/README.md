...
kernel-module-audit.py                                               # Inventories loaded modules from /proc/modules.                       # Reads the kernel taint flag from /proc/sys/kernel/tainted.           # Emits JSON; alerts when the kernel is tainted.                       #.
...
cd linux-sysadmin-toolkit/python/kernel-module-audit

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/kernel-module-audit/kernel-module-audit.py \
     -o /opt/scripts/kernel-module-audit.py
```
...
```python
MAINTENANCE_FILE = os.path.join(SCRIPT_DIR, "kernel-module-audit.maintenance")
```
...
```python
LOCK_FILE = os.path.join(SCRIPT_DIR, "kernel-module-audit.lock")
```
...
```python
STATUS_FILE = os.path.join(SCRIPT_DIR, "kernel-module-audit.status")
```
...
```python
STATE_FILE = os.path.join(SCRIPT_DIR, "kernel-module-audit.email.state")
```
...
```
Usage: kernel-module-audit.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 kernel-module-audit.py
```
...
```bash
python3 kernel-module-audit.py --dry-run
```
...
python3 kernel-module-audit.py --maintenance
...
kernel-module-audit/
├── kernel-module-audit.py
├── kernel-module-audit.status          # OK / ALERT state
├── kernel-module-audit.email.state     # last-email timestamp
├── kernel-module-audit.lock            # flock instance lock
├── kernel-module-audit.maintenance     # present while maintenance mode is on
├── kernel-module-audit-execution.log   # every run (START / END)
└── kernel-module-audit-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/kernel-module-audit.maintenance"
LOCK_FILE        = "/tmp/kernel-module-audit.lock"
STATUS_FILE      = "/tmp/kernel-module-audit.status"
STATE_FILE       = "/tmp/kernel-module-audit.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: kernel-module-audit
            image: python:3-slim
            command: ["python3", "/scripts/kernel-module-audit.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now kernel-module-audit.timer
```
...
```bash
python3 /opt/scripts/kernel-module-audit.py \
  | jq -r '"kernel-module-audit_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/kernel-module-audit.prom
```
