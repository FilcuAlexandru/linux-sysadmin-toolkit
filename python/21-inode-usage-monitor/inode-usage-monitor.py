...
# Reports inode usage for every real filesystem via 'df -iP'.          #
# Catches inode exhaustion that byte-based disk monitors miss.         #
# Emits JSON; alerts when any filesystem exceeds INODE_THRESHOLD.      #
...
VERSION     = "0.1"
SCRIPT_NAME = "inode-usage-monitor"
_ERROR_LOG     = os.path.join(SCRIPT_DIR, "inode-usage-monitor-error.log")
_EXECUTION_LOG = os.path.join(SCRIPT_DIR, "inode-usage-monitor-execution.log")
...
def collect_inodes() -> List[Dict]:
    ...
    for line in out.splitlines()[1:]:
        ...
        if len(p) < 6:
            continue
        fs, itot, iused, _ifree, ipct, mount = p[0], p[1], p[2], p[3], p[4], p[5]
        if fs.startswith(("tmpfs", "devtmpfs", "udev", "sysfs", "proc", "cgroup", "overlay")):
            continue
        try:
            pct = float(ipct.rstrip("%"))
        except ValueError:
            continue
        res.append({"filesystem": fs, "mount": mount, "ipercent": pct,
                    "iused": _int(iused), "itotal": _int(itot)})
    return res


def evaluate() -> Tuple[Dict, List[str]]:
    disks = collect_inodes()
    alerts = [f"Inode usage {d['mount']} at {d['ipercent']}% (threshold {INODE_THRESHOLD}%)"
              for d in disks if d["ipercent"] > INODE_THRESHOLD]
    return {"filesystems": disks, "threshold": INODE_THRESHOLD}, alerts
...
def main() -> None:
    ...
    if args.dry_run:
        print(json.dumps({
            ...
            "dry_run": {"inode_threshold": INODE_THRESHOLD,
