...
Captures full system state and diffs against previous run. Outputs structured JSON to stdout on every run. Part of the Python SysAdmin Toolkit.
...
```bash
./system-snapshot.py              # run and output JSON to stdout
./system-snapshot.py --dry-run    # show prerequisites and configuration without running
./system-snapshot.py --maintenance # toggle maintenance mode on/off
./system-snapshot.py --version    # print version and exit
./system-snapshot.py --help       # print help
```
...
| File | Content |
|---|---|
| `system-snapshot-execution.log` | Every run: start, result, end. |
| `system-snapshot-error.log` | Alerts, recovery emails, and errors only. |
...
```cron
*/5 * * * * /opt/scripts/system-snapshot.py >> /var/log/system-snapshot.json 2>/dev/null
```

### Pipe to jq

```bash
python3 system-snapshot.py | jq '.data'
python3 system-snapshot.py | jq '.alerts[]'
python3 system-snapshot.py | jq '.status'
```

### Checkmk local check

```bash
cp system-snapshot.py /usr/lib/check_mk_agent/local/system-snapshot.py
```
...
|---|---|
| `SNAPSHOT_FILE` | See inline comment in script. |
| `PREVIOUS_FILE` | See inline comment in script. |
| `CAPTURE_PACKAGES` | See inline comment in script. |
| `CAPTURE_SERVICES` | See inline comment in script. |
| `CAPTURE_PORTS` | See inline comment in script. |
| `CAPTURE_USERS` | See inline comment in script. |
| `CAPTURE_CRONTABS` | See inline comment in script. |
| `CAPTURE_SYSCTL` | See inline comment in script. |
