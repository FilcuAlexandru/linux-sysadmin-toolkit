...
system-snapshot.py                                                     # Captures a complete system state snapshot: installed packages, running # services, open ports, local users, crontabs, and kernel parameters.    # Saves the snapshot as JSON and diffs it against the previous run to    # report exactly what changed on the system between two points in time.  #.
...
cd linux-sysadmin-toolkit/python/system-snapshot

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/system-snapshot/system-snapshot.py \
     -o /opt/scripts/system-snapshot.py
```

### Manual copy

```bash
cp system-snapshot.py /opt/scripts/system-snapshot.py
chmod +x /opt/scripts/system-snapshot.py
```

### Verify

```bash
python3 /opt/scripts/system-snapshot.py --version
python3 /opt/scripts/system-snapshot.py --dry-run
```
...
```
Usage: system-snapshot.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 system-snapshot.py --dry-run
```
...
python3 system-snapshot.py --maintenance
...
system-snapshot/
├── system-snapshot.py
├── system-snapshot.status          # OK / ALERT state
├── system-snapshot.email.state     # last-email timestamp
├── system-snapshot.lock            # flock instance lock
├── system-snapshot.maintenance     # present while maintenance mode is on
├── system-snapshot-execution.log   # every run (START / END)
└── system-snapshot-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/system-snapshot.maintenance"
LOCK_FILE        = "/tmp/system-snapshot.lock"
STATUS_FILE      = "/tmp/system-snapshot.status"
STATE_FILE       = "/tmp/system-snapshot.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: system-snapshot
            image: python:3-slim
            command: ["python3", "/scripts/system-snapshot.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /usr/bin/python3 /opt/scripts/system-snapshot.py >/dev/null 2>&1
```
...
[Unit]
Description=system-snapshot check

[Service]
Type=oneshot
ExecStart=/usr/bin/python3 /opt/scripts/system-snapshot.py
```

```ini
# /etc/systemd/system/system-snapshot.timer
[Unit]
Description=Run system-snapshot every 5 minutes
...
```bash
sudo systemctl enable --now system-snapshot.timer
```
...
```bash
python3 /opt/scripts/system-snapshot.py \
  | jq -r '"system-snapshot_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/system-snapshot.prom
```
