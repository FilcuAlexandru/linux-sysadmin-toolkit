...
listening-services-map.py                                            # Inventories listening TCP/UDP sockets via ss, with a /proc fallback. # Flags services bound to all interfaces outside an allowlist.         # Emits JSON; alerts on unexpected wildcard exposure.                  #.
...
cd linux-sysadmin-toolkit/python/listening-services-map

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/python/listening-services-map/listening-services-map.py \
     -o /opt/scripts/listening-services-map.py
```
...
```
Usage: listening-services-map.py [--dry-run] [--maintenance] [--version]
...
```bash
python3 listening-services-map.py
```
...
```bash
python3 listening-services-map.py --dry-run
```
...
python3 listening-services-map.py --maintenance
...
```
listening-services-map/
├── listening-services-map.py
├── listening-services-map.status          # OK / ALERT state
├── listening-services-map.email.state     # last-email timestamp
├── listening-services-map.lock            # flock instance lock
├── listening-services-map.maintenance     # present while maintenance mode is on
├── listening-services-map-execution.log   # every run (START / END)
└── listening-services-map-error.log       # alerts, emails, and recoveries only
```
...
```python
MAINTENANCE_FILE = "/tmp/listening-services-map.maintenance"
LOCK_FILE        = "/tmp/listening-services-map.lock"
STATUS_FILE      = "/tmp/listening-services-map.status"
STATE_FILE       = "/tmp/listening-services-map.email.state"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: listening-services-map
            image: python:3-slim
            command: ["python3", "/scripts/listening-services-map.py"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now listening-services-map.timer
```
...
```bash
python3 /opt/scripts/listening-services-map.py \
  | jq -r '"listening-services-map_status{host=\"" + .host + "\"} " + (if .status=="OK" then "0" else "1" end)' \
  > /var/lib/node_exporter/listening-services-map.prom
```
