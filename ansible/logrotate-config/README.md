...
ansible-playbook -i inventory.ini logrotate-config/logrotate-config.yml --check
```
...
ansible-playbook -i inventory.ini logrotate-config/logrotate-config.yml -e 'logrotate_app_name=nginx'
```

```yaml
# Persistently, in group_vars/all.yml (applies to every host):
logrotate_app_name: nginx
```
...
ansible-playbook -i inventory.ini logrotate-config/logrotate-config.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini logrotate-config/logrotate-config.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini logrotate-config/logrotate-config.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini logrotate-config/logrotate-config.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/logrotate-config/logrotate-config.yml
    - ansible-playbook -i ansible/inventory.ini ansible/logrotate-config/logrotate-config.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/logrotate-config/logrotate-config.yml
```
...
`ansible/logrotate-config/logrotate-config.yml`; expose the variables above as survey fields.
