...
ansible-playbook -i inventory.ini automatic-updates/automatic-updates.yml --check
```
...
ansible-playbook -i inventory.ini automatic-updates/automatic-updates.yml -e 'auto_updates_enabled=true'
```
...
ansible-playbook -i inventory.ini automatic-updates/automatic-updates.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini automatic-updates/automatic-updates.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini automatic-updates/automatic-updates.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini automatic-updates/automatic-updates.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/automatic-updates/automatic-updates.yml
    - ansible-playbook -i ansible/inventory.ini ansible/automatic-updates/automatic-updates.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/automatic-updates/automatic-updates.yml
```
...
`ansible/automatic-updates/automatic-updates.yml`; expose the variables above as survey fields.
