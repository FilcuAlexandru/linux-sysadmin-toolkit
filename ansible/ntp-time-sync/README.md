...
ansible-playbook -i inventory.ini ntp-time-sync/ntp-time-sync.yml --check
```
...
ansible-playbook -i inventory.ini ntp-time-sync/ntp-time-sync.yml -e 'ntp_servers=['time.example.com']'
```
...
ansible-playbook -i inventory.ini ntp-time-sync/ntp-time-sync.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini ntp-time-sync/ntp-time-sync.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini ntp-time-sync/ntp-time-sync.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini ntp-time-sync/ntp-time-sync.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/ntp-time-sync/ntp-time-sync.yml
    - ansible-playbook -i ansible/inventory.ini ansible/ntp-time-sync/ntp-time-sync.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/ntp-time-sync/ntp-time-sync.yml
```
...
`ansible/ntp-time-sync/ntp-time-sync.yml`; expose the variables above as survey fields.
