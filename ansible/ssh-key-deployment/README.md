...
ansible-playbook -i inventory.ini ssh-key-deployment/ssh-key-deployment.yml --check
```
...
ansible-playbook -i inventory.ini ssh-key-deployment/ssh-key-deployment.yml -e 'ssh_key_target_user=admin'
```

```yaml
# Persistently, in group_vars/all.yml (applies to every host):
ssh_key_target_user: admin
```
...
ansible-playbook -i inventory.ini ssh-key-deployment/ssh-key-deployment.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini ssh-key-deployment/ssh-key-deployment.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini ssh-key-deployment/ssh-key-deployment.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini ssh-key-deployment/ssh-key-deployment.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/ssh-key-deployment/ssh-key-deployment.yml
    - ansible-playbook -i ansible/inventory.ini ansible/ssh-key-deployment/ssh-key-deployment.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/ssh-key-deployment/ssh-key-deployment.yml
```
...
`ansible/ssh-key-deployment/ssh-key-deployment.yml`; expose the variables above as survey fields.
