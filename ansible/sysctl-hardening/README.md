...
ansible-playbook -i inventory.ini sysctl-hardening/sysctl-hardening.yml --check
```
...
ansible-playbook -i inventory.ini sysctl-hardening/sysctl-hardening.yml -e 'sysctl_hardening='{net.ipv4.ip_forward: 1}''
```

```yaml
# Persistently, in group_vars/all.yml (applies to every host):
sysctl_hardening: '{net.ipv4.ip_forward: 1}'
```
...
ansible-playbook -i inventory.ini sysctl-hardening/sysctl-hardening.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini sysctl-hardening/sysctl-hardening.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini sysctl-hardening/sysctl-hardening.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini sysctl-hardening/sysctl-hardening.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/sysctl-hardening/sysctl-hardening.yml
    - ansible-playbook -i ansible/inventory.ini ansible/sysctl-hardening/sysctl-hardening.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/sysctl-hardening/sysctl-hardening.yml
```
...
`ansible/sysctl-hardening/sysctl-hardening.yml`; expose the variables above as survey fields.
