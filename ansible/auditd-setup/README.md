...
ansible-playbook -i inventory.ini auditd-setup/auditd-setup.yml --check
```
...
ansible-playbook -i inventory.ini auditd-setup/auditd-setup.yml -e 'auditd_watch_files=['/etc/passwd']'
```

```yaml
# Persistently, in group_vars/all.yml (applies to every host):
auditd_watch_files: ['/etc/passwd']
```
...
ansible-playbook -i inventory.ini auditd-setup/auditd-setup.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini auditd-setup/auditd-setup.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini auditd-setup/auditd-setup.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini auditd-setup/auditd-setup.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/auditd-setup/auditd-setup.yml
    - ansible-playbook -i ansible/inventory.ini ansible/auditd-setup/auditd-setup.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/auditd-setup/auditd-setup.yml
```
...
`ansible/auditd-setup/auditd-setup.yml`; expose the variables above as survey fields.
