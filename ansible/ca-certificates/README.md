...
ansible-playbook -i inventory.ini ca-certificates/ca-certificates.yml --check
```
...
ansible-playbook -i inventory.ini ca-certificates/ca-certificates.yml -e 'target_hosts=all'
```
...
ansible-playbook -i inventory.ini ca-certificates/ca-certificates.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini ca-certificates/ca-certificates.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini ca-certificates/ca-certificates.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini ca-certificates/ca-certificates.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/ca-certificates/ca-certificates.yml
    - ansible-playbook -i ansible/inventory.ini ansible/ca-certificates/ca-certificates.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/ca-certificates/ca-certificates.yml
```
...
`ansible/ca-certificates/ca-certificates.yml`; expose the variables above as survey fields.
