...
ansible-playbook -i inventory.ini timezone-locale/timezone-locale.yml --check
```
...
ansible-playbook -i inventory.ini timezone-locale/timezone-locale.yml -e 'system_timezone=Europe/Bucharest'
```
...
ansible-playbook -i inventory.ini timezone-locale/timezone-locale.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini timezone-locale/timezone-locale.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini timezone-locale/timezone-locale.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini timezone-locale/timezone-locale.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/timezone-locale/timezone-locale.yml
    - ansible-playbook -i ansible/inventory.ini ansible/timezone-locale/timezone-locale.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/timezone-locale/timezone-locale.yml
```
...
`ansible/timezone-locale/timezone-locale.yml`; expose the variables above as survey fields.
