...
ansible-playbook -i inventory.ini login-banner/login-banner.yml --check
```
...
ansible-playbook -i inventory.ini login-banner/login-banner.yml -e 'target_hosts=db'
```
...
ansible-playbook -i inventory.ini login-banner/login-banner.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini login-banner/login-banner.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini login-banner/login-banner.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini login-banner/login-banner.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/login-banner/login-banner.yml
    - ansible-playbook -i ansible/inventory.ini ansible/login-banner/login-banner.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/login-banner/login-banner.yml
```
...
`ansible/login-banner/login-banner.yml`; expose the variables above as survey fields.
