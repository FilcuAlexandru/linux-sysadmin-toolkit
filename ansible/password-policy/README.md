...
ansible-playbook -i inventory.ini password-policy/password-policy.yml --check
```
...
ansible-playbook -i inventory.ini password-policy/password-policy.yml -e 'pwquality_minlen=16'
```
...
ansible-playbook -i inventory.ini password-policy/password-policy.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini password-policy/password-policy.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini password-policy/password-policy.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini password-policy/password-policy.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/password-policy/password-policy.yml
    - ansible-playbook -i ansible/inventory.ini ansible/password-policy/password-policy.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/password-policy/password-policy.yml
```
...
`ansible/password-policy/password-policy.yml`; expose the variables above as survey fields.
