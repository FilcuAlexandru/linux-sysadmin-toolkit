...
ansible-playbook -i inventory.ini journald-config/journald-config.yml --check
```
...
ansible-playbook -i inventory.ini journald-config/journald-config.yml -e 'journald_max_use=1G'
```
...
ansible-playbook -i inventory.ini journald-config/journald-config.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini journald-config/journald-config.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini journald-config/journald-config.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini journald-config/journald-config.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/journald-config/journald-config.yml
    - ansible-playbook -i ansible/inventory.ini ansible/journald-config/journald-config.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/journald-config/journald-config.yml
```
...
`ansible/journald-config/journald-config.yml`; expose the variables above as survey fields.
