...
ansible-playbook -i inventory.ini ssh-hardening/ssh-hardening.yml --check
```
...
ansible-playbook -i inventory.ini ssh-hardening/ssh-hardening.yml -e 'ssh_permit_root_login=no'
```
...
ansible-playbook -i inventory.ini ssh-hardening/ssh-hardening.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini ssh-hardening/ssh-hardening.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini ssh-hardening/ssh-hardening.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini ssh-hardening/ssh-hardening.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/ssh-hardening/ssh-hardening.yml
    - ansible-playbook -i ansible/inventory.ini ansible/ssh-hardening/ssh-hardening.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/ssh-hardening/ssh-hardening.yml
```
...
`ansible/ssh-hardening/ssh-hardening.yml`; expose the variables above as survey fields.
