...
ansible-playbook -i inventory.ini firewall-baseline/firewall-baseline.yml --check
```
...
ansible-playbook -i inventory.ini firewall-baseline/firewall-baseline.yml -e 'firewall_allowed_tcp_ports=[22,80,443]'
```
...
ansible-playbook -i inventory.ini firewall-baseline/firewall-baseline.yml --check --diff

# 2) Apply to every host in the inventory.
ansible-playbook -i inventory.ini firewall-baseline/firewall-baseline.yml

# 3) Limit to a host group.
ansible-playbook -i inventory.ini firewall-baseline/firewall-baseline.yml --limit web

# 4) Prompt for the sudo password when the login user needs it.
ansible-playbook -i inventory.ini firewall-baseline/firewall-baseline.yml --ask-become-pass
```
...
ansible-verify:
  ...
  script:
    ...
    - yamllint ansible/
    - ansible-lint ansible/firewall-baseline/firewall-baseline.yml
    - ansible-playbook -i ansible/inventory.ini ansible/firewall-baseline/firewall-baseline.yml --syntax-check
```
...
ansible-pull -U https://github.com/YOUR_USER/linux-sysadmin-toolkit.git \
    ansible/firewall-baseline/firewall-baseline.yml
```
...
`ansible/firewall-baseline/firewall-baseline.yml`; expose the variables above as survey fields.
