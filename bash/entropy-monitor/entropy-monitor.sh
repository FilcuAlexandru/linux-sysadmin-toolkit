...
###################################################################################
# entropy-monitor.sh                                                              #
# Bash script that reads the kernel's available entropy pool.                     #
# Low entropy can stall cryptographic operations (TLS, SSH, GPG, key generation). #
# Reads /proc/sys/kernel/random/entropy_avail directly; no dependencies.          #
...
#   - Alert fires when available entropy drops below this many bits. #
#   - The kernel pool maximum is typically 4096 bits.               #
######################################################################
...
#   - Toggle with: ./entropy-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Available entropy recovered on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    if [[ -r /proc/sys/kernel/random/entropy_avail ]]; then
        printf '  %-28s %b\n' "/proc/sys/kernel/random/entropy_avail" "$ok"
    else
        printf '  %-28s %b (cannot read entropy)\n' "/proc/sys/kernel/random/entropy_avail" "$miss"
    fi
    for bin in awk mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            ...
            case "$bin" in
                awk) printf '  %-28s %b (not required but used by helpers)\n' "$bin" "$miss" ;;
    ...
    printf '  %-28s %s\n' "Minimum entropy:" "${MIN_ENTROPY} bits"
...
alert_body() {
    [[ -r /proc/sys/kernel/random/poolsize ]] && echo "Pool size: $(< /proc/sys/kernel/random/poolsize) bits"
    command -v rngd >/dev/null 2>&1 && echo "rngd is installed (consider enabling it)" || echo "rngd not installed; consider haveged or rng-tools"
}
...
evaluate() {
    local f=/proc/sys/kernel/random/entropy_avail
    [[ -r "$f" ]] || { OVER=0; SUMMARY="entropy_avail unreadable"; DETAIL="no entropy stats"; return; }
    local e; e=$(< "$f"); e=${e//[!0-9]/}; [[ -n "$e" ]] || e=0
    if (( e < MIN_ENTROPY )); then
        OVER=1; SUMMARY="Low entropy: ${e} bits (min ${MIN_ENTROPY})"; DETAIL="${e} bits available"
    else
        OVER=0; SUMMARY="Entropy OK: ${e} bits"; DETAIL="${e} bits available"
    fi
}
...
Report available kernel entropy and alert when it falls below ${MIN_ENTROPY} bits.
