...
# Handles systems with no swap gracefully (reports and exits cleanly).                         #
...
############################################################
# Swap threshold.                                          #
#   - Alert fires when swap usage exceeds this percentage. #
#   - Value is a percentage (0-100).                       #
#   - Has no effect on systems with no swap configured.    #
############################################################
...
#   - Toggle with: ./swap-monitor.sh --maintenance        #
...
####################################################################
# Status tracking.                                                 #
#   - Prevents repeated alerts while swap stays above threshold.   #
#   - Sends a recovery email when swap drops back below threshold. #
...
####################################################################
# Recovery.                                                        #
#   - send_recovery_mail : sends a single recovery email when swap #
...
send_recovery_mail() {
    ...
    local body="Swap usage is back below ${THRESHOLD}% on ${HOST_ID}"
    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email to: ${ALERT_EMAIL}"; return 0
    fi
    # shellcheck disable=SC2086
    echo "$body" | mail -s "Swap recovery on ${HOST_ID}" $ALERT_EMAIL
...
check_prerequisites() {
    ...
    for bin in free mail flock find; do
...
alert() {
    local detail="$*"
    local body="Swap usage above ${THRESHOLD}% on ${HOST_ID}: ${detail}"
    ...
    if should_send_email; then
        # shellcheck disable=SC2086
        echo "$body" | mail -s "Swap alert on ${HOST_ID}" $ALERT_EMAIL
        mark_email_sent
...
#####################################################################
# Swap usage reading.                                               #
#   - read_swap_usage : reads SwapTotal and SwapFree from           #
#                       /proc/meminfo and computes used/total in    #
#                       GB, percentage, and an over-threshold flag. #
#                       Also returns a hasswap flag (0 = no swap).  #
#                       Falls back to free -m if /proc/meminfo is   #
#                       unavailable. All float math is done in awk. #
#####################################################################

# Read swap usage and return: used_gb total_gb pct over(0/1) hasswap(0/1).
# hasswap=0 means no swap is configured; caller should exit cleanly.
# Source priority:
#   1) /proc/meminfo — SwapTotal / SwapFree; accurate, no binary needed.
#   2) free -m       — fallback; parses the Swap line (NR==3).
read_swap_usage() {
    if [[ -r /proc/meminfo ]]; then
        awk -v thr="$THRESHOLD" '
            /^SwapTotal:/ { total_kb = $2 }
            /^SwapFree:/  { free_kb  = $2 }
            END {
                used_kb = total_kb - free_kb
                has     = (total_kb > 0) ? 1 : 0
                pct     = has ? (used_kb * 100 / total_kb) : 0
                printf "%.2f %.2f %.2f %d %d\n",
...
Sends a recovery email when swap drops back below the threshold.
Email alerts are rate-limited to one every EMAIL_INTERVAL seconds (default 3600).
Reports cleanly when no swap is configured.
...
###########################################################################
# Main.                                                                   #
#   - Reads swap usage from /proc/meminfo (preferred) or free -m.         #
#   - Exits cleanly with a notice when no swap is configured (hasswap=0). #
#   - Prints usage in GB with percentage (red when above threshold).      #
#   - Status-aware alerting: alerts once when swap goes above threshold,  #
...
read -r used total pct over hasswap < <(read_swap_usage) \
    || die "could not read swap usage (/proc/meminfo unavailable and free not found)"
[[ -n "$pct" ]] || die "could not parse swap usage"

if (( ! hasswap )); then
    echo "Swap Usage: none configured"
    log_to "$EXECUTION_LOG" "RESULT no swap configured"
...
# Status-aware alerting: alert once when swap goes above threshold,
