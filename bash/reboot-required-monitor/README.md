# reboot-required-monitor.sh

Detect when the system requires a reboot after updates.

---

## Features

- Cross-distro: Debian/Ubuntu markers, RHEL `needs-restarting -r`, SLES `zypper needs-rebooting`.
- Lists the packages that requested the reboot when available.
...
- `/run/reboot-required` (Debian/Ubuntu) — optional
- `needs-restarting` (RHEL `dnf-utils`/`yum-utils`) — optional
- `zypper` (SLES) — optional
...
cd linux-sysadmin-toolkit/bash/reboot-required-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/reboot-required-monitor/reboot-required-monitor.sh \
     -o /opt/scripts/reboot-required-monitor.sh
```

### Manual copy

```bash
cp reboot-required-monitor.sh /opt/scripts/reboot-required-monitor.sh
chmod +x /opt/scripts/reboot-required-monitor.sh
```

### Verify

```bash
/opt/scripts/reboot-required-monitor.sh --version
/opt/scripts/reboot-required-monitor.sh --dry-run
```
...
|---|---|---|
| `CHECK_KERNEL` | `1` | Reserved flag to also compare running vs installed kernel where supported. |
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/reboot-required-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/reboot-required-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/reboot-required-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/reboot-required-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/reboot-required-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/reboot-required-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/reboot-required-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/reboot-required-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/reboot-required-monitor.status"
```
...
```
Usage: reboot-required-monitor.sh [--dry-run] [--maintenance] [--version] [--help]
...
```bash
./reboot-required-monitor.sh --dry-run
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/reboot-required-monitor-error.log
  Execution log:               /opt/scripts/logs/reboot-required-monitor-execution.log
...
./reboot-required-monitor.sh --maintenance
...
The script checks each distribution's native reboot indicator and alerts if any reports a pending reboot. On Debian/Ubuntu it also reads `/run/reboot-required.pkgs` to list the packages responsible.
...
scripts/
├── reboot-required-monitor.sh
├── reboot-required-monitor.status
├── reboot-required-monitor.email.state
├── reboot-required-monitor.lock
├── reboot-required-monitor.maintenance
└── logs/
    ├── reboot-required-monitor-error.log
    ├── reboot-required-monitor-error.log.2026-06-01_120000
    ├── reboot-required-monitor-execution.log
    └── reboot-required-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/reboot-required-monitor.status"
STATE_FILE="/tmp/reboot-required-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: reboot-required-monitor
            image: alpine/bash
            command: ["/scripts/reboot-required-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /opt/scripts/reboot-required-monitor.sh >/dev/null 2>&1
```
...
```ini
# /etc/systemd/system/reboot-required-monitor.service
[Unit]
Description=reboot-required-monitor check

[Service]
Type=oneshot
ExecStart=/opt/scripts/reboot-required-monitor.sh
```

```ini
# /etc/systemd/system/reboot-required-monitor.timer
[Unit]
Description=Run reboot-required-monitor every 5 minutes
...
```bash
sudo systemctl enable --now reboot-required-monitor.timer
```

### Checkmk (local check)

```bash
cp reboot-required-monitor.sh /usr/lib/check_mk_agent/local/reboot-required-monitor.sh
```
...
```bash
echo "reboot_required_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/reboot-required-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/reboot-required-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/reboot-required-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/reboot-required-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/reboot-required-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/reboot-required-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/reboot-required-monitor.status` | Tracks OK/ALERT state. |
