...
# Bash script that detects a pending reboot across Debian/Ubuntu, RHEL, and SLES. #
# Checks /run/reboot-required, needs-restarting -r, and zypper needs-rebooting.   #
# Useful after unattended security updates to schedule a maintenance window.      #
...
######################################################################
# This monitor has no numeric threshold: it alerts whenever the      #
# system reports that a reboot is required by any supported method.  #
######################################################################

CHECK_KERNEL=1   # also compare running vs newest installed kernel where possible
...
#   - Toggle with: ./reboot-required-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="System no longer requires a reboot on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    if [[ -r /run/reboot-required ]]; then
        printf '  %-28s %b\n' "/run/reboot-required" "$ok"
    else
        printf '  %-28s %b (Debian reboot marker absent (normal if no reboot needed))\n' "/run/reboot-required" "$miss"
    fi
    for bin in needs-restarting zypper mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                needs-restarting) printf '  %-28s %b (RHEL reboot check unavailable)\n' "$bin" "$miss" ;;
                zypper) printf '  %-28s %b (SLES reboot check unavailable)\n' "$bin" "$miss" ;;
    ...
    printf '  %-28s %s\n' "Kernel comparison:" "${CHECK_KERNEL}"
...
alert_body() {
    [[ -f /run/reboot-required.pkgs ]] && { echo "Packages requesting reboot:"; sed 's/^/  /' /run/reboot-required.pkgs 2>/dev/null; }
...
evaluate() {
    local need=0 reasons=""
    if [[ -f /run/reboot-required || -f /var/run/reboot-required ]]; then
        need=1; reasons+="reboot-required marker present; "
    fi
    if command -v needs-restarting >/dev/null 2>&1; then
        if ! needs-restarting -r >/dev/null 2>&1; then need=1; reasons+="needs-restarting -r: reboot needed; "; fi
    fi
    if command -v zypper >/dev/null 2>&1; then
        local rc=0; zypper -q needs-rebooting >/dev/null 2>&1 || rc=$?
        (( rc == 102 )) && { need=1; reasons+="zypper needs-rebooting: reboot needed; "; }
    fi
    OVER=$need
    if (( need )); then
        SUMMARY="Reboot REQUIRED"; DETAIL="${reasons%%; }"
    else
        SUMMARY="No reboot required"; DETAIL="system up to date"
    fi
}
...
Detect whether the system requires a reboot (Debian/RHEL/SLES) and alert if so.
