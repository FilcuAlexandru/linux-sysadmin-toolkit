...
# Bash script that checks the operational state of network interfaces via sysfs. #
# Alerts when a monitored interface is down (link lost).                         #
# Reads /sys/class/net directly; no ip/ifconfig dependency.                      #
...
#   - INTERFACES : space-separated names to monitor (e.g. "eth0").  #
#                  Leave empty to auto-detect physical interfaces.  #
######################################################################
...
#   - Toggle with: ./interface-link-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="All monitored interfaces back up on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    if [[ -r /sys/class/net ]]; then
        ...
        printf '  %-28s %b (interface state unavailable)\n' "/sys/class/net" "$miss"
    fi
    for bin in cat mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                cat) printf '  %-28s %b (required to read sysfs)\n' "$bin" "$miss" ;;
    ...
    printf '  %-28s %s\n' "Interfaces:" "${INTERFACES:-auto-detect}"
...
alert_body() {
    ...
    for i in /sys/class/net/*; do
        ...
        [[ -r "/sys/class/net/$i/operstate" ]] && printf "  %-12s %s\n" "$i" "$(< /sys/class/net/$i/operstate)"
    done
}
...
evaluate() {
    [[ -d /sys/class/net ]] || { OVER=0; SUMMARY="/sys/class/net unavailable"; DETAIL="no sysfs net"; return; }
    local ifaces="$INTERFACES" i down="" checked=0 state
    if [[ -z "$ifaces" ]]; then
        for i in /sys/class/net/*; do
            i=$(basename "$i")
            case "$i" in lo|veth*|docker*|br-*|virbr*|vnet*|tap*|tun*) continue ;; esac
            ifaces+="$i "
        done
    fi
    for i in $ifaces; do
        [[ -r "/sys/class/net/$i/operstate" ]] || continue
        checked=$((checked+1))
        state=$(< "/sys/class/net/$i/operstate")
        case "$state" in up|unknown) : ;; *) down+="${i}(${state}) " ;; esac
    done
    if (( checked == 0 )); then OVER=0; SUMMARY="No interfaces to monitor"; DETAIL="0 interfaces"; return; fi
    if [[ -n "$down" ]]; then
        OVER=1; SUMMARY="Interface(s) DOWN: ${down}"; DETAIL="down: ${down}"
    else
        OVER=0; SUMMARY="All ${checked} interface(s) up"; DETAIL="${checked} up"
    fi
}
...
Check network interface link state via sysfs and alert when one is down.
