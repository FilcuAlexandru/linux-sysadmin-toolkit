...
cd linux-sysadmin-toolkit/bash/interface-link-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/interface-link-monitor/interface-link-monitor.sh \
     -o /opt/scripts/interface-link-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/interface-link-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/interface-link-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/interface-link-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/interface-link-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/interface-link-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/interface-link-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/interface-link-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/interface-link-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/interface-link-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/interface-link-monitor-error.log
  Execution log:               /opt/scripts/logs/interface-link-monitor-execution.log
...
./interface-link-monitor.sh --maintenance
...
scripts/
├── interface-link-monitor.sh
├── interface-link-monitor.status
├── interface-link-monitor.email.state
├── interface-link-monitor.lock
├── interface-link-monitor.maintenance
└── logs/
    ├── interface-link-monitor-error.log
    ├── interface-link-monitor-error.log.2026-06-01_120000
    ├── interface-link-monitor-execution.log
    └── interface-link-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/interface-link-monitor.status"
STATE_FILE="/tmp/interface-link-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: interface-link-monitor
            image: alpine/bash
            command: ["/scripts/interface-link-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now interface-link-monitor.timer
```

### Checkmk (local check)

```bash
cp interface-link-monitor.sh /usr/lib/check_mk_agent/local/interface-link-monitor.sh
```
...
```bash
echo "interface_link_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/interface-link-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/interface-link-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/interface-link-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/interface-link-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/interface-link-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/interface-link-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/interface-link-monitor.status` | Tracks OK/ALERT state. |
