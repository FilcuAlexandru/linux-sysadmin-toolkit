...
# Idempotent: services already stopped are left untouched.                   #
...
#   - Each name is passed to systemctl stop <name>.          #
...
#   - Toggle with: ./service-stop.sh --maintenance        #
...
####################################################################
# Status tracking.                                                 #
#   - Prevents repeated alerts while stop failures keep occurring. #
#   - Sends a recovery email when all services stop successfully   #
...
# Colors only when writing to a terminal: green = stopped, red = running / failed to stop.
...
#                          services stop successfully after a       #
...
send_recovery_mail() {
    ...
    local body="All services stopped successfully on ${HOST_ID}"
    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email to: ${ALERT_EMAIL}"; return 0
    fi
    # shellcheck disable=SC2086
    echo "$body" | mail -s "Service stop recovery on ${HOST_ID}" $ALERT_EMAIL
...
alert() {
    local detail="$*"
    local body="Service stop failure on ${HOST_ID}: ${detail}"
    ...
    if should_send_email; then
        # shellcheck disable=SC2086
        echo "$body" | mail -s "Service stop alert on ${HOST_ID}" $ALERT_EMAIL
        mark_email_sent
...
Stop any SERVICES entry that is running. Idempotent: already-stopped
services are left untouched. Alerts on stop failure (not on stop action).
...
################################################################
# Main.                                                        #
#   - Checks each service: if already stopped, prints status.  #
#   - If running: attempts systemctl stop.                     #
#   - On stop failure: alerts and updates status to ALERT.     #
...
for svc in "${SERVICES[@]}"; do
    if pgrep -x "$svc" >/dev/null 2>&1; then
        if (( DRY_RUN )); then
            printf '%b%-24s would stop%b\n' "$RED" "$svc" "$RST"
            log_to "$EXECUTION_LOG" "DRY-RUN would stop ${svc}"
        else
            printf '%b%-24s stopping...%b\n' "$RED" "$svc" "$RST"
            if systemctl stop "$svc" 2>/dev/null; then
                printf '%b%-24s stopped%b\n' "$GREEN" "$svc" "$RST"
                log_to "$EXECUTION_LOG" "STOPPED ${svc}"
                stopped+=("$svc")
            else
                printf '%b%-24s FAILED to stop%b\n' "$RED" "$svc" "$RST" >&2
                log_to "$EXECUTION_LOG" "FAILED ${svc}"
                failed_to_stop+=("$svc")
            fi
        fi
    else
        printf '%b%-24s already stopped%b\n' "$GREEN" "$svc" "$RST"
        log_to "$EXECUTION_LOG" "STOPPED already ${svc}"
    fi
done

if (( ${#failed_to_stop[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "RESULT ${#failed_to_stop[@]} service(s) failed to stop: ${failed_to_stop[*]}"
    if [[ "$current_status" != "ALERT" ]]; then
        set_status ALERT
        alert "failed to stop: ${failed_to_stop[*]}"
    else
        log_to "$EXECUTION_LOG" "Already in ALERT state"
    fi
elif (( ${#stopped[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "RESULT stopped: ${stopped[*]}"
    if [[ "$current_status" == "ALERT" ]]; then
        set_status OK
        send_recovery_mail
    else
    ...
    log_to "$EXECUTION_LOG" "RESULT all services already stopped"
    if [[ "$current_status" == "ALERT" ]]; then
        set_status OK
        send_recovery_mail
    fi
