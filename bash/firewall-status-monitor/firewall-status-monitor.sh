...
# Bash script that confirms an active firewall (firewalld, ufw, nftables, or iptables). #
# Alerts when no firewall is enforcing rules on the host.                               #
# Reports which backend is active for inventory and compliance.                         #
...
######################################################################
# This monitor has no numeric threshold: it alerts when none of the  #
# supported firewall backends is active (firewalld, ufw, nftables,   #
# or iptables with non-default rules).                              #
######################################################################
...
#   - Toggle with: ./firewall-status-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Firewall is active again on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in firewall-cmd ufw nft iptables mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                firewall-cmd) printf '  %-28s %b (firewalld check unavailable)\n' "$bin" "$miss" ;;
                ufw) printf '  %-28s %b (ufw check unavailable)\n' "$bin" "$miss" ;;
                nft) printf '  %-28s %b (nftables check unavailable)\n' "$bin" "$miss" ;;
                iptables) printf '  %-28s %b (iptables check unavailable)\n' "$bin" "$miss" ;;
    ...
    printf '  %-28s %s\n' "Require firewall:" "${REQUIRE_FIREWALL}"
...
alert_body() {
    echo "Firewall backend probe:"
    command -v firewall-cmd >/dev/null 2>&1 && echo "  firewalld: $(firewall-cmd --state 2>/dev/null || echo not-running)"
    command -v ufw >/dev/null 2>&1 && echo "  ufw: $(ufw status 2>/dev/null | head -1)"
    command -v nft >/dev/null 2>&1 && echo "  nftables rules: $(nft list ruleset 2>/dev/null | grep -cE 'rule' )"
}
...
evaluate() {
    ...
    if command -v firewall-cmd >/dev/null 2>&1 && firewall-cmd --state 2>/dev/null | grep -qi running; then
        ...
        active=ufw
    elif command -v nft >/dev/null 2>&1 && nft list ruleset 2>/dev/null | grep -qE '^[[:space:]]*(chain|rule)'; then
        active=nftables
    elif command -v iptables >/dev/null 2>&1 && iptables -S 2>/dev/null | grep -qvE '^-P'; then
        active=iptables
    fi
    if [[ -n "$active" ]]; then
        OVER=0; SUMMARY="Firewall active (${active})"; DETAIL="${active} enforcing rules"
    else
        OVER=1; SUMMARY="NO active firewall detected"; DETAIL="no firewalld/ufw/nftables/iptables rules"
    fi
}
...
Verify a host firewall is active and alert when none is enforcing rules.
