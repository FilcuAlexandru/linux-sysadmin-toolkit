...
cd linux-sysadmin-toolkit/bash/firewall-status-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/firewall-status-monitor/firewall-status-monitor.sh \
     -o /opt/scripts/firewall-status-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/firewall-status-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/firewall-status-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/firewall-status-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/firewall-status-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/firewall-status-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/firewall-status-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/firewall-status-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/firewall-status-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/firewall-status-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/firewall-status-monitor-error.log
  Execution log:               /opt/scripts/logs/firewall-status-monitor-execution.log
...
./firewall-status-monitor.sh --maintenance
...
scripts/
├── firewall-status-monitor.sh
├── firewall-status-monitor.status
├── firewall-status-monitor.email.state
├── firewall-status-monitor.lock
├── firewall-status-monitor.maintenance
└── logs/
    ├── firewall-status-monitor-error.log
    ├── firewall-status-monitor-error.log.2026-06-01_120000
    ├── firewall-status-monitor-execution.log
    └── firewall-status-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/firewall-status-monitor.status"
STATE_FILE="/tmp/firewall-status-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: firewall-status-monitor
            image: alpine/bash
            command: ["/scripts/firewall-status-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now firewall-status-monitor.timer
```

### Checkmk (local check)

```bash
cp firewall-status-monitor.sh /usr/lib/check_mk_agent/local/firewall-status-monitor.sh
```
...
```bash
echo "firewall_status_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/firewall-status-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/firewall-status-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/firewall-status-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/firewall-status-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/firewall-status-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/firewall-status-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/firewall-status-monitor.status` | Tracks OK/ALERT state. |
