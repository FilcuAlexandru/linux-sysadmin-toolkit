...
# Bash script that monitors the nf_conntrack table against its maximum.              #
# A full conntrack table silently drops new connections under load.                  #
# Reads /proc/sys/net counters directly; cleanly reports when conntrack is inactive. #
...
######################################################################
# Conntrack usage threshold.                                        #
#   - Alert fires when the connection-tracking table is more than    #
#     this percent full.                                            #
######################################################################
...
#   - Toggle with: ./conntrack-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Conntrack table usage back below threshold on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    printf '  %-28s %s\n' "Threshold:" "${THRESHOLD}% of conntrack max"
...
alert_body() {
    echo "Conntrack tuning hints:"
    echo "  Increase with: sysctl -w net.netfilter.nf_conntrack_max=<n>"
    echo "  Inspect with:  conntrack -L (if conntrack-tools installed)"
}
...
evaluate() {
    ...
    [[ -n "$cf" && -n "$mf" ]] || { OVER=0; SUMMARY="conntrack not active"; DETAIL="nf_conntrack not loaded"; return; }
    local cur max; cur=$(< "$cf"); max=$(< "$mf")
    [[ "$cur" =~ ^[0-9]+$ && "$max" =~ ^[0-9]+$ && "$max" -gt 0 ]] || { OVER=0; SUMMARY="conntrack stats unparseable"; DETAIL="bad counters"; return; }
    local pct over
    read -r pct over < <(awk -v c="$cur" -v m="$max" -v t="$THRESHOLD" 'BEGIN{p=c/m*100; printf "%.1f %d\n", p, (p>t?1:0)}')
    OVER=$over
    if (( over )); then
        SUMMARY="Conntrack ${cur}/${max} (${pct}%) above ${THRESHOLD}%"; DETAIL="${cur}/${max} = ${pct}%"
    else
        SUMMARY="Conntrack ${cur}/${max} (${pct}%)"; DETAIL="${cur}/${max} = ${pct}%"
    fi
}
...
Monitor nf_conntrack table usage and alert above ${THRESHOLD}% full.
