# conntrack-monitor.sh

Monitor netfilter connection-tracking table usage.

---

## Features

- Reads conntrack counters directly from `/proc/sys/net`.
- Handles both the modern `netfilter/` and legacy paths.
- Cleanly reports when conntrack is not loaded.
...
- `nf_conntrack` kernel module loaded — required for a measurement
- `awk` — required
...
cd linux-sysadmin-toolkit/bash/conntrack-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/conntrack-monitor/conntrack-monitor.sh \
     -o /opt/scripts/conntrack-monitor.sh
```

### Manual copy

```bash
cp conntrack-monitor.sh /opt/scripts/conntrack-monitor.sh
chmod +x /opt/scripts/conntrack-monitor.sh
```

### Verify

```bash
/opt/scripts/conntrack-monitor.sh --version
/opt/scripts/conntrack-monitor.sh --dry-run
```
...
|---|---|---|
| `THRESHOLD` | `80` | Alert when the conntrack table exceeds this percent of its maximum. |
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/conntrack-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/conntrack-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/conntrack-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/conntrack-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/conntrack-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/conntrack-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/conntrack-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/conntrack-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/conntrack-monitor.status"
```
...
```
Usage: conntrack-monitor.sh [--dry-run] [--maintenance] [--version] [--help]
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/conntrack-monitor-error.log
  Execution log:               /opt/scripts/logs/conntrack-monitor-execution.log
...
./conntrack-monitor.sh --maintenance
...
The script reads `nf_conntrack_count` and `nf_conntrack_max`, computes the fill percentage in `awk`, and alerts when it exceeds `THRESHOLD`. If the module is not loaded it reports conntrack as inactive rather than erroring.
...
scripts/
├── conntrack-monitor.sh
├── conntrack-monitor.status
├── conntrack-monitor.email.state
├── conntrack-monitor.lock
├── conntrack-monitor.maintenance
└── logs/
    ├── conntrack-monitor-error.log
    ├── conntrack-monitor-error.log.2026-06-01_120000
    ├── conntrack-monitor-execution.log
    └── conntrack-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/conntrack-monitor.status"
STATE_FILE="/tmp/conntrack-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: conntrack-monitor
            image: alpine/bash
            command: ["/scripts/conntrack-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /opt/scripts/conntrack-monitor.sh >/dev/null 2>&1
```
...
```ini
# /etc/systemd/system/conntrack-monitor.service
[Unit]
Description=conntrack-monitor check

[Service]
Type=oneshot
ExecStart=/opt/scripts/conntrack-monitor.sh
```

```ini
# /etc/systemd/system/conntrack-monitor.timer
[Unit]
Description=Run conntrack-monitor every 5 minutes
...
```bash
sudo systemctl enable --now conntrack-monitor.timer
```

### Checkmk (local check)

```bash
cp conntrack-monitor.sh /usr/lib/check_mk_agent/local/conntrack-monitor.sh
```
...
```bash
echo "conntrack_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/conntrack-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/conntrack-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/conntrack-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/conntrack-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/conntrack-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/conntrack-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/conntrack-monitor.status` | Tracks OK/ALERT state. |
