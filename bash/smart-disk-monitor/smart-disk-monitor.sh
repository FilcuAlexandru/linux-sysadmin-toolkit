...
# Bash script that runs smartctl health checks against local block devices.  #
# Auto-detects devices from /sys/block and alerts on any non-PASSED result.  #
# Provides early warning of imminent drive failure.                          #
...
#   - DEVICES : space-separated device paths (e.g. "/dev/sda").     #
#               Leave empty to auto-detect from /sys/block.         #
######################################################################
...
#   - Toggle with: ./smart-disk-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="SMART health back to passing on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in smartctl mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                smartctl) printf '  %-28s %b (S.M.A.R.T. checks cannot run)\n' "$bin" "$miss" ;;
...
alert_body() {
    ...
    local devs="$DEVICES" d
    [[ -z "$devs" ]] && for d in /sys/block/sd* /sys/block/nvme* /sys/block/vd*; do [[ -e "$d" ]] && devs+="/dev/$(basename "$d") "; done
    for d in $devs; do [[ -e "$d" ]] || continue; echo "== $d =="; smartctl -H "$d" 2>/dev/null | grep -iE 'result|health' | sed 's/^/  /'; done
}
...
evaluate() {
    command -v smartctl >/dev/null 2>&1 || { OVER=0; SUMMARY="smartctl not installed"; DETAIL="smartmontools missing"; return; }
    local devs="$DEVICES" d failed="" checked=0
    if [[ -z "$devs" ]]; then
        local b
        for b in /sys/block/*; do
            b=$(basename "$b")
            case "$b" in sd*|nvme*|hd*|vd*) devs+="/dev/$b " ;; esac
        done
    fi
    for d in $devs; do
        [[ -e "$d" ]] || continue
        checked=$((checked+1))
        local out; out=$(smartctl -H "$d" 2>/dev/null || true)
        if grep -qiE 'PASSED|OK' <<<"$out"; then :;
        elif grep -qiE 'FAILED|FAILING' <<<"$out"; then failed+="$d "; fi
    done
    if (( checked == 0 )); then OVER=0; SUMMARY="No SMART-capable devices found"; DETAIL="0 devices"; return; fi
    if [[ -n "$failed" ]]; then
        OVER=1; SUMMARY="SMART health FAILED: ${failed}"; DETAIL="failing: ${failed}"
    else
        OVER=0; SUMMARY="SMART health OK (${checked} device(s))"; DETAIL="${checked} devices passed"
    fi
}
...
Run smartctl health checks on local disks and alert on any non-PASSED result.
