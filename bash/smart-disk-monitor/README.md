...
cd linux-sysadmin-toolkit/bash/smart-disk-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/smart-disk-monitor/smart-disk-monitor.sh \
     -o /opt/scripts/smart-disk-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/smart-disk-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/smart-disk-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/smart-disk-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/smart-disk-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/smart-disk-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/smart-disk-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/smart-disk-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/smart-disk-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/smart-disk-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/smart-disk-monitor-error.log
  Execution log:               /opt/scripts/logs/smart-disk-monitor-execution.log
...
./smart-disk-monitor.sh --maintenance
...
scripts/
├── smart-disk-monitor.sh
├── smart-disk-monitor.status
├── smart-disk-monitor.email.state
├── smart-disk-monitor.lock
├── smart-disk-monitor.maintenance
└── logs/
    ├── smart-disk-monitor-error.log
    ├── smart-disk-monitor-error.log.2026-06-01_120000
    ├── smart-disk-monitor-execution.log
    └── smart-disk-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/smart-disk-monitor.status"
STATE_FILE="/tmp/smart-disk-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: smart-disk-monitor
            image: alpine/bash
            command: ["/scripts/smart-disk-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now smart-disk-monitor.timer
```

### Checkmk (local check)

```bash
cp smart-disk-monitor.sh /usr/lib/check_mk_agent/local/smart-disk-monitor.sh
```
...
```bash
echo "smart_disk_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/smart-disk-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/smart-disk-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/smart-disk-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/smart-disk-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/smart-disk-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/smart-disk-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/smart-disk-monitor.status` | Tracks OK/ALERT state. |
