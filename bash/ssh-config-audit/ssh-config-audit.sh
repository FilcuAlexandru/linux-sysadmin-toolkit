...
################################################################################
# ssh-config-audit.sh                                                          #
# Bash script that audits the effective sshd configuration for risky settings. #
# Prefers 'sshd -T' (effective config) and falls back to parsing sshd_config.  #
# Flags root login, password auth, empty passwords, and protocol 1.            #
...
#   - SSHD_CONFIG : path to sshd_config used as a fallback when      #
...
#   - Toggle with: ./ssh-config-audit.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="sshd configuration no longer flags risky settings on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in sshd awk mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                sshd) printf '  %-28s %b (falls back to parsing sshd_config)\n' "$bin" "$miss" ;;
...
alert_body() {
    ...
    echo "  PasswordAuthentication no   (use keys)"
    echo "  PermitEmptyPasswords no"
    echo "  Protocol 2 (default on modern OpenSSH)"
}
...
evaluate() {
    ...
    if command -v sshd >/dev/null 2>&1 && sshd -T >/dev/null 2>&1; then
        ...
        cfg=$(grep -vE '^[[:space:]]*#' "$SSHD_CONFIG" 2>/dev/null)
    else
        OVER=0; SUMMARY="sshd config not readable"; DETAIL="no sshd -T and unreadable ${SSHD_CONFIG}"; return
    fi
    local low; low=$(tr 'A-Z' 'a-z' <<<"$cfg")
    grep -qE 'permitrootlogin[[:space:]]+yes' <<<"$low" && findings+="PermitRootLogin=yes "
    grep -qE 'passwordauthentication[[:space:]]+yes' <<<"$low" && findings+="PasswordAuthentication=yes "
    grep -qE 'permitemptypasswords[[:space:]]+yes' <<<"$low" && findings+="PermitEmptyPasswords=yes "
    grep -qE 'protocol[[:space:]]+1' <<<"$low" && findings+="Protocol=1 "
    grep -qE 'x11forwarding[[:space:]]+yes' <<<"$low" && findings+="X11Forwarding=yes "
    if [[ -n "$findings" ]]; then
        OVER=1; SUMMARY="sshd insecure settings: ${findings}"; DETAIL="${findings}"
    else
        OVER=0; SUMMARY="sshd configuration looks hardened"; DETAIL="no risky settings found"
    fi
}
...
Audit the effective sshd configuration and alert on insecure settings.
