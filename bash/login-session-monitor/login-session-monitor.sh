...
# Bash script that counts active login sessions via who(1) and alerts above a threshold. #
# Helps spot unexpected concurrent access or runaway session leaks.                      #
# Lists the logged-in users and source addresses in the alert email.                     #
...
#   - Alert fires when the number of active login sessions exceeds  #
#     this value.                                                   #
######################################################################
...
#   - Toggle with: ./login-session-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Active session count back to normal on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in who mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                who) printf '  %-28s %b (required to enumerate sessions)\n' "$bin" "$miss" ;;
...
alert_body() {
    command -v who >/dev/null 2>&1 || { echo "  (who not available)"; return; }
    echo "Active sessions (user tty from when):"
...
evaluate() {
    command -v who >/dev/null 2>&1 || { OVER=0; SUMMARY="who not available"; DETAIL="who missing"; return; }
    local n; n=$(who 2>/dev/null | grep -c . || true); n=${n//[!0-9]/}; [[ -n "$n" ]] || n=0
    if (( n > THRESHOLD )); then
        OVER=1; SUMMARY="Active sessions: ${n} (threshold ${THRESHOLD})"; DETAIL="${n} active sessions"
    else
        OVER=0; SUMMARY="Active sessions: ${n}"; DETAIL="${n} active sessions"
    fi
}
...
Count active interactive login sessions and alert when more than ${THRESHOLD} are present.
