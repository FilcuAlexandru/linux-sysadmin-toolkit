...
cd linux-sysadmin-toolkit/bash/login-session-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/login-session-monitor/login-session-monitor.sh \
     -o /opt/scripts/login-session-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/login-session-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/login-session-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/login-session-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/login-session-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/login-session-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/login-session-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/login-session-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/login-session-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/login-session-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/login-session-monitor-error.log
  Execution log:               /opt/scripts/logs/login-session-monitor-execution.log
...
./login-session-monitor.sh --maintenance
...
scripts/
├── login-session-monitor.sh
├── login-session-monitor.status
├── login-session-monitor.email.state
├── login-session-monitor.lock
├── login-session-monitor.maintenance
└── logs/
    ├── login-session-monitor-error.log
    ├── login-session-monitor-error.log.2026-06-01_120000
    ├── login-session-monitor-execution.log
    └── login-session-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/login-session-monitor.status"
STATE_FILE="/tmp/login-session-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: login-session-monitor
            image: alpine/bash
            command: ["/scripts/login-session-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now login-session-monitor.timer
```

### Checkmk (local check)

```bash
cp login-session-monitor.sh /usr/lib/check_mk_agent/local/login-session-monitor.sh
```
...
```bash
echo "login_session_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/login-session-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/login-session-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/login-session-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/login-session-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/login-session-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/login-session-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/login-session-monitor.status` | Tracks OK/ALERT state. |
