# zombie-process-monitor.sh

Count zombie (defunct) processes and alert when too many accumulate.

---

## Features

- Counts only true zombies via the process STAT column (`Z`).
- Lists offending zombies and their parent PIDs in alert emails.
...
cd linux-sysadmin-toolkit/bash/zombie-process-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/zombie-process-monitor/zombie-process-monitor.sh \
     -o /opt/scripts/zombie-process-monitor.sh
```

### Manual copy

```bash
cp zombie-process-monitor.sh /opt/scripts/zombie-process-monitor.sh
chmod +x /opt/scripts/zombie-process-monitor.sh
```

### Verify

```bash
/opt/scripts/zombie-process-monitor.sh --version
/opt/scripts/zombie-process-monitor.sh --dry-run
```
...
|---|---|---|
| `THRESHOLD` | `5` | Alert when the zombie process count exceeds this number. |
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/zombie-process-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/zombie-process-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/zombie-process-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/zombie-process-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/zombie-process-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/zombie-process-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/zombie-process-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/zombie-process-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/zombie-process-monitor.status"
```
...
```
Usage: zombie-process-monitor.sh [--dry-run] [--maintenance] [--version] [--help]
...
```bash
./zombie-process-monitor.sh --dry-run
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/zombie-process-monitor-error.log
  Execution log:               /opt/scripts/logs/zombie-process-monitor-execution.log
...
./zombie-process-monitor.sh --maintenance
...
`ps -eo stat=` lists every process state; the script counts those whose state begins with `Z`. When the count exceeds `THRESHOLD`, it alerts and includes the zombie PIDs and their parents so the misbehaving parent can be restarted.
...
scripts/
├── zombie-process-monitor.sh
├── zombie-process-monitor.status
├── zombie-process-monitor.email.state
├── zombie-process-monitor.lock
├── zombie-process-monitor.maintenance
└── logs/
    ├── zombie-process-monitor-error.log
    ├── zombie-process-monitor-error.log.2026-06-01_120000
    ├── zombie-process-monitor-execution.log
    └── zombie-process-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/zombie-process-monitor.status"
STATE_FILE="/tmp/zombie-process-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: zombie-process-monitor
            image: alpine/bash
            command: ["/scripts/zombie-process-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```cron
*/5 * * * * /opt/scripts/zombie-process-monitor.sh >/dev/null 2>&1
```
...
```ini
# /etc/systemd/system/zombie-process-monitor.service
[Unit]
Description=zombie-process-monitor check

[Service]
Type=oneshot
ExecStart=/opt/scripts/zombie-process-monitor.sh
```

```ini
# /etc/systemd/system/zombie-process-monitor.timer
[Unit]
Description=Run zombie-process-monitor every 5 minutes
...
```bash
sudo systemctl enable --now zombie-process-monitor.timer
```

### Checkmk (local check)

```bash
cp zombie-process-monitor.sh /usr/lib/check_mk_agent/local/zombie-process-monitor.sh
```
...
```bash
echo "zombie_process_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/zombie-process-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/zombie-process-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/zombie-process-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/zombie-process-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/zombie-process-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/zombie-process-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/zombie-process-monitor.status` | Tracks OK/ALERT state. |
