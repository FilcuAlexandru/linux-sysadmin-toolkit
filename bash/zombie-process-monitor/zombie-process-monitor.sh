...
# Bash script that counts zombie/defunct processes and alerts above a threshold.          #
# Zombies indicate a parent failing to reap children; large counts exhaust the PID table. #
# Lists the offending parent PIDs in the alert email for fast root-cause analysis.        #
...
#   - Alert fires when the number of zombie processes exceeds this.  #
######################################################################
...
#   - Toggle with: ./zombie-process-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Zombie process count back to normal on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in ps awk mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                ps) printf '  %-28s %b (required to enumerate processes)\n' "$bin" "$miss" ;;
...
alert_body() {
    ...
    echo "Zombie processes (PID PPID CMD):"
    ps -eo pid,ppid,stat,comm 2>/dev/null | awk '$3 ~ /^Z/{printf "  %-8s %-8s %s\n", $1, $2, $4}'
}
...
evaluate() {
    command -v ps >/dev/null 2>&1 || { OVER=0; SUMMARY="ps not available"; DETAIL="ps missing"; return; }
    local z; z=$(ps -eo stat= 2>/dev/null | awk '$1 ~ /^Z/{c++} END{print c+0}')
    [[ "$z" =~ ^[0-9]+$ ]] || z=0
    if (( z > THRESHOLD )); then
        OVER=1; SUMMARY="Zombie processes: ${z} (threshold ${THRESHOLD})"; DETAIL="${z} zombies"
    else
        OVER=0; SUMMARY="Zombie processes: ${z}"; DETAIL="${z} zombies"
    fi
}
...
Count defunct (zombie) processes and alert when more than ${THRESHOLD} are present.
