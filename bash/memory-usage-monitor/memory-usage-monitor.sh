...
# Reads /proc/meminfo first; falls back to free(1) if /proc/meminfo is unavailable.              #
# Includes the top 5 memory-consuming processes in email alerts.                                 #
...
############################################################################
# Memory threshold.                                                        #
#   - Alert fires when used memory (excluding cache/buffers) exceeds this. #
...
#   - Toggle with: ./memory-usage-monitor.sh --maintenance #
...
######################################################################
# Status tracking.                                                   #
#   - Prevents repeated alerts while memory stays above threshold.   #
#   - Sends a recovery email when memory drops back below threshold. #
...
send_recovery_mail() {
    ...
    local body="Memory usage is back below ${THRESHOLD}% on ${HOST_ID}"
    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email to: ${ALERT_EMAIL}"; return 0
    fi
    # shellcheck disable=SC2086
    echo "$body" | mail -s "Memory recovery on ${HOST_ID}" $ALERT_EMAIL
...
check_prerequisites() {
    ...
    if [[ -r /proc/meminfo ]]; then
        ...
        printf '  %-28s %b (free fallback will be used)\n' "/proc/meminfo" "$miss"
    fi
    for bin in free ps mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                free)  printf '  %-28s %b (fallback for /proc/meminfo)\n' "$bin" "$miss" ;;
...
#######################################################################
# Alert.                                                              #
#   - top5_processes : collects the 5 most memory-intensive processes #
#                      via ps (RSS in MB) and returns a formatted     #
#                      table ready to embed in the alert email body.  #
...
# Collect the top 5 memory-consuming processes via ps (RSS in MB).
# Returns a formatted multi-line string ready to embed in the alert email body.
top5_processes() {
    command -v ps >/dev/null 2>&1 || { echo "  (ps not available)"; return; }
    ps -eo pid,rss,comm --sort=-rss 2>/dev/null \
        | awk 'NR==1{next} NR<=6{printf "  %-8s %8.1fMB  %s\n", $1, $2/1024, $3}'
}

# Emit console alerts, log to ERROR_LOG, and optionally send email with top 5 processes.
# Designed as an integration point for Checkmk, Grafana, Prometheus, etc.
alert() {
    local detail="$*"
    local top5; top5=$(top5_processes)
    local body
    body="Memory usage above ${THRESHOLD}% on ${HOST_ID}: ${detail}
...
${top5}"
    ...
    echo "ALERT: Memory usage above ${THRESHOLD}% on ${HOST_ID}: ${detail}" >&2
    ...
    if should_send_email; then
        # shellcheck disable=SC2086
        echo "$body" | mail -s "Memory alert on ${HOST_ID}" $ALERT_EMAIL
        mark_email_sent
...
#   - read_mem_usage : reads used/total memory in GB and a 0/1     #
#                      over-threshold flag.                        #
#   - Source priority:                                             #
#       1) /proc/meminfo — direct kernel data, no binary needed.   #
#                          Uses MemTotal and MemAvailable for      #
#                          accurate used = total - available.      #
#       2) free -m       — fallback; converts MB to GB in awk.     #
####################################################################

# Read memory usage in GB and return: used_gb total_gb pct over(0/1).
# Source priority:
#   1) /proc/meminfo — MemTotal and MemAvailable; accurate, no binary needed.
#      used = total - available  (excludes kernel cache/buffers, like free -m's "used")
#   2) free -m       — fallback; converts MB to GB in awk.
read_mem_usage() {
    if [[ -r /proc/meminfo ]]; then
        awk -v thr="$THRESHOLD" '
            ...
            END {
                ...
                printf "%.2f %.2f %.2f %d\n",
                    used_kb / 1048576, total_kb / 1048576,
                    pct, (pct > thr ? 1 : 0)
            }' /proc/meminfo
        return 0
    fi
...
Sends a recovery email when memory drops back below the threshold.
Email alerts include the top 5 memory-consuming processes.
...
##############################################################################
# Main.                                                                      #
#   - Reads memory usage from /proc/meminfo (preferred) or free -m fallback. #
#   - Prints used/total in GB with percentage (red when above threshold).    #
#   - Status-aware alerting: alerts once when usage goes above threshold,    #
#     stays silent while it remains high, recovers once it drops back.       #
##############################################################################
...
read -r used total pct over < <(read_mem_usage) || die "could not read memory usage"
[[ -n "$pct" ]] || die "could not parse memory usage"

if (( over )); then
    printf 'Memory Usage: %b%s/%sGB (%s%%)%b\n' "$RED" "$used" "$total" "$pct" "$RST"
    log_to "$EXECUTION_LOG" "RESULT ${pct}% used=${used}GB total=${total}GB (above threshold)"
else
    printf 'Memory Usage: %s/%sGB (%s%%)\n' "$used" "$total" "$pct"
    log_to "$EXECUTION_LOG" "RESULT ${pct}% used=${used}GB total=${total}GB (ok)"
fi

# Status-aware alerting: alert once when memory goes above threshold,
