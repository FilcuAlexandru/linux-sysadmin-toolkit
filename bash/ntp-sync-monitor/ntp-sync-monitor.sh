...
##############################################################################################
# ntp-sync-monitor.sh                                                                        #
# Bash script that verifies time synchronization across chrony, ntpd, and systemd-timesyncd. #
# Detects an unsynchronized clock or an offset larger than MAX_OFFSET_MS.                    #
# Tries timedatectl, chronyc, and ntpq in turn for maximum portability.                      #
...
#   - Alert fires when the absolute clock offset exceeds this, or    #
#     when the time service reports the clock is not synchronized.   #
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/ntp-sync-monitor.email.state"
...
ERROR_LOG="${LOG_DIR}/ntp-sync-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/ntp-sync-monitor-execution.log"
...
#   - Toggle with: ./ntp-sync-monitor.sh --maintenance            #
#   - State is stored in MAINTENANCE_FILE (auto-managed). #
###########################################################

MAINTENANCE_FILE="${SCRIPT_DIR}/ntp-sync-monitor.maintenance"
...
LOCK_FILE="${SCRIPT_DIR}/ntp-sync-monitor.lock"
...
STATUS_FILE="${SCRIPT_DIR}/ntp-sync-monitor.status"
...
send_recovery_mail() {
    ...
    local body="Clock back in sync on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in timedatectl chronyc ntpq mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                timedatectl) printf '  %-28s %b (systemd time status unavailable)\n' "$bin" "$miss" ;;
                chronyc) printf '  %-28s %b (chrony status unavailable)\n' "$bin" "$miss" ;;
                ntpq) printf '  %-28s %b (ntpd status unavailable)\n' "$bin" "$miss" ;;
...
alert_body() {
    if command -v chronyc >/dev/null 2>&1; then chronyc tracking 2>/dev/null && return; fi
    if command -v timedatectl >/dev/null 2>&1; then timedatectl status 2>/dev/null; fi
}
...
evaluate() {
    local synced="unknown" offset="" src="none"
    if command -v chronyc >/dev/null 2>&1; then
        local trk; trk=$(chronyc tracking 2>/dev/null || true)
        if [[ -n "$trk" ]]; then
            src="chronyc"
            local lo leap
            lo=$(awk -F': ' '/Last offset/{print $2}' <<<"$trk" | awk '{print $1}')
            leap=$(awk -F': ' '/Leap status/{print $2}' <<<"$trk")
            [[ -n "$lo" ]] && offset=$(awk -v o="$lo" 'BEGIN{o=(o<0?-o:o); printf "%.0f", o*1000}')
            [[ -n "$leap" ]] && { [[ "$leap" == Normal* ]] && synced=yes || synced=no; }
        fi
    fi
    if [[ "$src" == none ]] && command -v ntpq >/dev/null 2>&1; then
        local off; off=$(ntpq -pn 2>/dev/null | awk '/^\*/{print $9; exit}')
        if [[ -n "$off" ]]; then src="ntpq"; offset=$(awk -v o="$off" 'BEGIN{o=(o<0?-o:o); printf "%.0f", o}'); synced=yes; fi
    fi
    if [[ "$src" == none ]] && command -v timedatectl >/dev/null 2>&1; then
        local ts; ts=$(timedatectl show -p NTPSynchronized --value 2>/dev/null || true)
        [[ -n "$ts" ]] && { src="timedatectl"; [[ "$ts" == yes ]] && synced=yes || synced=no; }
    fi
    if [[ "$src" == none ]]; then
        OVER=1; SUMMARY="No time-sync service detected"; DETAIL="no chrony/ntpd/timesyncd found"; return
    fi
    local over=0
    [[ "$synced" == no ]] && over=1
    if [[ -n "$offset" && "$offset" =~ ^[0-9]+$ ]] && (( offset > MAX_OFFSET_MS )); then over=1; fi
    OVER=$over
    if (( over )); then
        SUMMARY="Clock NOT in sync (${src}: synced=${synced} offset=${offset:-?}ms)"
        DETAIL="synced=${synced} offset=${offset:-unknown}ms threshold=${MAX_OFFSET_MS}ms"
    else
        SUMMARY="Clock in sync (${src}: offset=${offset:-0}ms)"; DETAIL="offset=${offset:-0}ms"
    fi
}
...
Check that the system clock is synchronized and within ${MAX_OFFSET_MS} ms of true time.
