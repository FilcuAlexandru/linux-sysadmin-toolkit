...
cd linux-sysadmin-toolkit/bash/package-update-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/package-update-monitor/package-update-monitor.sh \
     -o /opt/scripts/package-update-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/package-update-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/package-update-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/package-update-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/package-update-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/package-update-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/package-update-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/package-update-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/package-update-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/package-update-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/package-update-monitor-error.log
  Execution log:               /opt/scripts/logs/package-update-monitor-execution.log
...
./package-update-monitor.sh --maintenance
...
scripts/
├── package-update-monitor.sh
├── package-update-monitor.status
├── package-update-monitor.email.state
├── package-update-monitor.lock
├── package-update-monitor.maintenance
└── logs/
    ├── package-update-monitor-error.log
    ├── package-update-monitor-error.log.2026-06-01_120000
    ├── package-update-monitor-execution.log
    └── package-update-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/package-update-monitor.status"
STATE_FILE="/tmp/package-update-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: package-update-monitor
            image: alpine/bash
            command: ["/scripts/package-update-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now package-update-monitor.timer
```

### Checkmk (local check)

```bash
cp package-update-monitor.sh /usr/lib/check_mk_agent/local/package-update-monitor.sh
```
...
```bash
echo "package_update_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/package-update-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/package-update-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/package-update-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/package-update-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/package-update-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/package-update-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/package-update-monitor.status` | Tracks OK/ALERT state. |
