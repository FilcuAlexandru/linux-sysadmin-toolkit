...
# Bash script that counts available package updates on any major distribution.      #
# Auto-detects apt, dnf, yum, or zypper and alerts when updates exceed a threshold. #
# Read-only: it never installs anything, only reports what is pending.              #
...
#   - Alert fires when the number of available updates exceeds this. #
#   - Set to 0 to alert whenever any update is available.           #
######################################################################
...
#   - Toggle with: ./package-update-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Pending updates back below threshold on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in apt-get dnf zypper mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                apt-get) printf '  %-28s %b (Debian/Ubuntu update check unavailable)\n' "$bin" "$miss" ;;
                dnf) printf '  %-28s %b (Fedora/RHEL update check unavailable)\n' "$bin" "$miss" ;;
                zypper) printf '  %-28s %b (SLES update check unavailable)\n' "$bin" "$miss" ;;
...
alert_body() {
    if command -v apt-get >/dev/null 2>&1; then
        echo "Pending updates (first 20):"; apt-get --just-print upgrade 2>/dev/null | awk '/^Inst/{print "  "$2}' | head -20
    elif command -v dnf >/dev/null 2>&1; then
        echo "Pending updates (first 20):"; dnf -q check-update 2>/dev/null | awk 'NF>=3{print "  "$1}' | head -20
    fi
}
...
evaluate() {
    local mgr=none cnt=0
    if command -v apt-get >/dev/null 2>&1; then
        mgr=apt; cnt=$(apt-get --just-print upgrade 2>/dev/null | grep -c '^Inst' || true)
    elif command -v dnf >/dev/null 2>&1; then
        mgr=dnf; cnt=$(dnf -q check-update 2>/dev/null | grep -cE '^[a-zA-Z0-9._+-]+[[:space:]]' || true)
    elif command -v yum >/dev/null 2>&1; then
        mgr=yum; cnt=$(yum -q check-update 2>/dev/null | grep -cE '^[a-zA-Z0-9._+-]+[[:space:]]' || true)
    elif command -v zypper >/dev/null 2>&1; then
        mgr=zypper; cnt=$(zypper -q list-updates 2>/dev/null | grep -cE '^v \|' || true)
    fi
    cnt=${cnt//[!0-9]/}; [[ -n "$cnt" ]] || cnt=0
    if [[ "$mgr" == none ]]; then
        OVER=0; SUMMARY="No supported package manager found"; DETAIL="no apt/dnf/yum/zypper"; return
    fi
    if (( cnt > THRESHOLD )); then
        OVER=1; SUMMARY="${cnt} updates available (${mgr}, threshold ${THRESHOLD})"; DETAIL="${cnt} updates via ${mgr}"
    else
        OVER=0; SUMMARY="${cnt} updates available (${mgr})"; DETAIL="${cnt} updates via ${mgr}"
    fi
}
...
Count pending package updates (apt/dnf/yum/zypper) and alert above ${THRESHOLD}.
