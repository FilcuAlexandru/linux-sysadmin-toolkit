...
# Idempotent: services already running are left untouched.                   #
...
#   - Each name is passed to systemctl restart <name>.       #
#   - Running check uses pgrep -x (exact binary name match). #
...
#   - Toggle with: ./service-watchdog.sh --maintenance    #
...
#######################################################################
# Status tracking.                                                    #
#   - Prevents repeated alerts while restart failures keep occurring. #
#   - Sends a recovery email when all services are running again      #
...
# Colors only when writing to a terminal: green = running, red = not running / failed.
...
########################################################################
# Recovery.                                                            #
#   - send_recovery_mail : sends a single recovery email when all      #
#                          services are found running after a previous #
#                          restart failure (status was ALERT).         #
#                          Not rate-limited. Skipped if ALERT_EMAIL    #
#                          is empty or mail(1) is unavailable.         #
########################################################################

send_recovery_mail() {
    [[ -n "$ALERT_EMAIL" ]]        || return 0
    command -v mail >/dev/null 2>&1 || return 0
    local body="All services are running again on ${HOST_ID}"
    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email to: ${ALERT_EMAIL}"; return 0
    fi
    # shellcheck disable=SC2086
    echo "$body" | mail -s "Service watchdog recovery on ${HOST_ID}" $ALERT_EMAIL
...
#     the configured service list, and the current runtime state. #
...
alert() {
    local detail="$*"
    local body="Service restart failure on ${HOST_ID}: ${detail}"
    ...
    if should_send_email; then
        # shellcheck disable=SC2086
        echo "$body" | mail -s "Service watchdog alert on ${HOST_ID}" $ALERT_EMAIL
        mark_email_sent
...
Restart any SERVICES entry that is not running. Idempotent: already-running
services are left untouched. Alerts on restart failure (not on restart action).
Sends a recovery email when all services are running after a previous failure.
...
#######################################################################
# Main.                                                               #
#   - Checks each service: if already running, prints status.         #
#   - If not running: attempts systemctl restart.                     #
#   - On restart failure: alerts and updates status to ALERT.         #
#   - On restart success: logs RESTARTED, no alert (expected action). #
#   - On full success (all running): updates status to OK and sends   #
#     recovery email if previous run had failures.                    #
#######################################################################
...
log_to "$EXECUTION_LOG" "START [${HOST_ID}] managing ${#SERVICES[@]} service(s): ${SERVICES[*]}"
...
for svc in "${SERVICES[@]}"; do
    if pgrep -x "$svc" >/dev/null 2>&1; then
        ...
        log_to "$EXECUTION_LOG" "RUNNING ${svc}"
    else
        printf '%b%-24s not running%b\n' "$RED" "$svc" "$RST"
        if (( DRY_RUN )); then
            printf '%b%-24s would restart%b\n' "$RED" "$svc" "$RST"
            log_to "$EXECUTION_LOG" "DRY-RUN would restart ${svc}"
        else
            if systemctl restart "$svc" 2>/dev/null; then
                printf '%b%-24s restarted%b\n' "$GREEN" "$svc" "$RST"
                log_to "$EXECUTION_LOG" "RESTARTED ${svc}"
                restarted+=("$svc")
            else
                printf '%b%-24s FAILED to restart%b\n' "$RED" "$svc" "$RST" >&2
                log_to "$EXECUTION_LOG" "FAILED ${svc}"
                failed_to_restart+=("$svc")
            fi
        fi
    fi
done

if (( ${#failed_to_restart[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "RESULT ${#failed_to_restart[@]} service(s) failed to restart: ${failed_to_restart[*]}"
    if [[ "$current_status" != "ALERT" ]]; then
        set_status ALERT
        alert "failed to restart: ${failed_to_restart[*]}"
    else
        log_to "$EXECUTION_LOG" "Already in ALERT state"
    fi
elif (( ${#restarted[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "RESULT restarted: ${restarted[*]}"
    if [[ "$current_status" == "ALERT" ]]; then
        set_status OK
        send_recovery_mail
    else
    ...
    log_to "$EXECUTION_LOG" "RESULT all services running"
    if [[ "$current_status" == "ALERT" ]]; then
        set_status OK
        send_recovery_mail
    fi
