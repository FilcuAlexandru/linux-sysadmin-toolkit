...
# Bash script that scans the kernel log for OOM-killer events in a recent window. #
# Uses journalctl -k where available and falls back to dmesg.                     #
# Surfaces which processes the kernel killed to reclaim memory.                   #
...
######################################################################
# Scan window.                                                      #
#   - SINCE : how far back to scan for OOM events (journalctl form). #
#             Only used when journalctl is available.               #
######################################################################
...
#   - Toggle with: ./oom-killer-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="No further OOM-killer activity on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in journalctl dmesg mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                journalctl) printf '  %-28s %b (falls back to dmesg)\n' "$bin" "$miss" ;;
                dmesg) printf '  %-28s %b (fallback kernel-log source)\n' "$bin" "$miss" ;;
...
alert_body() {
    echo "Recent OOM-killer log lines:"
    if command -v journalctl >/dev/null 2>&1; then
        journalctl -k --since "$SINCE" 2>/dev/null | grep -iE 'out of memory|oom-kill|killed process' | tail -10 | sed 's/^/  /'
    elif command -v dmesg >/dev/null 2>&1; then
        dmesg 2>/dev/null | grep -iE 'out of memory|oom-kill|killed process' | tail -10 | sed 's/^/  /'
    fi
}
...
evaluate() {
    local hits=0 src=none
    if command -v journalctl >/dev/null 2>&1; then
        src=journalctl
        hits=$(journalctl -k --since "$SINCE" 2>/dev/null | grep -icE 'out of memory|oom-kill|killed process' || true)
    elif command -v dmesg >/dev/null 2>&1; then
        src=dmesg
        hits=$(dmesg 2>/dev/null | grep -icE 'out of memory|oom-kill|killed process' || true)
    fi
    hits=${hits//[!0-9]/}; [[ -n "$hits" ]] || hits=0
    if [[ "$src" == none ]]; then
        OVER=0; SUMMARY="No kernel-log source available"; DETAIL="no journalctl/dmesg"; return
    fi
    if (( hits > 0 )); then
        OVER=1; SUMMARY="OOM-killer activity detected (${hits} events, ${src})"; DETAIL="${hits} OOM events"
    else
        OVER=0; SUMMARY="No OOM-killer activity (${src})"; DETAIL="0 OOM events"
    fi
}
...
Scan the kernel log for OOM-killer events within ${SINCE} and alert if any are found.
