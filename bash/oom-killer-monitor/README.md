...
cd linux-sysadmin-toolkit/bash/oom-killer-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/oom-killer-monitor/oom-killer-monitor.sh \
     -o /opt/scripts/oom-killer-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/oom-killer-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/oom-killer-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/oom-killer-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/oom-killer-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/oom-killer-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/oom-killer-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/oom-killer-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/oom-killer-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/oom-killer-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/oom-killer-monitor-error.log
  Execution log:               /opt/scripts/logs/oom-killer-monitor-execution.log
...
./oom-killer-monitor.sh --maintenance
...
scripts/
├── oom-killer-monitor.sh
├── oom-killer-monitor.status
├── oom-killer-monitor.email.state
├── oom-killer-monitor.lock
├── oom-killer-monitor.maintenance
└── logs/
    ├── oom-killer-monitor-error.log
    ├── oom-killer-monitor-error.log.2026-06-01_120000
    ├── oom-killer-monitor-execution.log
    └── oom-killer-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/oom-killer-monitor.status"
STATE_FILE="/tmp/oom-killer-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: oom-killer-monitor
            image: alpine/bash
            command: ["/scripts/oom-killer-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now oom-killer-monitor.timer
```

### Checkmk (local check)

```bash
cp oom-killer-monitor.sh /usr/lib/check_mk_agent/local/oom-killer-monitor.sh
```
...
```bash
echo "oom_killer_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/oom-killer-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/oom-killer-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/oom-killer-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/oom-killer-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/oom-killer-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/oom-killer-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/oom-killer-monitor.status` | Tracks OK/ALERT state. |
