...
###################################################################################
# tmp-cleanup.sh                                                                  #
# Bash script that removes files older than a given age from a temp directory     #
# and prunes empty subdirectories.                                                #
...
#   - These are the script's own run logs, not the temp dir being cleaned. #
...
ERROR_LOG="${LOG_DIR_META}/tmp-cleanup-error.log"
EXECUTION_LOG="${LOG_DIR_META}/tmp-cleanup-execution.log"
...
STATUS_FILE="${SCRIPT_DIR}/tmp-cleanup.status"
...
######################################################################
# Recovery.                                                          #
#   - send_recovery_mail : sends a single recovery email when tmp    #
...
send_recovery_mail() {
    ...
    local body="Temp cleanup of ${TMP_DIR} succeeded again on ${HOST_ID}"
    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email to: ${ALERT_EMAIL}"; return 0
    fi
    # shellcheck disable=SC2086
    echo "$body" | mail -s "Temp cleanup recovery on ${HOST_ID}" $ALERT_EMAIL
...
Delete files older than AGE_DAYS in TMP_DIR, then prune empty subdirectories.
...
####################################################################
# Main.                                                            #
#   - Finds files older than AGE_DAYS in TMP_DIR.                  #
#   - In dry-run: lists what would be deleted, then exits.         #
#   - Deletes each file; prunes empty subdirectories.              #
...
log_to "$EXECUTION_LOG" "START [${HOST_ID}] dir=${TMP_DIR} age=${AGE_DAYS}d"

[[ -d "$TMP_DIR" ]] || {
    current_status=$(get_status)
    log_to "$EXECUTION_LOG" "RESULT directory not found: ${TMP_DIR}"
    if [[ "$current_status" != "ALERT" ]]; then
        set_status ALERT
        alert "directory not found: ${TMP_DIR}"
    else
        log_to "$EXECUTION_LOG" "Already in ALERT state"
    fi
    die "directory not found: $TMP_DIR"
}
...
if (( fail > 0 )); then
    ...
    if [[ "$current_status" != "ALERT" ]]; then
        set_status ALERT
        alert "${fail} file(s) could not be removed from ${TMP_DIR}"
    else
        log_to "$EXECUTION_LOG" "Already in ALERT state"
    fi
else
