...
# Bash script for monitoring inode usage across all real filesystems.                  #
# Parses df -iP (POSIX) so output is locale- and distro-independent.                   #
# Catches the classic 'No space left on device' caused by inode (not byte) exhaustion. #
...
#   - Alert fires when any filesystem's inode usage exceeds this.    #
...
#   - Toggle with: ./inode-usage-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Inode usage back below threshold on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    for bin in df awk mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            ...
            case "$bin" in
                df) printf '  %-28s %b (required for inode statistics)\n' "$bin" "$miss" ;;
                awk) printf '  %-28s %b (required for parsing)\n' "$bin" "$miss" ;;
...
alert_body() {
    command -v df >/dev/null 2>&1 || { echo "  (df not available)"; return; }
    echo "Inode usage by filesystem:"
    df -iP 2>/dev/null | awk 'NR==1 || $5+0 > 0 {printf "  %-24s %6s used of %s\n", $6, $5, $2}'
}
...
evaluate() {
    command -v df >/dev/null 2>&1 || { OVER=0; SUMMARY="df not available"; DETAIL="df missing"; return; }
    local worst_pct=0 worst_mnt="none" details="" p mnt iuse
    while read -r _fs _it _iu _if iuse mnt; do
        [[ "$iuse" == *% ]] || continue
        p=${iuse%\%}
        [[ "$p" =~ ^[0-9]+$ ]] || continue
        if (( p > worst_pct )); then worst_pct=$p; worst_mnt=$mnt; fi
        if (( p > THRESHOLD )); then details+="${mnt}=${p}% "; fi
    done < <(df -iP 2>/dev/null | awk 'NR>1')
    if [[ -n "$details" ]]; then
        OVER=1; SUMMARY="Inode usage high: ${details}"; DETAIL="${details}(threshold ${THRESHOLD}%)"
    else
        OVER=0; SUMMARY="Inode usage OK (max ${worst_pct}% on ${worst_mnt})"; DETAIL="max ${worst_pct}%"
    fi
}
...
Report inode usage for every real filesystem and alert when any exceeds ${THRESHOLD}%.
