...
cd linux-sysadmin-toolkit/bash/inode-usage-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/inode-usage-monitor/inode-usage-monitor.sh \
     -o /opt/scripts/inode-usage-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/inode-usage-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/inode-usage-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/inode-usage-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/inode-usage-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/inode-usage-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/inode-usage-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/inode-usage-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/inode-usage-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/inode-usage-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/inode-usage-monitor-error.log
  Execution log:               /opt/scripts/logs/inode-usage-monitor-execution.log
...
./inode-usage-monitor.sh --maintenance
...
scripts/
├── inode-usage-monitor.sh
├── inode-usage-monitor.status
├── inode-usage-monitor.email.state
├── inode-usage-monitor.lock
├── inode-usage-monitor.maintenance
└── logs/
    ├── inode-usage-monitor-error.log
    ├── inode-usage-monitor-error.log.2026-06-01_120000
    ├── inode-usage-monitor-execution.log
    └── inode-usage-monitor-execution.log.2026-06-01_120000
```
    ...
    spec:
      template:
        spec:
          ...
          - name: inode-usage-monitor
            image: alpine/bash
            command: ["/scripts/inode-usage-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
cp inode-usage-monitor.sh /usr/lib/check_mk_agent/local/inode-usage-monitor.sh
```
...
```bash
echo "inode_usage_monitor_alert{host=\"${HOST_ID}\"} 1" \
...
| `STATE_FILE` | `<script_dir>/inode-usage-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/inode-usage-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/inode-usage-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/inode-usage-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/inode-usage-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/inode-usage-monitor.status` | Tracks OK/ALERT state. |
