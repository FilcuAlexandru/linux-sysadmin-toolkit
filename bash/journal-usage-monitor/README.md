...
cd linux-sysadmin-toolkit/bash/journal-usage-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/journal-usage-monitor/journal-usage-monitor.sh \
     -o /opt/scripts/journal-usage-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/journal-usage-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/journal-usage-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/journal-usage-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/journal-usage-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/journal-usage-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/journal-usage-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/journal-usage-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/journal-usage-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/journal-usage-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/journal-usage-monitor-error.log
  Execution log:               /opt/scripts/logs/journal-usage-monitor-execution.log
...
./journal-usage-monitor.sh --maintenance
...
scripts/
├── journal-usage-monitor.sh
├── journal-usage-monitor.status
├── journal-usage-monitor.email.state
├── journal-usage-monitor.lock
├── journal-usage-monitor.maintenance
└── logs/
    ├── journal-usage-monitor-error.log
    ├── journal-usage-monitor-error.log.2026-06-01_120000
    ├── journal-usage-monitor-execution.log
    └── journal-usage-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/journal-usage-monitor.status"
STATE_FILE="/tmp/journal-usage-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: journal-usage-monitor
            image: alpine/bash
            command: ["/scripts/journal-usage-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now journal-usage-monitor.timer
```

### Checkmk (local check)

```bash
cp journal-usage-monitor.sh /usr/lib/check_mk_agent/local/journal-usage-monitor.sh
```
...
```bash
echo "journal_usage_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/journal-usage-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/journal-usage-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/journal-usage-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/journal-usage-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/journal-usage-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/journal-usage-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/journal-usage-monitor.status` | Tracks OK/ALERT state. |
