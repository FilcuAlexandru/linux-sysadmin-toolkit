...
# Bash script that monitors how much disk the systemd journal consumes.      #
# Alerts when journal storage exceeds a configurable size in megabytes.      #
# Cleanly reports when journald is not in use (non-systemd hosts).           #
...
######################################################################
# Journal size threshold.                                           #
#   - Alert fires when the systemd journal uses more than this many  #
#     megabytes on disk.                                            #
######################################################################
...
#   - Toggle with: ./journal-usage-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Journal disk usage back below threshold on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in journalctl mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                journalctl) printf '  %-28s %b (journal usage cannot be measured)\n' "$bin" "$miss" ;;
...
alert_body() {
    echo "Reduce journal size with one of:"
    echo "  journalctl --vacuum-size=500M"
    echo "  journalctl --vacuum-time=7d"
    echo "Or set SystemMaxUse= in /etc/systemd/journald.conf"
}
...
evaluate() {
    command -v journalctl >/dev/null 2>&1 || { OVER=0; SUMMARY="journalctl not available"; DETAIL="non-systemd host"; return; }
    local line mb
    line=$(journalctl --disk-usage 2>/dev/null || true)
    mb=$(awk '{for(i=1;i<=NF;i++){ if($i ~ /^[0-9.]+[KMGTkmgt]?B?$/){v=$i} }
              n=v; sub(/[A-Za-z]+$/,"",n); u=v; gsub(/[0-9.]/,"",u); u=toupper(substr(u,1,1));
              f=1; if(u=="K")f=1/1024; else if(u=="M")f=1; else if(u=="G")f=1024; else if(u=="T")f=1048576; else f=1/1048576;
              if(n!="") printf "%.0f", n*f}' <<<"$line")
    [[ "$mb" =~ ^[0-9]+$ ]] || { OVER=0; SUMMARY="Could not parse journal size"; DETAIL="parse failed: ${line}"; return; }
    if (( mb > MAX_MB )); then
        OVER=1; SUMMARY="Journal using ${mb} MB (max ${MAX_MB} MB)"; DETAIL="${mb} MB on disk"
    else
        OVER=0; SUMMARY="Journal using ${mb} MB"; DETAIL="${mb} MB on disk"
    fi
}
...
Monitor systemd journal disk usage and alert above ${MAX_MB} MB.
