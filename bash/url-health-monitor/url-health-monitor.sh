...
###################################################################################
# URLs to check.                                                                  #
#   - Add or remove entries in the URLS array below.                              #
#   - Each entry is checked independently.                                        #
#   - Example: URLS=("https://example.com" "https://api.example.com:8080/health") #
###################################################################################
...
#   - EXPECTED_2XX : set to 1 to accept only 2xx (reject 3xx redirects). #
##########################################################################
...
#   - Toggle with: ./url-health-monitor.sh --maintenance  #
...
#   - Tracks which URLs are currently in ALERT state.       #
#   - Alerts once when a URL first fails.                   #
#   - Sends a recovery email when it comes back up.         #
#   - State file holds a space-separated list of down URLs. #
#############################################################
...
#                    separated list of URLs currently in ALERT.   #
#                    Returns empty string when file is missing,   #
#                    empty, or unreadable.                        #
#   - is_in_alert  : returns 0 if a given URL is in the current   #
#                    ALERT list; 1 otherwise.                     #
#   - add_alert    : adds a URL to the ALERT list and saves it.   #
#   - remove_alert : removes a URL from the ALERT list and saves. #
###################################################################
...
add_alert() {
    ...
    local alerted; alerted=$(get_alerted)
    if [[ " $alerted " != *" $url "* ]]; then
        echo "${alerted} ${url}" | xargs > "$STATUS_FILE" 2>/dev/null || true
    fi
}
...
#                          URL comes back up after being in ALERT #
#                          state. Not rate-limited. Skipped if    #
...
send_recovery_mail() {
    ...
    local body="${url} is back up on ${HOST_ID}"

    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email for ${url} to: ${ALERT_EMAIL}"
        return 0
    fi

    # shellcheck disable=SC2086
    echo "$body" | mail -s "URL recovery on ${HOST_ID}" $ALERT_EMAIL
    log_to "$ERROR_LOG" "RECOVERY EMAIL sent for ${url} to ${ALERT_EMAIL}"
}
...
check_prerequisites() {
    ...
    printf '  %-28s %s\n' "Host ID:"        "$HOST_ID"
    printf '  %-28s %ss\n' "Timeout:"       "$TIMEOUT"
    printf '  %-28s %s\n' "Strict 2xx:"    "$(( EXPECTED_2XX )) == 1 && echo yes || echo no"
...
alert() {
    local detail="$*"
    local body="URLs unhealthy on ${HOST_ID}: ${detail}"
    ...
    if should_send_email; then
        # shellcheck disable=SC2086
        echo "$body" | mail -s "URL alert on ${HOST_ID}" $ALERT_EMAIL
        mark_email_sent
...
####################################################################
# URL check.                                                       #
#   - check_url : sends an HTTP HEAD request (falling back to GET  #
#                 if the server rejects HEAD) via curl and returns #
#                 the HTTP status code. Returns 000 on connection  #
#                 failure, timeout, or any curl error.             #
#                 Respects TIMEOUT and EXPECTED_2XX.               #
####################################################################

# Check a single URL and return its HTTP status code.
# Tries HEAD first (lighter); falls back to GET if HEAD returns 405 (Method Not Allowed).
# Returns 000 on connection failure, timeout, or any curl error.
check_url() {
    local url=$1
    local code

    code=$(curl -s -o /dev/null -w '%{http_code}' \
        --head --max-time "$TIMEOUT" "$url" 2>/dev/null || echo "000")
...
# Return 0 if the status code is acceptable.
# Default: 2xx or 3xx. With EXPECTED_2XX=1: 2xx only.
is_healthy() {
    local code=$1
    if (( EXPECTED_2XX )); then
        [[ "$code" =~ ^2 ]]
    else
        [[ "$code" =~ ^[23] ]]
    fi
...
Sends a recovery email when a URL comes back up after a failure.
...
#################################################################
# Main.                                                         #
#   - Checks each URL in the URLS array independently.          #
#   - Per-URL status tracking: alerts once on first failure,    #
#     stays silent while down, recovers once when back up.      #
#   - Sends a single aggregated email for all newly failed URLs #
#     and individual recovery emails as each URL recovers.      #
#################################################################
...
log_to "$EXECUTION_LOG" "START [${HOST_ID}] checking ${#URLS[@]} URL(s)"
...
for url in "${URLS[@]}"; do
    ...
    if is_healthy "$code"; then
        printf '%b%-50s %s%b\n' "$GREEN" "$url" "$code" "$RST"
        log_to "$EXECUTION_LOG" "OK ${url} (${code})"

        if is_in_alert "$url"; then
            newly_recovered+=("$url")
            (( DRY_RUN )) || remove_alert "$url"
        fi
    else
        printf '%b%-50s %s%b\n' "$RED" "$url" "$code" "$RST"
        log_to "$EXECUTION_LOG" "DOWN ${url} (${code})"

        if ! is_in_alert "$url"; then
            newly_down+=("$url (${code})")
            (( DRY_RUN )) || add_alert "$url"
        fi
    fi
done

# Single aggregated alert for all newly down URLs.
if (( ${#newly_down[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "ALERT ${newly_down[*]}"
    alert "${newly_down[*]}"
fi

# Individual recovery email per recovered URL.
for url in "${newly_recovered[@]}"; do
    log_to "$EXECUTION_LOG" "RECOVERY ${url}"
    send_recovery_mail "$url"
done

if (( ${#newly_down[@]} == 0 && ${#newly_recovered[@]} == 0 )); then
    log_to "$EXECUTION_LOG" "RESULT all URLs healthy"
fi
