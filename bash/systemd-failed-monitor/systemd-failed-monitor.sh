...
#   - Toggle with: ./systemd-failed-monitor.sh --maintenance #
#   - State is stored in MAINTENANCE_FILE (auto-managed).    #
##############################################################

MAINTENANCE_FILE="${SCRIPT_DIR}/systemd-failed-monitor.maintenance"
...
LOCK_FILE="${SCRIPT_DIR}/systemd-failed-monitor.lock"

#####################################################################
# Status tracking (per-unit).                                       #
#   - Tracks which systemd units are currently in ALERT state.      #
#   - Alerts once when a unit first enters the failed state.        #
#   - Sends a recovery email when it leaves the failed state.       #
#   - State file holds a space-separated list of failed unit names. #
#####################################################################

STATUS_FILE="${SCRIPT_DIR}/systemd-failed-monitor.status"
...
#                    separated list of unit names currently in   #
#                    ALERT state. Returns empty string when file #
#                    is missing, empty, or unreadable.           #
#   - is_in_alert  : returns 0 if a given unit name is in the    #
#                    current ALERT list; 1 otherwise.            #
#   - add_alert    : adds a unit name to the ALERT list and      #
#                    saves it to STATUS_FILE.                    #
#   - remove_alert : removes a unit name from the ALERT list     #
...
add_alert() {
    local unit=$1
    local alerted; alerted=$(get_alerted)
    if [[ " $alerted " != *" $unit "* ]]; then
        echo "${alerted} ${unit}" | xargs > "$STATUS_FILE" 2>/dev/null || true
    fi
}
...
#                          unit leaves the failed state. Not rate-  #
...
send_recovery_mail() {
    ...
    local body="${unit} is no longer in failed state on ${HOST_ID}"

    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email for ${unit} to: ${ALERT_EMAIL}"
        return 0
    fi

    # shellcheck disable=SC2086
    echo "$body" | mail -s "Systemd recovery on ${HOST_ID}" $ALERT_EMAIL
    log_to "$ERROR_LOG" "RECOVERY EMAIL sent for ${unit} to ${ALERT_EMAIL}"
}

###################################################################
# Prerequisites check.                                            #
#   - Prints the availability of all required and optional tools, #
#     the active configuration, and the current alert state.      #
...
alert() {
    local detail="$*"
    local body="Failed systemd units on ${HOST_ID}: ${detail}"
    ...
    if should_send_email; then
        # shellcheck disable=SC2086
        echo "$body" | mail -s "Systemd alert on ${HOST_ID}" $ALERT_EMAIL
        mark_email_sent
...
Sends a recovery email when a unit leaves the failed state.
...
##################################################################
# Main.                                                          #
#   - Queries systemctl for all units in the failed state.       #
#   - Per-unit status tracking: alerts once on first failure,    #
#     stays silent while the unit remains failed, recovers once  #
#     the unit leaves the failed state (reset or restarted).     #
#   - Sends a single aggregated email for all newly failed units #
#     and individual recovery emails as each unit recovers.      #
##################################################################
...
# Query systemd for all units currently in the failed state.
mapfile -t currently_failed < <(
...
# Cross-reference current failed list against saved ALERT state.
for unit in "${currently_failed[@]}"; do
    if ! is_in_alert "$unit"; then
        newly_failed+=("$unit")
        (( DRY_RUN )) || add_alert "$unit"
    fi
done

# Units previously in ALERT that are no longer in the failed list have recovered.
alerted_list=$(get_alerted)
if [[ -n "$alerted_list" ]]; then
    while IFS= read -r unit; do
        [[ -z "$unit" ]] && continue
        local_found=0
        for f in "${currently_failed[@]}"; do
            [[ "$f" == "$unit" ]] && { local_found=1; break; }
        done
        if (( ! local_found )); then
            newly_recovered+=("$unit")
            (( DRY_RUN )) || remove_alert "$unit"
        fi
    done < <(echo "$alerted_list" | tr ' ' '\n')
fi

# Display results.
if (( ${#currently_failed[@]} == 0 )); then
    printf '%bNo failed units%b\n' "$GREEN" "$RST"
    log_to "$EXECUTION_LOG" "RESULT no failed units"
else
    echo "Failed units:"
    for unit in "${currently_failed[@]}"; do
        printf '%b  %s%b\n' "$RED" "$unit" "$RST"
    done
    log_to "$EXECUTION_LOG" "RESULT failed: ${currently_failed[*]}"
fi

# Single aggregated alert for all newly failed units.
if (( ${#newly_failed[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "ALERT ${newly_failed[*]}"
    alert "${newly_failed[*]}"
fi

# Individual recovery email per recovered unit.
for unit in "${newly_recovered[@]}"; do
    log_to "$EXECUTION_LOG" "RECOVERY ${unit}"
    send_recovery_mail "$unit"
done
