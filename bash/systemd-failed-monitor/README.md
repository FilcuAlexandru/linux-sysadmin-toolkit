...
```bash
STATUS_FILE="/tmp/systemd-failed-monitor.status"
STATE_FILE="/tmp/systemd-failed-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: systemd-failed-monitor
            image: alpine/bash
            command: ["/scripts/systemd-failed-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
