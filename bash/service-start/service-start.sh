...
##############################################################################
# service-start.sh                                                           #
# Bash script that starts any configured service that is not running.        #
...
#   - Each name is passed to systemctl start <name>.         #
...
#   - Toggle with: ./service-start.sh --maintenance       #
...
#####################################################################
# Status tracking.                                                  #
#   - Prevents repeated alerts while start failures keep occurring. #
#   - Sends a recovery email when all services start successfully   #
...
#                          services start successfully after a      #
#                          previous failure (status was ALERT).     #
...
send_recovery_mail() {
    ...
    local body="All services started successfully on ${HOST_ID}"
    if (( DRY_RUN )); then
        echo "[dry-run] would send recovery email to: ${ALERT_EMAIL}"; return 0
    fi
    # shellcheck disable=SC2086
    echo "$body" | mail -s "Service start recovery on ${HOST_ID}" $ALERT_EMAIL
...
alert() {
    local detail="$*"
    local body="Service start failure on ${HOST_ID}: ${detail}"
    ...
    if should_send_email; then
        # shellcheck disable=SC2086
        echo "$body" | mail -s "Service start alert on ${HOST_ID}" $ALERT_EMAIL
        mark_email_sent
...
Start any SERVICES entry that is not running. Idempotent: already-running
services are left untouched. Alerts on start failure (not on start action).
Sends a recovery email when all services succeed after a previous failure.
...
#   - If not running: attempts systemctl start.                #
#   - On start failure: alerts and updates status to ALERT.    #
#   - On full success: updates status to OK and sends recovery #
#     email if previous run had failures.                      #
################################################################
...
for svc in "${SERVICES[@]}"; do
    if pgrep -x "$svc" >/dev/null 2>&1; then
        printf '%b%-24s already running%b\n' "$GREEN" "$svc" "$RST"
        log_to "$EXECUTION_LOG" "RUNNING ${svc}"
    else
        if (( DRY_RUN )); then
            printf '%b%-24s would start%b\n' "$RED" "$svc" "$RST"
            log_to "$EXECUTION_LOG" "DRY-RUN would start ${svc}"
        else
            printf '%b%-24s starting...%b\n' "$RED" "$svc" "$RST"
            if systemctl start "$svc" 2>/dev/null; then
                printf '%b%-24s started%b\n' "$GREEN" "$svc" "$RST"
                log_to "$EXECUTION_LOG" "STARTED ${svc}"
                started+=("$svc")
            else
                printf '%b%-24s FAILED to start%b\n' "$RED" "$svc" "$RST" >&2
...
if (( ${#failed_to_start[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "RESULT ${#failed_to_start[@]} service(s) failed to start: ${failed_to_start[*]}"
    if [[ "$current_status" != "ALERT" ]]; then
        set_status ALERT
        alert "failed to start: ${failed_to_start[*]}"
    else
        log_to "$EXECUTION_LOG" "Already in ALERT state"
    fi
elif (( ${#started[@]} > 0 )); then
    log_to "$EXECUTION_LOG" "RESULT started: ${started[*]}"
    if [[ "$current_status" == "ALERT" ]]; then
        set_status OK
        send_recovery_mail
    else
    ...
    log_to "$EXECUTION_LOG" "RESULT all services already running"
    if [[ "$current_status" == "ALERT" ]]; then
        set_status OK
        send_recovery_mail
    fi
