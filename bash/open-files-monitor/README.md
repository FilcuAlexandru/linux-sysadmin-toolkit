...
cd linux-sysadmin-toolkit/bash/open-files-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/open-files-monitor/open-files-monitor.sh \
     -o /opt/scripts/open-files-monitor.sh
```
...
ALERT_EMAIL=""
EMAIL_INTERVAL="3600"
STATE_FILE="${SCRIPT_DIR}/open-files-monitor.email.state"
```
...
| `STATE_FILE` | `<script_dir>/open-files-monitor.email.state` | Stores the timestamp of the last sent email. |

### Logging

```bash
LOG_DIR="${SCRIPT_DIR}/logs"
ERROR_LOG="${LOG_DIR}/open-files-monitor-error.log"
EXECUTION_LOG="${LOG_DIR}/open-files-monitor-execution.log"
...
| `ERROR_LOG` | `<LOG_DIR>/open-files-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/open-files-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
```bash
MAINTENANCE_FILE="${SCRIPT_DIR}/open-files-monitor.maintenance"
LOCK_FILE="${SCRIPT_DIR}/open-files-monitor.lock"
STATUS_FILE="${SCRIPT_DIR}/open-files-monitor.status"
```
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/open-files-monitor-error.log
  Execution log:               /opt/scripts/logs/open-files-monitor-execution.log
...
./open-files-monitor.sh --maintenance
...
scripts/
├── open-files-monitor.sh
├── open-files-monitor.status
├── open-files-monitor.email.state
├── open-files-monitor.lock
├── open-files-monitor.maintenance
└── logs/
    ├── open-files-monitor-error.log
    ├── open-files-monitor-error.log.2026-06-01_120000
    ├── open-files-monitor-execution.log
    └── open-files-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/open-files-monitor.status"
STATE_FILE="/tmp/open-files-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: open-files-monitor
            image: alpine/bash
            command: ["/scripts/open-files-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
sudo systemctl enable --now open-files-monitor.timer
```

### Checkmk (local check)

```bash
cp open-files-monitor.sh /usr/lib/check_mk_agent/local/open-files-monitor.sh
```
...
```bash
echo "open_files_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/open-files-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/open-files-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/open-files-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/open-files-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/open-files-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/open-files-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/open-files-monitor.status` | Tracks OK/ALERT state. |
