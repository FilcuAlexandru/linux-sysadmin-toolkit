...
# Bash script that reports allocated file descriptors versus the kernel maximum. #
# Reads /proc/sys/fs/file-nr directly; no external binary required.              #
# Warns before 'Too many open files' (EMFILE/ENFILE) takes down services.        #
...
######################################################################
# File-descriptor usage threshold.                                  #
#   - Alert fires when allocated FDs exceed this percent of the      #
#     kernel maximum (fs.file-max).                                  #
######################################################################
...
#   - Toggle with: ./open-files-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="Open file-descriptor usage back below threshold on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    if [[ -r /proc/sys/fs/file-nr ]]; then
        printf '  %-28s %b\n' "/proc/sys/fs/file-nr" "$ok"
    else
        printf '  %-28s %b (cannot read FD statistics)\n' "/proc/sys/fs/file-nr" "$miss"
    fi
    for bin in awk mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                awk) printf '  %-28s %b (required for percentage math)\n' "$bin" "$miss" ;;
    ...
    printf '  %-28s %s\n' "Threshold:" "${THRESHOLD}% of fs.file-max"
...
alert_body() {
    command -v lsof >/dev/null 2>&1 || { echo "  (lsof not available for per-process breakdown)"; return; }
    echo "Top FD consumers (count PID):"
    lsof -n 2>/dev/null | awk 'NR>1{c[$2]++} END{for(p in c) print c[p], p}' | sort -rn | head -5 | awk '{printf "  %-8s %s\n", $1, $2}'
}
...
evaluate() {
    [[ -r /proc/sys/fs/file-nr ]] || { OVER=0; SUMMARY="/proc/sys/fs/file-nr unreadable"; DETAIL="no FD stats"; return; }
    local alloc unused maxf
    read -r alloc unused maxf < /proc/sys/fs/file-nr
    [[ "$alloc" =~ ^[0-9]+$ && "$maxf" =~ ^[0-9]+$ && "$maxf" -gt 0 ]] || { OVER=0; SUMMARY="FD stats unparseable"; DETAIL="bad file-nr"; return; }
    local pct over
    read -r pct over < <(awk -v a="$alloc" -v m="$maxf" -v t="$THRESHOLD" 'BEGIN{p=a/m*100; printf "%.1f %d\n", p, (p>t?1:0)}')
    OVER=$over
    if (( over )); then
        SUMMARY="Open FDs: ${alloc}/${maxf} (${pct}%) above ${THRESHOLD}%"; DETAIL="${alloc}/${maxf} = ${pct}%"
    else
        SUMMARY="Open FDs: ${alloc}/${maxf} (${pct}%)"; DETAIL="${alloc}/${maxf} = ${pct}%"
    fi
}
...
Report system-wide open file descriptors and alert above ${THRESHOLD}% of fs.file-max.
