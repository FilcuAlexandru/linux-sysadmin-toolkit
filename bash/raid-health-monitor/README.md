...
cd linux-sysadmin-toolkit/bash/raid-health-monitor

# Or fetch just this script with curl.
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/linux-sysadmin-toolkit/main/bash/raid-health-monitor/raid-health-monitor.sh \
     -o /opt/scripts/raid-health-monitor.sh
```
...
| `STATE_FILE` | `<script_dir>/raid-health-monitor.email.state` | Stores the timestamp of the last sent email. |
...
| `ERROR_LOG` | `<LOG_DIR>/raid-health-monitor-error.log` | Alerts, emails, and recoveries only. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/raid-health-monitor-execution.log` | Every run (start, result, end). `""` = disabled. |
...
Configuration:
  ...
  Error log:                   /opt/scripts/logs/raid-health-monitor-error.log
  Execution log:               /opt/scripts/logs/raid-health-monitor-execution.log
...
./raid-health-monitor.sh --maintenance
...
scripts/
├── raid-health-monitor.sh
├── raid-health-monitor.status
├── raid-health-monitor.email.state
├── raid-health-monitor.lock
├── raid-health-monitor.maintenance
└── logs/
    ├── raid-health-monitor-error.log
    ├── raid-health-monitor-error.log.2026-06-01_120000
    ├── raid-health-monitor-execution.log
    └── raid-health-monitor-execution.log.2026-06-01_120000
```
...
```bash
HOSTNAME_LABEL="app-prod-01"
STATUS_FILE="/tmp/raid-health-monitor.status"
STATE_FILE="/tmp/raid-health-monitor.email.state"
LOG_DIR="/tmp/logs"
```
    ...
    spec:
      template:
        spec:
          ...
          - name: raid-health-monitor
            image: alpine/bash
            command: ["/scripts/raid-health-monitor.sh"]
            env:
            - name: HOSTNAME_LABEL
              valueFrom:
                fieldRef:
...
```bash
cp raid-health-monitor.sh /usr/lib/check_mk_agent/local/raid-health-monitor.sh
```
...
```bash
echo "raid_health_monitor_alert{host=\"${HOST_ID}\"} 1" \
    > /var/lib/node_exporter/raid-health-monitor.prom
```
...
| `STATE_FILE` | `<script_dir>/raid-health-monitor.email.state` | Last-email timestamp file. |
| `LOG_DIR` | `<script_dir>/logs/` | Log directory (auto-created). |
| `ERROR_LOG` | `<LOG_DIR>/raid-health-monitor-error.log` | Error/alert log. `""` = disabled. |
| `EXECUTION_LOG` | `<LOG_DIR>/raid-health-monitor-execution.log` | Execution log. `""` = disabled. |
| `LOG_RETENTION_DAYS` | `14` | Days to keep logs; `0` = keep forever. |
| `HOSTNAME_LABEL` | `""` *(auto)* | Custom hostname for alerts and logs. |
| `MAINTENANCE_FILE` | `<script_dir>/raid-health-monitor.maintenance` | Maintenance marker. |
| `LOCK_FILE` | `<script_dir>/raid-health-monitor.lock` | Instance lock file for `flock`. |
| `STATUS_FILE` | `<script_dir>/raid-health-monitor.status` | Tracks OK/ALERT state. |
