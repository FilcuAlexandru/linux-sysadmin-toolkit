...
# Bash script that resolves a list of hostnames and alerts on any failure.     #
# Tries getent, host, and nslookup in turn so it works on minimal systems.     #
# Detects broken resolv.conf, unreachable resolvers, and split-horizon issues. #
...
#   - TARGETS : space-separated hostnames to resolve.               #
#   - TIMEOUT : per-query timeout in seconds (host/nslookup).       #
######################################################################
...
#   - Toggle with: ./dns-resolution-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="DNS resolution restored on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    echo "Prerequisites:"
    :
    for bin in getent host nslookup mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            printf '  %-28s %b\n' "$bin" "$ok"
        else
            case "$bin" in
                getent) printf '  %-28s %b (primary resolver path unavailable)\n' "$bin" "$miss" ;;
                host) printf '  %-28s %b (host fallback unavailable)\n' "$bin" "$miss" ;;
                nslookup) printf '  %-28s %b (nslookup fallback unavailable)\n' "$bin" "$miss" ;;
...
alert_body() {
    ...
    grep -E '^(nameserver|search|domain)' /etc/resolv.conf 2>/dev/null | sed 's/^/  /' || echo "  (unreadable)"
}
...
evaluate() {
    local total=0 ok=0 fails="" t good
    for t in $TARGETS; do
        total=$((total+1)); good=0
        if command -v getent >/dev/null 2>&1; then getent ahosts "$t" >/dev/null 2>&1 && good=1; fi
        if (( ! good )) && command -v host >/dev/null 2>&1; then host -W "$TIMEOUT" "$t" >/dev/null 2>&1 && good=1; fi
        if (( ! good )) && command -v nslookup >/dev/null 2>&1; then nslookup "$t" >/dev/null 2>&1 && good=1; fi
        if (( good )); then ok=$((ok+1)); else fails+="$t "; fi
    done
    if [[ -n "$fails" ]]; then
        OVER=1; SUMMARY="DNS resolution FAILED for: ${fails}(${ok}/${total} ok)"; DETAIL="failed: ${fails}"
    else
        OVER=0; SUMMARY="DNS resolution OK (${ok}/${total})"; DETAIL="${ok}/${total} resolved"
    fi
}
...
Resolve each hostname in TARGETS and alert if any cannot be resolved.
