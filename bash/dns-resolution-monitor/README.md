...
```bash
./dns-resolution-monitor.sh --maintenance   # toggle on/off
```
...
- *Idempotent / status-aware:* the script alerts **once** when the condition appears, stays silent while
  it persists, and sends a single recovery email when it clears (state kept in `dns-resolution-monitor.status`).
