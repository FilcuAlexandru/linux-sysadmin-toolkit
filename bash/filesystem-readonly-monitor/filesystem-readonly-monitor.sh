...
# Bash script that detects writable filesystems remounted read-only by the kernel. #
# A read-only remount usually signals underlying disk or controller errors.        #
# Parses /proc/mounts directly and can enforce expected-writable mount points.     #
...
#   - WATCH : space-separated mount points that MUST be read-write   #
#             (e.g. "/ /var"). Leave empty to scan all real disk    #
...
#   - Toggle with: ./filesystem-readonly-monitor.sh --maintenance            #
...
send_recovery_mail() {
    ...
    local body="All monitored filesystems are read-write again on ${HOST_ID}"
    if (( DRY_RUN )); then
...
check_prerequisites() {
    ...
    if [[ -r /proc/mounts ]]; then
        ...
        printf '  %-28s %b (mount table unavailable)\n' "/proc/mounts" "$miss"
    fi
    for bin in awk mail flock find; do
        if command -v "$bin" >/dev/null 2>&1; then
            ...
            case "$bin" in
                awk) printf '  %-28s %b (not strictly required; used by helpers)\n' "$bin" "$miss" ;;
    ...
    printf '  %-28s %s\n' "Watch mounts:" "${WATCH:-all real filesystems}"
...
alert_body() {
    echo "Mount table (real filesystems):"
    awk '$3 ~ /^(ext2|ext3|ext4|xfs|btrfs|reiserfs|jfs|f2fs)$/ {printf "  %-20s %-8s %s\n", $2, $3, $4}' /proc/mounts 2>/dev/null
    echo "Check dmesg for I/O errors if a filesystem is unexpectedly read-only."
}
...
evaluate() {
    [[ -r /proc/mounts ]] || { OVER=0; SUMMARY="/proc/mounts unreadable"; DETAIL="no mount table"; return; }
    local ro="" checked=0 dev mnt type opts rest
    if [[ -n "$WATCH" ]]; then
        local w found state
        for w in $WATCH; do
            checked=$((checked+1)); found=0
            while read -r dev mnt type opts rest; do
                [[ "$mnt" == "$w" ]] || continue
                found=1
                [[ ",$opts," == *",ro,"* ]] && ro+="$mnt "
            done < /proc/mounts
            (( found )) || ro+="${w}(not-mounted) "
        done
    else
        while read -r dev mnt type opts rest; do
            case "$type" in ext2|ext3|ext4|xfs|btrfs|reiserfs|jfs|f2fs)
                checked=$((checked+1))
                [[ ",$opts," == *",ro,"* ]] && ro+="$mnt " ;;
            esac
        done < /proc/mounts
    ...
    if [[ -n "$ro" ]]; then
        OVER=1; SUMMARY="Read-only filesystem(s): ${ro}"; DETAIL="ro/missing: ${ro}"
    else
        OVER=0; SUMMARY="All ${checked} checked filesystem(s) read-write"; DETAIL="${checked} rw"
    fi
}
...
Detect real filesystems remounted read-only and alert (often signals disk errors).
