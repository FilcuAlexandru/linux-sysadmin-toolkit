...
`file-change-monitor.sh` requires `inotifywait`, which in turn requires the
kernel `inotify` subsystem. Most container runtimes expose this by default.

```bash
docker run --rm \
  -v /host/path/to/watch:/watch \
  alpine/bash \
  bash /scripts/file-change-monitor.sh
```

Set `WATCH_DIR="/watch"` and `HOSTNAME_LABEL` to a meaningful name inside the container.


### Cron

This script is a daemon, not a one-shot script — do not run it via cron. Use a systemd service instead (see `--install-service`).
...
```bash
cp file-change-monitor.sh /usr/lib/check_mk_agent/local/file-change-monitor.sh
```

Adapt the `alert()` function body for Checkmk-compatible output.

### logrotate (for log rotation while running)

Because this script runs continuously, rotation only happens at startup. For mid-run rotation use `logrotate` with `copytruncate`:
